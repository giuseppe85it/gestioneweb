...
Delta dal 2026-05-08 al 2026-05-16. Origine: `DIARIO_DECISIONI.md` + `AUDIT_NEXT_COMPLETO_2026-05-16.md`.

- `[AGGIORNATO]` DV-33 "Autisti NEXT writer reale" — stato resta **RISCHIO REALE**, ma il perimetro è ridotto. Decisione 2026-05-09 introduce 4 scope barrier per CC che aprono scritture business mirate: `SEGNALAZIONI_WRITE_SCOPE` (markSegnalazioneChiusa), `CONTROLLI_WRITE_SCOPE` (markControlloChiuso), `RICHIESTE_WRITE_SCOPE` (markRichiestaEvasa), `DELETE_MEZZO_WRITE_SCOPE` (hard-delete cascade), `CHIUSURA_DA_EVENTO_WRITE_SCOPE` (chiusura ciclo eventi), `CENTRO_CONTROLLO_LEGAME_WRITE_SCOPE` (aggancio inverso PROMPT 47), `MANUTENZIONE_DAFARE_CREATE_WRITE_SCOPE`. La pagina `/next/autisti-admin` però **monta ancora `AutistiAdmin.tsx` madre**: la sostituzione NEXT è descritta in `AUDIT_NEXT_COMPLETO_2026-05-16.md` cap. 5.3. Vedi anche file 05 aggiornato.
- `[AGGIORNATO]` DV-22 `@preventivi_approvazioni` writer NEXT — invariato: rischio reale, writer business resta in madre `src/pages/CapoCostiMezzo.tsx:647`.
- `[AGGIORNATO]` DV-24 `@analisi_economica_mezzi` writer NEXT — invariato + decisione 2026-05-04 punto 5: la collection è **esclusa by design** dal motore generico v1 (narrativa IA viola Zero-Invenzioni). Writer reale resta in madre.
- `[AGGIORNATO]` DV-20 / DV-21 Firestore/Storage rules — invariati come stato (catch-all signed-in). Il barrier `cloneWriteBarrier.ts` ora ha 15+ scope (~985 righe) ed è una mitigazione client-side importante ma non sostituisce le rules server-side. Riferimento `AUDIT_NEXT_COMPLETO_2026-05-16.md` cap. 9 oss. 3.
- `[AGGIORNATO]` DV-23 `@costiMezzo` writer — invariato (delete/update via documenti costi domain già DIMOSTRATO).
- `[AGGIORNATO]` DV-27 `@mezzi_foto_viste` / DV-28 `@mezzi_hotspot_mapping` — invariati come writer, ma la decisione 2026-05-04 punto 7 prevede coordinate hotspot `x/y/areaId/uploadedAt` in allowedFields (ancora aperta).
- `[NUOVO]` Regola permanente `AUDIT-CERCA-PER-TARGA` in [AGENTS.md:213-231](../../../AGENTS.md#L213-L231) — derivata dall'errore PROMPT 45 T5 corretto in PROMPT 46 (caso TI298409): negli audit di record correlati a un mezzo, cercare sempre per targa, mai solo per legame. Record orfani con `linkedLavoroId`/`chiusuraRefId`/`origineRefId` che puntano a target inesistenti esistono nella realtà e vanno identificati esplicitamente. Riferimento [docs/_live/AUDIT_TI298409_RICCARDO_FENDERICO_2026-05-15.md](../../_live/AUDIT_TI298409_RICCARDO_FENDERICO_2026-05-15.md).
- `[NUOVO]` Regola permanente `TIMESTAMP-MAI-DA-CLICK` in [AGENTS.md:235-276](../../../AGENTS.md#L235-L276) — i campi temporali persistiti (`chiusuraData`, `dataPresaInCarico`, `dataEsecuzione`, `dataChiusura`) **non devono mai essere scritti con `Date.now()` come effetto collaterale di operazioni non temporali** (aggancio, merge, sgancio, completamento automatico). Per chiusure derivate il timestamp eredita dal record di origine. Solo azioni utente esplicitamente temporali producono `Date.now()`. Riferimento [docs/_live/REPORT_PROMPT50_TIMESTAMP_FIX_2026-05-15.md](../../_live/REPORT_PROMPT50_TIMESTAMP_FIX_2026-05-15.md).
- `[NUOVO]` Punti audit aperti aggiunti dopo 2026-05-09 (segnalati come "audit-only" in `DIARIO_DECISIONI.md`, non ancora chiusi):
  - whitelist categorie tipografiche unificata (Sinottica V2 vs `nextCentroControlloDomain.classifyMezzoCategoria`) — vedi PROMPT 27.13 INTERVENTO 6.
  - hard-delete fallback `mezzoId` cascade — vedi PROMPT 27.13 INTERVENTO 7.
  - aggregator badge "ASSE X da verificare" Gomme Sinottica hardcoded `null` (`NextCentroControlloSinottica.tsx:760`) — vedi PROMPT 27.13 INTERVENTO 5.
