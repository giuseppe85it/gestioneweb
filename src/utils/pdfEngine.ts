...
async function buildRifornimentiMensiliMonthlyDocument(
  input: RifornimentiMensiliPdfInput
): Promise<{ doc: jsPDF; fileName: string }> {
  const WARNING_GLYPH = "*";
  const monthNum = Number(input?.mese);
  const month =
    Number.isFinite(monthNum) && monthNum >= 1 && monthNum <= 12 ? monthNum : 1;
  const yearNum = Number(input?.anno);
  const year = Number.isFinite(yearNum) ? yearNum : new Date().getFullYear();
  ...
  const totalLitri = items.reduce(
    (sum, item) => sum + (toFinitePdfNumber(item?.litri) ?? 0),
    0,
  );
  ...
  const mediaFlotta = input?.mediaFlotta;
  if (mediaFlotta && mediaFlotta.value !== null) {
    const valueLabel = `${mediaFlotta.value
      .toFixed(2)
      .replace(".", ",")} km/L`;
    const rangeLabel =
    ...
    doc.setDrawColor(...COLORS.headerLine);
    doc.setFillColor(255, 255, 255);
    doc.roundedRect(14, currentY, 268, 14, 3, 3, "FD");
    doc.setTextColor(...COLORS.textBlack);
    doc.setFont("helvetica", "bold");
    doc.setFontSize(10);
    doc.text(`Media flotta del mese: ${valueLabel}`, 18, currentY + 6);
    if (rangeLabel) {
      doc.setFont("helvetica", "normal");
      doc.setFontSize(9);
      doc.text(rangeLabel, 18, currentY + 11);
    }
    currentY += 18;
  } else if (mediaFlotta && mediaFlotta.value === null) {
    doc.setTextColor(...COLORS.textBlack);
    doc.setFont("helvetica", "italic");
    doc.setFontSize(9);
    doc.text(
      "Media flotta non calcolabile (dati insufficienti).",
      18,
      currentY + 5,
    );
    currentY += 10;
  ...
  const head = [
    ["Data", "Targa", "Autista", "Litri", "Km", "Fonte", "Media km/L"],
  ];

  const body = items.map((item) => {
    const fonte =
      safeStr(item?.fonteLabel) ||
  ...
  const ANOMALA_BG: [number, number, number] = [255, 245, 244];
  ...
  autoTable(doc, {
    ...
    didParseCell: (data) => {
      ...
      if (idx >= 0 && idx < anomalaFlags.length && anomalaFlags[idx]) {
        data.cell.styles.fillColor = ANOMALA_BG;
      }
    },
  });
  ...
  doc.setFont("helvetica", "bold");
  doc.setFontSize(10);
  doc.text(`Rifornimenti: ${items.length}`, 14, summaryY);
  doc.text(
    `Totale litri: ${formatPdfNumber(totalLitri, 2)} L`,
    80,
    summaryY,
  );

  const anomalie = input?.anomalieSummary;
  if (anomalie && anomalie.totalRows > 0) {
    summaryY += 8;
  ...
  const dettaglio = input?.anomalieDettaglio;
  if (dettaglio && dettaglio.length > 0) {
    ...
    const BULLET_LINE_HEIGHT = 5;
    const ENTRY_GAP = 3;

    let detailY = summaryY + 10;

    const ensureSpace = async (needed: number) => {
      if (detailY + needed > pageHeight - FOOTER_RESERVE) {
        doc.addPage();
        detailY = await drawHeader(doc, title);
      }
    };
    ...
    doc.setFont("helvetica", "bold");
    doc.setFontSize(14);
    doc.text("Dettaglio anomalie rilevate", LEFT_MARGIN, detailY);
    detailY += 2;
    doc.setDrawColor(...COLORS.headerLine);
    doc.setLineWidth(0.3);
    doc.line(LEFT_MARGIN, detailY, pageWidth - RIGHT_MARGIN, detailY);
    detailY += 5;
    ...
    for (const entry of dettaglio) {
      ...
      const entryHeight =
        HEADER_HEIGHT + bulletsLineCount * BULLET_LINE_HEIGHT + ENTRY_GAP;

      await ensureSpace(entryHeight);

      doc.setTextColor(...COLORS.textBlack);
      doc.setFont("helvetica", "bold");
      doc.setFontSize(12);
      const headerText = `${WARNING_GLYPH} ${entry.targa} - ${entry.dataDisplay} (${entry.autistaNome})`;
      doc.text(headerText, LEFT_MARGIN, detailY);
      detailY += HEADER_HEIGHT;
      ...
      for (const lines of wrappedBullets) {
        for (const line of lines) {
          doc.text(line, LEFT_MARGIN + BULLET_INDENT, detailY);
          detailY += BULLET_LINE_HEIGHT;
        }
      }
  ...
  const datePart = `${String(month).padStart(2, "0")}-${year}`;
  const fileName = `${sanitizeFileName(`rifornimenti_mensili_${datePart}`)}.pdf`;
  return { doc, fileName };
}
...
export async function generateRifornimentiMensiliPDFBlob(
  ...
  const { doc, fileName } = await buildRifornimentiMensiliMonthlyDocument(input);
  return { blob: doc.output("blob") as Blob, fileName };
}
...
// Colonna "No pausa": "X" SOLO quando la pausa NON Ã¨ stata fatta (noPausa=true, cioÃ¨
// r.pausa === "No"); vuota se la pausa Ã¨ stata fatta. Footer Monte ore: due righe NON
// compensate (+ verde / âˆ’ rosso). renderOrariCartellinoPage = una pagina (singolo + massivo).
// =============================================================================
...
// "X" solo se la pausa NON Ã¨ stata fatta (noPausa=true â†’ pausaLabel "No").
