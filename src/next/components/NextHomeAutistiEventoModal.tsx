...
export default function NextHomeAutistiEventoModal({
  ...
  const canPrev: boolean = hasNav && (eventIndex ?? 0) > 0;
  const canNext: boolean =
    hasNav && (eventIndex ?? 0) < ((eventsCount ?? 1) - 1);

  useEffect(() => {
    if (!hasNav) return;
    const handleArrow = (evt: KeyboardEvent) => {
      if (evt.key === "ArrowLeft" && canPrev && onPrevEvent) {
        evt.preventDefault();
        onPrevEvent();
      } else if (evt.key === "ArrowRight" && canNext && onNextEvent) {
        evt.preventDefault();
        onNextEvent();
      }
    };
  ...
  }, [hasNav, canPrev, canNext, onPrevEvent, onNextEvent]);
      ...
      <div className="aix-backdrop" onClick={closeDetails}>
        ...
        >
          ...
          <div className="aix-body">
            ...
            >
              {markError && (
                <div style={{ color: "#b3261e", font: "500 12.5px/1.4 system-ui" }}>
                  {markError}
                </div>
              )}
              <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
                {event.tipo === "richiesta_attrezzature" && onMarkEvasa && (
                  ...
                  >
                    {marking ? "Salvataggio…" : "Marca evasa"}
                  </button>
                )}
                {event.tipo === "segnalazione" && onMarkChiusa && (
                  ...
                  >
                    {marking ? "Salvataggio…" : "Marca chiusa"}
                  </button>
                )}
                {event.tipo === "controllo" && onMarkChiuso && (
                  ...
                  >
                    {marking ? "Salvataggio…" : "Marca chiuso"}
                  </button>
                )}
              </div>
            </div>
