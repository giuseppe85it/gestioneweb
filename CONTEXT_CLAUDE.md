...
## 3. STATO ATTUALE
- Ultimo task completato: separazione del ramo reale `Documento mezzo -> Libretto` nel modale review operativo di `/next/ia/interna` il `2026-04-16`. `src/next/NextInternalAiPage.tsx` importa e monta ora `src/next/internal-ai/NextEstrazioneLibretto.tsx` solo quando `documentReviewModalState.isOpen && activeDocumentReviewRoute` e la route attiva corrisponde al caso libretto; il resto del modale review resta invariato. Gli stili dedicati sono isolati in `src/next/internal-ai/next-estrazione-libretto.css`. Verifiche tecniche: `npx eslint src/next/NextInternalAiPage.tsx src/next/internal-ai/NextEstrazioneLibretto.tsx` `OK`, `npm run build` `OK`. Verifica browser su `http://127.0.0.1:5173/next/ia/interna` eseguita con iniezione locale di stato documentale nel runtime reale: il caso `Documento mezzo -> Libretto` monta il nuovo componente separato, mentre `Preventivo fornitore` continua a usare il modale generico; nessun errore console nuovo imputabile alla patch, restano i `403` Storage gia noti.
...
- Il backend IA separato espone `/internal-ai-backend/documents/manutenzione-analyze` e restituisce review strutturata con:
  - riassunto breve;
  - targa;
  - fornitore;
  - data;
