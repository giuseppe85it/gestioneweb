...
| NEXT IA hub | Controparti clone di `IA`, `apikey`, `libretto`, `documenti`, `copertura-libretti`, `libretti-export`; `/next/ia/documenti` e ora una vista `Documenti e costi` con gruppi per fornitore, filtro `DDT` UI/fallback, ricerca locale, modale dettaglio, `Riapri review`, targa cliccabile verso il Dossier e cambio valuta inline source-aware sui soli dataset consentiti (`@documenti_mezzi`, `@documenti_magazzino`, `@documenti_generici`, `@costiMezzo`), ma ancora senza righe documento perche `NextIADocumentiArchiveItem` non espone `voci`. | in sviluppo |
...
### Aggiornamento rapido 2026-04-21
- Il backend IA separato accetta ora `pages[]` opzionale solo su `/internal-ai-backend/documents/manutenzione-analyze`.
- Il ramo `Archivista documenti -> Fattura / DDT + Manutenzione` puo analizzare piu immagini/PDF dello stesso documento come un unico input logico.
- Dopo l'archiviazione, lo stesso ramo puo aprire uno step 2 opzionale che crea una manutenzione reale via `saveNextManutenzioneBusinessRecord()`.
- La deroga `/next/ia/archivista` in `cloneWriteBarrier.ts` include ora solo `@manutenzioni`, `@inventario`, `@materialiconsegnati` oltre alle eccezioni Archivista gia presenti.

### Aggiornamento rapido 2026-04-15
- `Archivista documenti` ha ora il ramo `Fattura / DDT + Manutenzione` su backend OpenAI server-side dedicato.
- Il bridge `src/next/internal-ai/ArchivistaManutenzioneBridge.tsx` non usa piu `estrazioneDocumenti`.
- Il backend IA separato espone `/internal-ai-backend/documents/manutenzione-analyze`, accetta `contentBase64` singolo oppure `pages[]` opzionale solo per questo ramo e restituisce review strutturata con:
  - riassunto breve;
  - targa;
  - fornitore;
  - data;
  ...
  - avvisi;
  - campi mancanti.
- `Magazzino` resta invariato in questo step.

### Aggiornamento rapido 2026-04-15 bis
- La schermata IA 2 si presenta ora come `Importa documenti`.
- `src/next/NextIAArchivistaPage.tsx` usa topbar, hero e card `Destinazione rilevata` coerenti con il layout approvato.
- I bridge `Magazzino`, `Manutenzione`, `Documento mezzo` e `Preventivo + Magazzino` restano invariati nella logica ma usano il vocabolario visuale `iai-*` dentro la schermata reale.
- Nessuna modifica a backend, estrazione, writer business o barrier in questo passaggio.

### Aggiornamento rapido 2026-04-22 bis
- Fix leak: i preventivi Archivista Manutenzione (`ambitoPreventivo: "manutenzione"`) non compaiono piu nelle superfici procurement NEXT (`Materiali da ordinare`, `Magazzino documenti-costi`, IA interna).
- `nextProcurementDomain.ts`: `readNextProcurementSnapshot()` filtra ora `ambitoPreventivo === "manutenzione"` in ingresso.
- `nextDocumentiCostiDomain.ts`: `readNextDocumentiCostiProcurementSupportSnapshot()` estende il filtro `preventiviItems` con la stessa condizione.
- `internalAiUnifiedIntelligenceEngine.ts`: il loop `raw_storage_doc` filtra `storage/@preventivi` prima della materializzazione.
- Record storici senza `ambitoPreventivo` restano inclusi (retrocompatibilita).
- Writer, barrier, `storage.rules` non toccati.
