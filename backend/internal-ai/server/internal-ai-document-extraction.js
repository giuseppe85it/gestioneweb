...
const CISTERNA_DOCUMENTI_PROMPT = `
...
`;
const CISTERNA_SCHEDE_CELLS_PROMPT = `
Riceverai 3 immagini distinte nello stesso ordine:
1) DATA
2) TARGA
3) LITRI

Trascrivi SOLO cio che leggi. Non correggere, non normalizzare, non dedurre.
Se non sei sicuro scrivi "INCERTO". Se la cella e vuota scrivi "".
...
function normalizeCisternaDate(value) {
...
function cisternaYearMonthFromDate(dataDocumento) {
...
function normalizeCisternaTipo(value, fileName) {
  const text = `${String(value ?? "")} ${String(fileName ?? "")}`.toLowerCase();
...
function normalizeCisternaValuta(value) {
...
function parseCisternaNumberOrNull(value) {
...
function normalizeCisternaDocumentExtractResult(raw, fileName) {
  const source = raw && typeof raw === "object" ? raw : {};
  const tipoDocumento = normalizeCisternaTipo(source.tipoDocumento, fileName);
  const dataDocumento = normalizeCisternaDate(source.dataDocumento);
  const litriTotali = parseCisternaNumberOrNull(source.litriTotali);
  const totaleDocumento = parseCisternaNumberOrNull(source.totaleDocumento);
  const valuta = normalizeCisternaValuta(source.valuta);
  const fornitore = toNullIfEmpty(source.fornitore);
  const destinatario = toNullIfEmpty(source.destinatario);
  const numeroDocumento = toNullIfEmpty(source.numeroDocumento);
  const prodotto = toNullIfEmpty(source.prodotto);
  const testo = toNullIfEmpty(source.testo);
  const yearMonth = cisternaYearMonthFromDate(dataDocumento);
  const forcedReview = Boolean(source.daVerificare);
  const reasonFromModel = toNullIfEmpty(source.motivoVerifica);
...
function normalizeCisternaSchedaStatus(rawValue, kind) {
  ...
  if (kind === "targa" && !/^TI\s*\d{6}$/i.test(raw)) {
...
function normalizeCisternaSchedaCellRow(rawRow, rowIndex) {
  const source = rawRow && typeof rawRow === "object" ? rawRow : {};
  const dataEval = normalizeCisternaSchedaStatus(source.data_raw, "data");
  const targaEval = normalizeCisternaSchedaStatus(source.targa_raw, "targa");
  const litriEval = normalizeCisternaSchedaStatus(source.litri_raw, "litri");
...
function buildCisternaSchedeCellsEnvelope(rows, rawText) {
...
function buildProviderSystemPrompt(profile = "magazzino", subtype = null) {
  ...
  if (profile === "documento_cisterna") {
    return (
      "Sei il parser documentale OpenAI della nuova IA interna del gestionale. " +
      "Leggi fatture e bollettini/DAS carburante della Cisterna Caravate. " +
      "Rispondi solo con JSON valido. " +
      "Non inventare mai dati: se un campo non e chiaramente leggibile usa null. " +
      "Estrai solo i campi richiesti dal contratto documenti cisterna."
    );
  }

  if (profile === "scheda_cisterna") {
    return (
      "Sei il parser OCR OpenAI della nuova IA interna del gestionale. " +
      "Leggi celle ritagliate di schede carburante Cisterna Caravate. " +
      "Rispondi solo con JSON valido. " +
      "Trascrivi solo cio che vedi nelle tre immagini DATA, TARGA e LITRI. " +
      "Non correggere, non normalizzare e non dedurre."
    );
  }
...
function buildProviderUserInstructions(args) {
  ...
  if ((args.profile ?? "magazzino") === "documento_cisterna") {
    return [
      CISTERNA_DOCUMENTI_PROMPT.trim(),
      `Nome file: ${args.fileName || "documento_cisterna"}.`,
      `Input provider: documento allegato come ${args.sourceHint === "image_document" ? "input_image" : "input_file/input_text"}.`,
    ].join("\n\n");
  }

  if ((args.profile ?? "magazzino") === "scheda_cisterna") {
    return [
      CISTERNA_SCHEDE_CELLS_PROMPT.trim(),
      `Riga batch: ${Number.isFinite(args.rowIndex) ? args.rowIndex : "n/d"}.`,
      "Input provider: tre immagini allegate come input_image nell'ordine DATA, TARGA, LITRI.",
    ].join("\n\n");
  }
...
async function runProviderCisternaSchedaCellExtraction(args) {
  const content = [
    {
      type: "input_text",
      text: buildProviderUserInstructions({
    ...
    {
      type: "input_image",
      image_url: args.dataBase64.startsWith("data:")
        ? args.dataBase64
        : `data:image/jpeg;base64,${args.dataBase64}`,
    },
    {
      type: "input_image",
      image_url: args.targaBase64.startsWith("data:")
        ? args.targaBase64
        : `data:image/jpeg;base64,${args.targaBase64}`,
    },
    {
      type: "input_image",
      image_url: args.litriBase64.startsWith("data:")
        ? args.litriBase64
        : `data:image/jpeg;base64,${args.litriBase64}`,
    },
  ];
    ...
    input: [
      {
        ...
        content: [
          {
            type: "input_text",
            text: buildProviderSystemPrompt("scheda_cisterna"),
          },
        ],
      },
      {
  ...
  return {
    parsed: parseProviderJson(response.output_text),
...
export async function extractInternalAiDocumentAnalysis(args) {
  ...
  if (profile === "scheda_cisterna") {
    ...
    if (!args.providerClient) {
      if (args.providerRequired) {
        throw new Error("Provider OpenAI non configurato per l'analisi schede Cisterna.");
      }
      return buildCisternaSchedeCellsEnvelope([], "");
    }

    const rows = [];
    const rawParts = [];
    for (const cell of cells) {
      if (!cell.dataBase64 || !cell.targaBase64 || !cell.litriBase64) {
        rows.push(
          normalizeCisternaSchedaCellRow(
            { data_raw: "", targa_raw: "", litri_raw: "" },
            cell.rowIndex,
          ),
        );
        continue;
      ...
      const providerResult = await runProviderCisternaSchedaCellExtraction({
        providerClient: args.providerClient,
        providerTarget: args.providerTarget,
        rowIndex: cell.rowIndex,
        dataBase64: cell.dataBase64,
        targaBase64: cell.targaBase64,
        litriBase64: cell.litriBase64,
      });
      rows.push(normalizeCisternaSchedaCellRow(providerResult.parsed, cell.rowIndex));
      rawParts.push(providerResult.rawText);
    }

    return buildCisternaSchedeCellsEnvelope(rows, rawParts.filter(Boolean).join("\n"));
  }
  ...
  if (profile === "documento_cisterna") {
    if (!args.providerClient) {
      if (args.providerRequired) {
        throw new Error("Provider OpenAI non configurato per l'analisi documentale Cisterna.");
      }
      return normalizeCisternaDocumentExtractResult({}, effectiveFileName);
    }

    const isEffectivePdf =
      normalizeText(effectiveMimeType).toLowerCase() === "application/pdf" ||
      normalizeText(effectiveFileName).toLowerCase().endsWith(".pdf") ||
      pagesContainPdf;
    const isEffectiveImage =
      hasPages ||
      normalizeText(effectiveMimeType).toLowerCase().startsWith("image/") ||
      ["image_document", "pdf_scan"].includes(tipoSorgente);
    let providerParsed = null;

    if (isEffectivePdf || isEffectiveImage) {
      providerParsed = await runProviderBinaryExtraction({
        providerClient: args.providerClient,
        providerTarget: args.providerTarget,
        profile,
    ...
    if (!providerParsed) {
      throw new Error("Il provider OpenAI non ha restituito un JSON valido per documento_cisterna.");
    }

    return normalizeCisternaDocumentExtractResult(providerParsed, effectiveFileName);
  }
