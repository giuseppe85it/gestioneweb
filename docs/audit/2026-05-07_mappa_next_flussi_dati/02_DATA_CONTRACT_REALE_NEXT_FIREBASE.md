...
| `storage/@lavori` | storage document con array/value | lavori/dossier (`nextLavoriDomain.ts:669`) | `NextLavoriDaEseguirePage.tsx:524`, `NextDettaglioLavoroPage.tsx:841` | id, descrizione, targa, urgenza, eseguito, chiHaEseguito, dataInserimento, dataEsecuzione | alto | DIMOSTRATO |
...
| `@lavori` | Lavori da eseguire | lista temporanea di lavori: `id`, `descrizione`, `targa`, `urgenza`, `eseguito`, sottoElementi | object array | si per descrizione | input UI | append a esistenti | `NextLavoriDaEseguirePage.tsx:522-524` |
| `@lavori` | Dettaglio lavoro | `eseguito`, `chiHaEseguito`, `dataEsecuzione`; oppure `descrizione`, `dataInserimento`; delete per id | object array | dipende | input UI | map/filter array | `NextDettaglioLavoroPage.tsx:839-894` |
