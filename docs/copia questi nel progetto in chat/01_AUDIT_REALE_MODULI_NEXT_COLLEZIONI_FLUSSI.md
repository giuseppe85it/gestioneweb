...
Sintesi dei delta dal 2026-05-08 al 2026-05-16. Origine: `DIARIO_DECISIONI.md` + audit data-based + `AUDIT_NEXT_COMPLETO_2026-05-16.md`. Per il dettaglio non duplicato qui, vedi `docs/_live/AUDIT_NEXT_COMPLETO_2026-05-16.md`.

- `[OBSOLETO]` Modulo Lavori NEXT come modulo scrivente — dismesso 2026-05-12/13/14 (decisioni J.1–J.11 + override J.10). Sidebar voce "Lavori" rimossa; route `/next/lavori-da-eseguire`, `/next/lavori-in-attesa`, `/next/lavori-eseguiti`, `/next/dettagliolavori/:lavoroId` ora redirect compat → `/next/manutenzioni`. Tabella sezione 0 righe Lavori (Lavori da eseguire / Lavori in attesa / Lavori eseguiti / Dettaglio lavoro) lette nel codice originale restano come traccia storica ma il modulo non scrive più. Strategia 3a: `@lavori` Firestore intoccabile, madre continua a scriverlo. (rif. DIARIO_DECISIONI 2026-05-12, 2026-05-13, 2026-05-14).
- `[AGGIORNATO]` Manutenzioni NEXT — nuovi stati `daFare`/`programmata`/`eseguita`/`chiusa_da_evento` (macchina chiusura ciclo eventi); aggancio retroattivo `gomme_evento` via [src/next/components/NextAggancioEventoModal.tsx](../../src/next/components/NextAggancioEventoModal.tsx) + helper [src/next/helpers/eventiCompatibili.ts](../../src/next/helpers/eventiCompatibili.ts). Campo `linkedLavoroId`/`linkedLavoroIds` mantiene il nome ma punta a `@manutenzioni` (decisione J.7). PDF Quadro separa "Manutenzioni risolte tramite eventi esterni" (rif. DIARIO_DECISIONI 2026-05-14).
- `[NUOVO]` Modulo Archivio Storico in Centro Controllo — `src/next/centroControllo/archivioStorico/` (22 file: hooks `useArchivio*`, rows `ArchivioRow*`, toolbar, sub-tabs, feed, kebab menu, confirm delete, day separator, mini timeline, empty state, foto veicolo). 4 sub-tab: Lavori (link inverso), Manutenzioni, Segnalazioni, Richieste. Soft-hide via `nascostoInArchivio` con scope `ARCHIVIO_HIDE_WRITE_SCOPE`. Riferimento: PROMPT 29.0 → 31.2 chiusura 2026-05-12.
- `[NUOVO]` Modulo Scadenze Collaudi scrivente `/next/scadenze-collaudi` — pagina dedicata + writer `nextScadenzeCollaudiWriter.ts` (`setPrenotazioneCollaudo`, `setPreCollaudo`, `markRevisioneCompletata`), scope `scadenze_collaudi_write_scope`. Sostituisce 3 modali read-only nel CC (decisione 2026-04-30, già attivo al 2026-05-07 ma da censire in tabella sezione 0).
- `[AGGIORNATO]` Centro di Controllo NEXT — torre operativa (decisione 2026-05-09): scope barrier `RIFORNIMENTI_WRITE_SCOPE`, `SEGNALAZIONI_WRITE_SCOPE`, `CONTROLLI_WRITE_SCOPE`, `RICHIESTE_WRITE_SCOPE`, `DELETE_MEZZO_WRITE_SCOPE` (vedi [src/utils/cloneWriteBarrier.ts:111-184](../../src/utils/cloneWriteBarrier.ts#L111-L184)); soft-delete pattern (`chiusa:true`/`chiuso:true`/`evasa:true` + `dataChiusura/dataEvasione` + `*_by`); hard-delete mezzo con doppia conferma (shift+click foto). Sinottica V2 stile Design isolato sotto `.cc-sinottica-scope-v2`. Cronologia mezzo `NextMezzoCronologiaModal.tsx`. Toggle `manutenzioneContrattoAttivo: boolean` su `@mezzi_aziendali`. Rimossi tab "Manutenzioni programmate" e "Richieste attrezzature" dal CC (ridondanti, dati nelle pill Sinottica).
- `[AGGIORNATO]` Chat IA NEXT `/next/chat` — V1 chiusa 100% al 2026-05-06 (PRE-data audit originale, ma stato qui era genericamente "DIMOSTRATO da route" — l'aggiornamento puntualizza: 5 viste certificate Driver360/Vehicle360/Site360/Euromecc360/Ricerca360 attive; modalità Zero-Invenzioni; Blocco 8 chiuso, Playwright 17-21 PASS 10/10; Registro Collection Firestore v1.0 STABLE; SPEC Motore Generico v1.0 STABLE.
- `[NUOVO]` Helper `frasestoriaRecord.ts` + componente `FraseStoriaRecord` — frase standard "<Tipo> del <data>, presa in carico il <data>, eseguita il <data>" + suffisso modalità chiusura. `StoriaRecordTimeline` deprecato (non eliminato). Allineate 8 superfici S1–S8 (rif. DIARIO_DECISIONI 2026-05-14).
- `[NUOVO]` Date NEXT unificate ISO `yyyy-mm-dd` via helper `dateUnica.ts`; migration `@manutenzioni.data` ha convertito 56 record; 31 residui patchati.
...
| Lavori da eseguire NEXT | Lavori | `/next/lavori-da-eseguire` | `src/next/NextLavoriDaEseguirePage.tsx` (`src/App.tsx:325`) | `nextLavoriDomain` | `@lavori`, `@mezzi_aziendali` | NON PRESENTE runtime madre | DIMOSTRATO DA ROUTE [OBSOLETO 2026-05-16: modulo Lavori NEXT dismesso 2026-05-12/13; route ora redirect compat a `/next/manutenzioni`; strategia 3a su `@lavori`] |
| Lavori in attesa NEXT | Lavori | `/next/lavori-in-attesa` | `src/next/NextLavoriInAttesaPage.tsx` (`src/App.tsx:379`) | `nextLavoriDomain` | `@lavori`, `@mezzi_aziendali` | NON PRESENTE runtime madre | DIMOSTRATO DA ROUTE [OBSOLETO 2026-05-16: come sopra] |
| Lavori eseguiti NEXT | Lavori | `/next/lavori-eseguiti` | `src/next/NextLavoriEseguitiPage.tsx` (`src/App.tsx:387`) | `nextLavoriDomain` | `@lavori`, `@mezzi_aziendali` | NON PRESENTE runtime madre | DIMOSTRATO DA ROUTE [OBSOLETO 2026-05-16: come sopra] |
| Dettaglio lavoro NEXT | Lavori | `/next/dettagliolavori/:lavoroId` | `src/next/NextDettaglioLavoroPage.tsx` (`src/App.tsx:395`) | `nextLavoriDomain` | `@lavori`, `@mezzi_aziendali`, `@segnalazioni_autisti_tmp`, `@controlli_mezzo_autisti` | redirect legacy query in `NextLegacyStructuralRedirects.tsx:41` | DIMOSTRATO DA ROUTE [OBSOLETO 2026-05-16: route compat verso dettaglio manutenzione equivalente; vedi decisione J.4] |
...
| Lavori | SCRIVENTE [OBSOLETO 2026-05-16: dismesso, vedi DIARIO_DECISIONI 2026-05-12/13; NEXT non scrive più `@lavori`, madre continua via strategia 3a] | `nextLavoriDomain` | `setItemSync("@lavori")` in lista/dettaglio (`NextLavoriDaEseguirePage.tsx:524`, `NextDettaglioLavoroPage.tsx:841`) | `@lavori`, `@mezzi_aziendali` | `@lavori` | Dossier, Manutenzioni, Gestione operativa | alto: stato lavoro condiviso |
