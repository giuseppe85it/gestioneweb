...
import { generateLavoriPDFBlob } from "../utils/pdfEngine";
import {
  buildPdfShareText,
...
export default function NextLavoriInAttesaPage() {
  ...
  const [pdfPreviewFileName, setPdfPreviewFileName] = useState(
    "lavori-in-attesa.pdf",
  );
  const [pdfPreviewTitle, setPdfPreviewTitle] = useState(
    "Anteprima PDF lavori in attesa",
  );
  ...
  const handleExportPDF = async (group: NextLavoriListaGroup) => {
    ...
    const lavori = group.items.map((item) => ({
      ...
      note: item.dettagli ?? undefined,
      tipo: group.kind === "magazzino" ? "magazzino" : "targa",
    }));
    ...
    try {
      ...
      const preview = await openPreview({
        source: async () =>
          generateLavoriPDFBlob(
            `Lavori in Attesa - ${titolo}`,
            lavori,
            titolo,
          ),
      ...
      console.error("Errore anteprima PDF lavori in attesa:", error);
    }
  };
    ...
    <div className="lia-page">
      <div className="lia-card">
        ...
        <div className="lia-footer">
          <button
            type="button"
            className="lia-back-button lavori-btn is-primary"
            onClick={() => navigate(-1)}
          >
            TORNA INDIETRO
          </button>
