...
### Riferimento Zero-Invenzioni
Per asserzioni su dati business mancanti, relazioni non certificate o `relationProof` non producibile, si applica anche la regola permanente in `AGENTS.md`: `Regola operativa "Esplorazione prima di asserzione" (Zero-Invenzioni)`. Prima di dichiarare assenza del dato, l'agente deve eseguire esplorazione Firestore sola lettura entro il boundary autorizzato, oppure dichiarare che la verifica non e' eseguibile per credenziali o boundary insufficienti.
...
- necessita di aggiornare anche la copia corrispondente in `docs/fonti-pronte/` se il task tocca un file sorgente chiave gia specchiato
...
## 11. Regola permanente `docs/fonti-pronte/`
- `docs/fonti-pronte/` e la cartella mirror delle fonti piu importanti del progetto.
- Quando un task aggiorna uno dei documenti chiave gia presenti in `docs/fonti-pronte/`, la copia mirror va aggiornata nello stesso task.
- Se cambia l'elenco delle fonti mirrorate, va aggiornato anche `docs/fonti-pronte/00_INDICE_FONTI_PRONTE.md`.
- Una patch documentale che lascia stale il mirror in `docs/fonti-pronte/` va considerata incompleta.
