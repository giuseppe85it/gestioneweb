...
export default function NextMaterialiDaOrdinarePage() {
  ...
  const aggiungiMateriale = async () => {
    ...
    if (fotoFile) {
      try {
        ...
        const path = `materiali/${id}-${Date.now()}.${ext}`;
        const snap = await uploadBytes(storageRef(storage, path), fotoFile);
        fotoUrl = await getDownloadURL(snap.ref);
        fotoStoragePath = path;
      } catch (err) {
        console.error("Errore upload foto materiale:", err);
        window.alert("Errore caricamento foto. Materiale non aggiunto, riprova.");
        return;
      }
    }
  ...
  const eliminaMateriale = async (id: string) => {
    const item = materiali.find((m) => m.id === id);
    if (item?.fotoStoragePath) {
      try {
        await deleteObject(storageRef(storage, item.fotoStoragePath));
      } catch (err) {
        console.error("Errore rimozione foto materiale:", err);
      }
    }
    setMateriali((current) => current.filter((m) => m.id !== id));
    setNoteByMaterialeId((current) => {
      const next = { ...current };
      delete next[id];
      return next;
  ...
  const aggiornaFotoMateriale = async (materialId: string, file: File) => {
    const currentMateriale = materiali.find((item) => item.id === materialId);
    if (!currentMateriale) return;
    ...
    try {
      ...
      if (currentMateriale.fotoStoragePath) {
        try {
          await deleteObject(storageRef(storage, currentMateriale.fotoStoragePath));
        } catch (deleteError) {
          console.warn("Errore rimozione foto materiale precedente:", deleteError);
        }
      }
      ...
      console.error("Errore upload nuova foto materiale:", err);
  ...
  const salvaOrdine = async () => {
    ...
    try {
      ...
      const nuovoOrdine: Ordine = {
        ...
        dataOrdine: oggi(),
        materiali: materiali.map((m): MaterialeOrdine => ({
          id: m.id,
      ...
      await setDoc(refDoc, { value: updated }, { merge: true });

      clearProcurementDraftState();
      await refreshProcurementSnapshot();
    } catch (err) {
          ...
          {showFabbisogni ? (
                        ...
                        <div className="om-entry-primary">
                          ...
                          <div className="om-entry-main">
                            ...
                            <button
                              ...
                              onClick={() => void aggiungiMateriale()}
                              ...
                              <div className="om-row-actions-menu">
                                <input
                                  ...
                                  onChange={(event) => {
                                    ...
                                    void aggiornaFotoMateriale(materiale.id, file);
                                  }}
                                />
                                <div
                                  ...
                                  openMaterialMenuPosition ? (
                                    ...
                                    >
                                      <button
                                        ...
                                        title={savingOrdine ? "Caricamento foto in corso..." : undefined}
                                        onClick={() => {
                                      ...
                                      <button
                                        ...
                                        onClick={() => {
                                          void eliminaMateriale(materiale.id);
                                          setOpenMaterialMenuId(null);
                                          setOpenMaterialMenuPosition(null);
                                        }}
                                      >
            ...
            <div className="mdo-card-footer-bar om-side om-right om-rightCard">
              ...
              <div className="mdo-sticky-actions om-side-actions om-footerActions">
                ...
                >
                  {savingOrdine ? "Salvataggio..." : "CONFERMA ORDINE"}
                </button>
              </div>
              {totalsSummary.prezziMancanti > 0 || totalsSummary.udmDaVerificare > 0 ? (
            ...
            loadingProcurement ? (
              ...
              <NextProcurementReadOnlyPanel
                ...
                onOrderSaved={refreshProcurementSnapshot}
              />
            ) : (
              <section className="acq-content">
                <div className="acq-tab-panel">
