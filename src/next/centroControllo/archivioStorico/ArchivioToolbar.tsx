...
type Props = {
  ...
  targaOptions: string[];
  // PROMPT 30.5: export PDF lista corrente
    ...
    <section className="archivio-toolbar" aria-label="Filtri archivio">
      ...
      <div className="archivio-toolbar-meta">
        ...
        {onRequestPdfPreview ? (
          <button
            type="button"
            className="archivio-pdf-btn"
            onClick={onRequestPdfPreview}
            disabled={
              generatingPdf ||
              (activeKindResultsCount !== undefined &&
                activeKindResultsCount === 0)
            }
            title="Genera anteprima PDF della lista corrente"
          >
            {generatingPdf ? "Generazione…" : "Anteprima PDF"}
          </button>
        ) : null}
      </div>
    </section>
