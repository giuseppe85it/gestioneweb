...
- **Copertura runtime UI quasi totale verificabile della NEXT (2026-03-23)**: l'observer read-only della nuova IA interna cataloga ora 53 route candidate della NEXT, ne osserva davvero 52 con 70 screenshot e 18 stati interni su 26 tentati; la chat backend riceve una vista runtime compatta ma completa di tutte le route osservate e del mapping `schermata -> file/modulo/flusso`, mentre l'unica route non osservata e gli 8 stati interni residui restano dichiarati con motivazione esplicita.
- **Readiness Firebase tipizzata con whitelist candidate della nuova IA interna (2026-03-22/2026-03-23)**: il backend IA separato espone prerequisiti condivisi e whitelist candidate non attive per un futuro bridge Firestore/Storage read-only; oggi il package dedicato governa le dipendenze runtime dell'adapter e il runtime locale risolve `firebase-admin`, ma restano bloccanti reali credenziali server-side dedicate, `firestore.rules` assente e `storage.rules` in conflitto con l'uso legacy.
