...
export default function AutistiInboxHome({
  ...
  const homePath = cloneConfig?.homePath ?? "/next";
  const adminPath = cloneConfig?.adminPath ?? "/next/autisti-admin";
  const segnalazioniPath = cloneConfig?.segnalazioniPath ?? "/next/autisti-inbox/segnalazioni";
  const controlliPath = cloneConfig?.controlliPath ?? "/next/autisti-inbox/controlli";
  const richiestaAttrezzaturePath =
    cloneConfig?.richiestaAttrezzaturePath ?? "/next/autisti-inbox/richiesta-attrezzature";
  const logAccessiPath = cloneConfig?.logAccessiPath ?? "/next/autisti-inbox/log-accessi";
  const gommePath = cloneConfig?.gommePath ?? "/next/autisti-inbox/gomme";
  const buildCambioMezzoPath =
    cloneConfig?.buildCambioMezzoPath ??
    ((value: Date) => `/next/autisti-inbox/cambio-mezzo?day=${formatDateInputValue(value)}`);
