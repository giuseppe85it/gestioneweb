...
export default function NextSessioniAttiveCard({ sessions }: Props) {
  ...
  const filteredSessions = useMemo(() => {
    ...
    const list = sessions.filter((session) => {
      const badgeValue = normalize(session.badgeAutista);
      const motrice = normalizeTarga(session.targaMotrice);
      const rimorchio = normalizeTarga(session.targaRimorchio);
  ...
  function getConflictPeers(session: ActiveSession) {
    ...
    for (const item of list) {
      if (toKey(item) === selfKey) continue;
      const label = formatAutistaLabel(item);
  ...
  function renderSessionRow(session: ActiveSession) {
    ...
    return (
      ...
      >
        ...
        {hasConflict ? (
          <div className="sessione-conflict-text">In uso anche da: {conflictPeers.join(", ")}</div>
        ) : null}
      </div>
    );
  }
    ...
    <div className="sessioni-card">
      ...
      <div className="sessioni-filters">
        ...
        <input
          ...
          value={targaFilter}
          onChange={(event) => setTargaFilter(event.target.value)}
        />
      </div>
      ...
      {filteredSessions.length === 0 ? (
        ...
        previewSessions.map((session) => renderSessionRow(session))
      )}

      {listOpen ? (
        <div className="aix-backdrop" onClick={() => setListOpen(false)}>
          <div className="aix-modal" onClick={(event) => event.stopPropagation()}>
            <div className="aix-head">
              <h3>Sessioni attive</h3>
              <button className="aix-close" onClick={() => setListOpen(false)} aria-label="Chiudi">
                X
              </button>
            </div>
      ...
      {detailSession ? (
        <div className="aix-backdrop" onClick={() => setDetailSession(null)}>
          <div className="aix-modal" onClick={(event) => event.stopPropagation()}>
            <div className="aix-head">
              <h3>Dettaglio sessione</h3>
              <button className="aix-close" onClick={() => setDetailSession(null)} aria-label="Chiudi">
                X
              </button>
            </div>
            <div className="aix-body">
              <div className="aix-row"><div className="aix-row-top"><strong>Autista</strong><span>{detailSession.nomeAutista || "-"}</span></div></div>
              ...
              {detailConflict.length > 0 ? (
                <div className="aix-row">
                  <div className="aix-row-top">
                    ...
                    <span className="sessione-conflict-text">In uso anche da: {detailConflict.join(", ")}</span>
                  </div>
                </div>
              ) : null}
            </div>
