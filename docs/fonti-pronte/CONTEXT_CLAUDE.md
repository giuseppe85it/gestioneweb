...
| NEXT IA interna universale | Chat controllata, richieste, sessioni, artifacts, audit, registry universale, handoff IA, retriever Euromecc read-only e capability `Magazzino` ora document-driven con pipeline documentale reale nel backend IA separato: allegato -> estrazione `pdf_text` / `pdf_scan` / `image_document` -> header + righe strutturate -> classificazione -> scheda dossier -> review full screen -> scelta utente -> conferma/esecuzione/esito inline per i soli casi `riconcilia_senza_carico` o `carica_stock_adblue`, con `Apri in Magazzino` lasciato come fallback/ispezione e guard-rail scriventi invariati. | in sviluppo |
