...
- FILE TOCCATI: `backend/internal-ai/server/internal-ai-firebase-readonly-boundary.js`, `docs/product/REGISTRO_COLLECTION_FIRESTORE.md`, `docs/DIARIO_DECISIONI.md`, `docs/_live/REGISTRO_MODIFICHE_CLONE.md`
- COSA: AllowedFields riallineati per 22 collection dopo runtime check e sezione "Alias e ricerca flessibile" introdotta nel registro con separazione alias/boundary.
- ESITO: FATTO
- NOTE: Campi sensibili e liberi esclusi by design. Alias concettuali non autorizzano accesso runtime; decide solo il boundary. Verifica eseguita: `node --check` boundary.

### Voce 2026-05-04 2223
- DATA: 2026-05-04
- TITOLO: Boundary readonly Zero-Invenzioni esteso
- FILE TOCCATI: `backend/internal-ai/server/internal-ai-firebase-readonly-boundary.js`, `docs/product/REGISTRO_COLLECTION_FIRESTORE.md`, `docs/DIARIO_DECISIONI.md`, `docs/_live/REGISTRO_MODIFICHE_CLONE.md`
- COSA: Boundary readonly esteso da 5 a 38 collection con pattern read-only + field-filtered. Registro Collection Firestore aggiornato a v0.3 BOZZA con 33 voci BOUNDARY OPEN — RUNTIME PENDING.
- ESITO: FATTO_CON_VERIFICA_MANCANTE
- NOTE: Campi sensibili esclusi by design; runtime check Firestore rinviato a prompt successivo dedicato. Eseguito solo `node --check` del boundary, nessun runtime Firestore.

### Voce 2026-05-04 1739
- DATA: 2026-05-04
- TITOLO: Chat IA NEXT Driver360 Zero-Invenzioni Fase 2
- FILE TOCCATI: `backend/internal-ai/server/internal-ai-adapter.js`, `backend/internal-ai/server/internal-ai-firebase-readonly-boundary.js`, `backend/internal-ai/server/lib/catalog-validator.js`, `backend/internal-ai/server/lib/post-llm-resolver.js`, `src/next/chat-ia/views/Driver360.tsx`, `src/next/chat-ia/relations/driverRelationResolver.ts`, `src/next/chat-ia/components/ChatIaMessageItem.tsx`, `tests/e2e/14-driver360ZeroInvenzioni.spec.ts`
- COSA: Introdotti resolver post-LLM backend, Driver360 certificato e Relation Resolver autista-mezzo con `relationProof`; estesa la whitelist readonly boundary alle collection autisti necessarie.
- ESITO: FATTO
- NOTE: Firebase Admin `ready` via `GOOGLE_APPLICATION_CREDENTIALS`; build OK; E2E 12 fingerprint 6/6 PASS, E2E 11 anti-allucinazione 2/2 PASS, E2E 14 Driver360 4/4 PASS. Tool e agenti non modificati; Ricerca360/Vehicle360/Site360/Euromecc360 non implementate.

### Voce 2026-05-04 1506
- DATA: 2026-05-04
- TITOLO: Chat IA NEXT Zero-Invenzioni Fase 1
- FILE TOCCATI: `backend/internal-ai/server/internal-ai-adapter.js`, `backend/internal-ai/server/lib/catalog-validator.js`, `backend/internal-ai/server/lib/chat-zero-preflight.js`, `src/next/chat-ia/backend/chatIaBackendBridge.ts`, `src/next/chat-ia/core/chatIaTypes.ts`, `src/next/chat-ia/core/catalogValidator.ts`, `src/next/chat-ia/ChatIaToolUsePage.tsx`, `src/next/chat-ia/components/ChatIaMessageItem.tsx`, `src/next/chat-ia/intent-catalog.json`, `tests/e2e/11-antiAllucinazione.spec.ts`, `tests/e2e/12-fingerprintIntegrity.spec.ts`, `tests/e2e/helpers/chatHelpers.ts`
- COSA: Implementata Fase 1 Zero-Invenzioni: schema strict senza testo libero, prompt Action Router, preflight backend dedicato, Catalog Validator backend, fallback solo strutturato, bypass multi-agente old-shape bloccato e rendering senza `message.text` assistente.
- ESITO: FATTO
- NOTE: `npm run build` OK; `npx playwright test tests/e2e/12-fingerprintIntegrity.spec.ts` 6/6 PASS; `npx playwright test tests/e2e/11-antiAllucinazione.spec.ts` 2/2 PASS. Driver360, Ricerca360, viste certificate e Relation Resolver restano fasi successive.

### Voce 2026-04-30 1632
- DATA: 2026-04-30
- TITOLO: UI Scadenze Collaudi NEXT
- FILE TOCCATI: `src/next/NextScadenzeCollaudiPage.tsx`, `src/next/nextScadenzeCollaudiWriter.ts`, `src/next/next-shell.css`
- COSA: Restyling pagina `/next/scadenze-collaudi` con header, counter, card e pannelli inline; targa cliccabile verso dossier e autocomplete officine da `storage/@officine`.
- ESITO: FATTO_CON_VERIFICA_MANCANTE
- NOTE: Build OK, typecheck OK, lint mirato OK; browser smoke OK su render, toggle, link dossier, pannelli e autocomplete. Scritture reali non eseguite per non alterare dati senza mezzo di test dichiarato.

### Voce 2026-04-30 1357
- DATA: 2026-04-30
- TITOLO: Chat IA NEXT fingerprint anti-allucinazione
- FILE TOCCATI: `backend/internal-ai/server/lib/fingerprint-validator.js`, `backend/internal-ai/server/internal-ai-adapter.js`, `src/next/chat-ia/tools/registry/*` tool record-based, `src/next/chat-ia/agents/*`, `src/next/chat-ia/components/blocks/*`, `tests/e2e/11-antiAllucinazione.spec.ts`, `tests/e2e/12-fingerprintIntegrity.spec.ts`, `docs/_live/REPORT_FINGERPRINT_ARCHITECTURE_2026-04-30.md`
- COSA: Aggiunta architettura fingerprint: i risultati tool vengono arricchiti con `_id`, lo schema strict richiede `_id` nei blocchi record, il backend valida ID/date contro i dati tool e rigenera o fallbacka su dati raw verificati.
- ESITO: FATTO_CON_VERIFICA_MANCANTE
- NOTE: Build OK, TypeScript OK, lint mirato OK, `node --check` OK, validator E2E 3/3 PASS; suite browser bloccata prima della chat da `Firebase: Error (auth/too-many-requests)`.

### Voce 2026-04-30 1148
- DATA: 2026-04-30
- TITOLO: Chat IA NEXT coerenza aggregati e dettagli
- FILE TOCCATI: `src/next/chat-ia/tools/chatIaToolFilters.ts`, `src/next/chat-ia/tools/registry/*` tool con liste limitate, `src/next/chat-ia/agents/analytics.ts`, `backend/internal-ai/server/internal-ai-adapter.js`, `tests/e2e/10-coerenzaAggregati.spec.ts`, `docs/_live/REPORT_FIX_COERENZA_AGGREGATI_2026-04-30.md`
- COSA: Aggiunto pattern `total_count/shown/is_truncated/truncation_reason` ai tool con cap e corretto `get_refuelings_aggregated` per usare ranking canonico per numero rifornimenti.
- ESITO: FATTO
- NOTE: TI324633 aprile 2026 verificato: 15 rifornimenti canonici, totale flotta 70; build OK, lint mirato OK, E2E completo 87 test con 1 flaky recuperato al retry.

### Voce 2026-04-30 1035
- DATA: 2026-04-30
- TITOLO: Chat IA NEXT visualization arricchita e cliccabile
- FILE TOCCATI: `src/next/chat-ia/agents/*`, `src/next/chat-ia/core/chatIaTypes.ts`, `src/next/chat-ia/components/ChatIaMessageItem.tsx`, `src/next/chat-ia/components/blocks/*`, `backend/internal-ai/server/internal-ai-adapter.js`, `tests/e2e/05-domandeArgute.spec.ts`, `tests/e2e/08-cliccabilita.spec.ts`, `tests/e2e/09-copia.spec.ts`
- COSA: Arricchiti i dati mostrati da D1-D9, aggiunti metadata/action link ai blocchi visualization, nuovo blocco `nested_list`, confronto con titolo chiaro e bottone copia nei messaggi assistente.
- ESITO: FATTO
- NOTE: E2E mirati 15/15 PASS; suite completa 85 test PASS con 1 flaky storico recuperato al retry; build, lint mirato e node check OK.

### Voce 2026-04-30 0000
- DATA: 2026-04-30
- TITOLO: Chat IA NEXT multi-agente su flotta reale
- FILE TOCCATI: `src/next/chat-ia/agents/*`, `backend/internal-ai/server/internal-ai-adapter.js`, `tests/e2e/05-domandeArgute.spec.ts`, `tests/e2e/07-coerenzaFlotta.spec.ts`, `docs/_live/REPORT_FIX_MULTI_AGENTE_FLOTTA_REALE_2026-04-30.md`
- COSA: Rimossi hardcoded e leakage legati ai dati di test E2E dal multi-agente. Le analisi consumi/cantieri usano ora dataset reali completi letti in modo read-only e i test verificano coerenza flotta e assenza di leakage tecnico.
- ESITO: FATTO
- NOTE: `npm run build` OK; lint mirato OK; `node --check` backend OK; suite E2E completa 79 test PASS con 1 flaky storico recuperato al retry.

### Voce 2026-04-29 2330
- DATA: 2026-04-29
- TITOLO: Chat IA NEXT multi-agente e visualization
- FILE TOCCATI: `src/next/chat-ia/agents/*`, `src/next/chat-ia/components/blocks/*`, `src/next/chat-ia/backend/chatIaBackendBridge.ts`, `src/next/chat-ia/components/ChatIaMessageItem.tsx`, `src/next/chat-ia/core/chatIaTypes.ts`, `src/next/chat-ia/tools/chatIaToolTypes.ts`, `backend/internal-ai/server/internal-ai-adapter.js`, `tests/e2e/05-domandeArgute.spec.ts`, `tests/e2e/06-visualization.spec.ts`
- COSA: Aggiunto percorso multi-agente per 9 domande analitiche D1-D9: orchestratore, 5 specialisti, analytics, visualization e 11 blocchi React strutturati. Il flusso tool-use backend resta fallback per i prompt ordinari.
- ESITO: FATTO
- NOTE: nuovi test 13/13 PASS; suite completa 75 test verde con 1 flaky recuperato e rilanciato singolarmente PASS; build, node --check e lint mirato OK.

### Voce 2026-04-29 2148
- DATA: 2026-04-29
- TITOLO: E2E Chat IA NEXT Playwright e Nodemon
- FILE TOCCATI: `package.json`, `package-lock.json`, `nodemon.json`, `playwright.config.ts`, `tests/e2e/*`, `backend/internal-ai/server/internal-ai-adapter.js`, `src/next/chat-ia/tools/chatIaToolDates.ts`, tool registry mirati
- COSA: Aggiunta infrastruttura Playwright E2E per `/next/chat-tool` e Nodemon backend. La suite copre 62 prompt su verita base, verita calcolata, incroci e edge case; durante il ciclo sono stati fixati date mezzo, timeout structured table, finalizzazione backend post-tool e helper Firestore E2E documenti.
- ESITO: FATTO
- NOTE: `npm run test:e2e` 62/62 PASS, `npm run build` OK, lint mirato OK; `npm run lint` globale resta KO per baseline storico fuori perimetro.

### Voce 2026-04-29 1548
- DATA: 2026-04-29
- TITOLO: Chat IA NEXT structured output e date italiane
- FILE TOCCATI: `backend/internal-ai/server/internal-ai-adapter.js`, `src/next/chat-ia/tools/chatIaToolDates.ts`, `src/next/chat-ia/tools/registry/*` tool con date in output
- COSA: L'endpoint tool-use usa Structured Outputs strict con schema `ChatIaAssistantFinalMessage`; il prompt backend inietta data corrente e regole periodo a ogni chiamata; i tool che espongono date aggiungono date italiane `DD/MM/YYYY` tramite helper centrale.
- ESITO: FATTO
- NOTE: `node --check` OK, lint mirato OK, build OK; `npm run lint` globale resta KO sul baseline storico `581/567/14` fuori perimetro.

### Voce 2026-04-29 1511
- DATA: 2026-04-29
- TITOLO: Rimozione pagine IA NEXT morte
- FILE TOCCATI: `src/App.tsx`, `src/next/NextIALibrettoPage.tsx`, `src/next/NextLibrettiExportPage.tsx`, `src/next/nextStructuralPaths.ts`, `src/next/nextData.ts`, `src/next/nextCloneNavigation.ts`, `src/next/NextCentroControlloPage.tsx`, `src/next/NextIntelligenzaArtificialePage.tsx`, `src/next/NextIAApiKeyPage.tsx`
- COSA: Rimossi hub IA NEXT morto e pagina API Key NEXT, piu bottone API Key nel libretto e retarget del ritorno Libretti Export a `/next`. Madre legacy invariata.
- ESITO: FATTO_CON_VERIFICA_MANCANTE
- NOTE: reachability runtime pulita sui target; lint perimetro KO per due `no-constant-condition` preesistenti in `NextCentroControlloPage.tsx`.

### Voce 2026-04-29 1451
- DATA: 2026-04-29
- TITOLO: Fix tool chat IA Round 2
- FILE TOCCATI: `src/next/chat-ia/tools/registry/toolSearchVehiclesByAttribute.ts`, `src/next/chat-ia/tools/registry/toolSearchDocumentsAndInvoices.ts`, `src/next/chat-ia/tools/registry/toolGetDocumentCostsByVehicle.ts`, `src/next/chat-ia/tools/registry/toolSearchWorkOrders.ts`, `src/next/chat-ia/tools/registry/toolListScheduledMaintenanceDue.ts`, `src/next/chat-ia/tools/registry/toolGetVehicleMaintenanceHistory.ts`, `backend/internal-ai/server/internal-ai-adapter.js`
- COSA: Normalizzati match telaio/targa/tipo nei tool chat IA e aggiunti alias manutenzione/officina per fatture e documenti. Estese description e prompt backend per agganciare richieste su manutenzioni effettuate, lavori fatti e lavori officina.
- ESITO: FATTO
- NOTE: build OK; lint mirato OK; `npm run lint` globale resta KO sul baseline storico fuori perimetro.
...
- File toccati:
  - `storage.rules`
- Obiettivo:
  - sbloccare l'upload su path `preventivi/` per entrambi i rami Archivista Preventivo.
- Causa:
  - la regola `preventivi/{allPaths=**}` non era mai stata presente in `storage.rules` (confermato su 4 commit di storia); qualsiasi upload cadeva nel catch-all `allow read, write: if false` con errore `storage/unauthorized`.
- Fix:
  - aggiunto `match /preventivi/{allPaths=**} { allow read, write: if request.auth != null; }` prima del catch-all.
- Deploy:
  - `firebase deploy --only storage` -> `OK` — progetto `gestionemanutenzione-934ef`.
- Impatto:
  - ramo `Preventivo -> Manutenzione`: ora operativo end-to-end lato Storage.
  - ramo `Preventivo -> Magazzino`: idem, effettivamente scrivibile su Storage per la prima volta.
  - nessun altro path, regola Firestore o file sorgente toccato.
- Verifiche:
  - `npm run build` -> `OK` (20.56s)
  - `npm run lint` -> `582 / 567 / 15` — delta zero rispetto a baseline
- Debito:
  - verifica runtime con preventivo reale da eseguire quando disponibile

## 2.0 2026-04-22 - Distinzione esplicita `Preventivo -> Magazzino` vs `Preventivo -> Manutenzione`
...
- Obiettivo:
  - distinguere in modo esplicito i preventivi archivisti dei due rami senza spostare la destinazione, mantenendo tutti i record in `storage/@preventivi`.
- Modifiche reali:
  - `ArchivistaPreventivoManutenzioneBridge.tsx`:
    - usa ora family `preventivo_manutenzione`
    - passa `ambitoPreventivo: "manutenzione"`
    - limita il duplicate check alla stessa family `preventivo_manutenzione`
  ...
  - `ArchivistaArchiveClient.ts`:
    - accetta la nuova family `preventivo_manutenzione`
    - persiste in modo additivo `ambitoPreventivo`
    - tratta entrambe le family preventivo come file sotto `preventivi/`
    - filtra i duplicate candidate per family effettiva del record
- Continuita e debito noto:
  - nessuna migrazione dei record gia archiviati in `storage/@preventivi`
  - eventuali record storici creati dal ramo manutenzione prima di questa patch con family `preventivo_magazzino` restano invariati e vanno considerati debito da tenere distinto a livello audit/report
- Verifiche:
...
- Note:
  ...
  - nessun cambio UI dei bridge
  - consumer NEXT di `@preventivi` letti nel task non filtrano hard-coded su `preventivo_magazzino`

## 2.0 2026-04-22 - Archivista `Preventivo -> Manutenzione` attivato senza scritture consuntive
- File runtime toccati:
  - `src/next/internal-ai/ArchivistaPreventivoManutenzioneBridge.tsx`
  - `src/next/internal-ai/ArchivistaArchiveClient.ts`
  - `src/next/NextIAArchivistaPage.tsx`
- Obiettivo:
  - attivare davvero in Archivista la combinazione `Preventivo -> Manutenzione`, riusando la UI del ramo manutenzione ma mantenendo la pipeline dati del preventivo e la persistenza esclusiva in `storage/@preventivi`.
- Modifiche reali:
  - creato `ArchivistaPreventivoManutenzioneBridge.tsx` con layout step-based coerente al bridge manutenzione:
    - upload multipagina
    - analisi
    - review campi e righe
    - controllo duplicati
  ...
  - `NextIAArchivistaPage.tsx`:
    - rende `preventivo_manutenzione` `active`
    - aggiorna `FLOW_MATRIX`
    - consente `preventivo -> manutenzione` nel dispatcher
    - monta il nuovo bridge reale
  - `ArchivistaArchiveClient.ts` aggiunge solo l'estensione additiva `metadatiMezzo?: { targa, km }` al payload/record preventivo;
- Verifiche:
...
- Note:
  ...
  - `targa` e obbligatoria in UI
  - `km` resta opzionale
  - la verifica browser del ramo attivo resta `DA VERIFICARE`
...
### Voce 2026-04-15 154007

- DATA: `2026-04-15 15:40`
- TITOLO MODIFICA: `Archivista V1 step 2 - ramo Manutenzione review attivo senza save`
- OBIETTIVO:
  - rendere reale dentro `Archivista documenti` il ramo `Fattura / DDT + Manutenzione`, mantenendo separati IA 1 e IA 2, senza rifare Magazzino e senza aprire archiviazione definitiva o writer business.
- FILE TOCCATI:
  - `src/next/NextIAArchivistaPage.tsx`
  - `src/next/internal-ai/ArchivistaManutenzioneBridge.tsx`
  - `src/next/internal-ai/ArchivistaMagazzinoBridge.tsx`
...
- COSA E STATO CAMBIATO:
  - `NextIAArchivistaPage.tsx` rende ora attivi davvero i due soli rami V1 `Fattura / DDT + Magazzino` e `Fattura / DDT + Manutenzione`;
  - il nuovo `ArchivistaManutenzioneBridge.tsx` richiama il servizio reale `estrazioneDocumenti` e mostra una review manutenzione non chat con stato analisi, riassunto breve, dati principali, righe trovate, avvisi, campi mancanti e callout espliciti di non salvataggio;
  - `ArchivistaMagazzinoBridge.tsx` non viene rifatto, ma distingue meglio il proprio ramo dal nuovo flusso Manutenzione;
  - `internal-ai.css` aggiunge solo il styling necessario per callout, righe e review manutenzione.
- IMPATTO SU UI / LETTURA / BLOCCO SCRITTURE:
  - UI: Archivista passa da un solo ramo attivo a due rami attivi, chiaramente separati e non chat;
  - lettura/analisi: il ramo Manutenzione usa il servizio documentale gia attivo nel progetto e non ricicla la review Magazzino;
  - blocco scritture: invariato in questa patch; nessun writer business nuovo, nessun save automatico, nessun uso di `@costiMezzo` come destinazione primaria.
- COME VERIFICARE:
  - aprire `/next/ia/archivista`;
  - selezionare `Fattura / DDT` + `Manutenzione`;
  - caricare un PDF o una foto e verificare che `Analizza documento` mostri review con dati principali, righe, avvisi e callout `Documento analizzato`, `Non ancora archiviato`, `Nessuna manutenzione ancora creata`;
  - selezionare `Fattura / DDT` + `Magazzino` e verificare che il ramo preesistente resti attivo senza regressioni;
  - verificare che `Documento mezzo` e `Preventivo + Magazzino` restino visibili ma non attivi;
  - eseguire `npx eslint src/next/NextIAArchivistaPage.tsx src/next/internal-ai/ArchivistaMagazzinoBridge.tsx src/next/internal-ai/ArchivistaManutenzioneBridge.tsx` e `npm run build`.
- SE E CANDIDABILE A ESSERE PORTATO NELLA MADRE IN FUTURO: `NO`
- NOTE:
  - nessun backend, function, api o writer business toccato;
  - `npx eslint src/next/internal-ai/internal-ai.css` restituisce il warning noto `File ignored because no matching configuration was supplied`.

### Voce 2026-04-15 151953

- DATA: `2026-04-15 15:19`
- TITOLO MODIFICA: `Archivista V1 step 1 - ramo Magazzino attivo`
- OBIETTIVO:
  - rendere reale dentro `Archivista documenti` il solo caso forte `Fattura / DDT + Magazzino`, senza rifare il cervello Magazzino, senza nuovi writer business e senza trasformare IA 2 in una chat.
- FILE TOCCATI:
  - `src/next/NextIAArchivistaPage.tsx`
  - `src/next/internal-ai/ArchivistaMagazzinoBridge.tsx`
  - `src/next/internal-ai/internal-ai.css`
  ...
  - `CONTEXT_CLAUDE.md`
- COSA E STATO CAMBIATO:
  - `NextIAArchivistaPage.tsx` rende ora attivo davvero solo `Fattura / DDT + Magazzino`;
  - i rami `Fattura / DDT + Manutenzione`, `Documento mezzo` e `Preventivo + Magazzino` restano visibili ma marcati `In arrivo`;
  - `Preventivo + Manutenzione` resta fuori V1;
  - il nuovo bridge `ArchivistaMagazzinoBridge.tsx` richiama il servizio reale `estrazioneDocumenti` e mostra una review pulita con stato analisi, dati principali, righe trovate e avvisi;
  - `cloneWriteBarrier.ts` apre solo l'eccezione minima gia esistente anche per il pathname `/next/ia/archivista`, limitata al `POST` verso `estrazioneDocumenti`.
- IMPATTO SU UI / LETTURA / BLOCCO SCRITTURE:
  - UI: Archivista non e piu solo shell, ma primo ramo attivo reale e non chat;
  - lettura/analisi: il ramo Magazzino usa il servizio documentale gia attivo nel progetto;
  - scritture: nessun writer business nuovo, nessun save automatico, nessun handoff nuovo.
- COME VERIFICARE:
  - aprire `/next/ia/archivista`;
  - selezionare `Fattura / DDT` + `Magazzino`;
  - caricare un PDF o una foto e verificare che `Analizza documento` mostri review con dati principali, righe e avvisi;
  - selezionare `Fattura / DDT + Manutenzione`, `Documento mezzo` o `Preventivo + Magazzino` e verificare che risultino visibili ma non ancora attivi;
  - eseguire `npx eslint src/next/NextIAArchivistaPage.tsx src/next/internal-ai/ArchivistaMagazzinoBridge.tsx src/utils/cloneWriteBarrier.ts` e `npm run build`.
- SE E CANDIDABILE A ESSERE PORTATO NELLA MADRE IN FUTURO: `NO`
- NOTE:
  - nessun backend, function, api o writer business toccato;
  - `npx eslint src/next/internal-ai/internal-ai.css` restituisce il warning noto `File ignored because no matching configuration was supplied`.

### Voce 2026-04-15 144354
- DATA: 2026-04-15
- TITOLO MODIFICA: separazione runtime IA 1 `Report` / IA 2 `Archivista documenti`
- OBIETTIVO: dare al sottosistema IA della NEXT una forma prodotto corretta e distinta, separando davvero la parte report/chat dalla nuova entrata guidata Archivista, senza aprire nuove scritture business
- FILE TOCCATI:
  - `src/App.tsx`
...
- COSA E STATO CAMBIATO:
  - aggiunta la nuova route `/next/ia/archivista` e l'alias leggibile `/next/ia/report`
  - creata `NextIAArchivistaPage.tsx` come nuova pagina non chat con scelta `Tipo`, scelta `Contesto`, area upload e pulsante `Analizza documento`
  - riscritto `HomeInternalAiLauncher` per mostrare due strumenti distinti, con documenti mezzo riallineati all'Archivista e `Cisterna Caravate` lasciata fuori V1
  - riposizionata `NextInternalAiPage` come area `IA Report`, con callout esplicito verso l'Archivista e vecchio blocco documentale declassato a supporto allegati temporaneo
  - riallineata la card IA della Home e aggiunte le classi CSS necessarie alla nuova separazione visiva
- IMPATTO SU UI / LETTURA / BLOCCO SCRITTURE:
  - UI: l'utente vede finalmente due strumenti diversi e non un solo ingresso ibrido
  - Lettura: la parte report/chat resta disponibile senza regressioni e senza perdere i flussi tecnici gia presenti
  - Blocco scritture: invariato; nessun writer nuovo, nessuna modifica backend, nessuna apertura nuova su `cloneWriteBarrier.ts`
- COME VERIFICARE:
  - eseguire `npx eslint src/App.tsx src/next/components/HomeInternalAiLauncher.tsx src/next/NextInternalAiPage.tsx src/next/NextHomePage.tsx src/next/NextIAArchivistaPage.tsx`
  - eseguire `npm run build`
  - aprire `/next` e verificare due ingressi distinti nel launcher
  - aprire `/next/ia/report` e verificare che la pagina sia percepita come area report/chat
  - aprire `/next/ia/archivista` e verificare pagina non chat con scelta tipo, contesto, upload e flussi fuori V1 non disponibili
- SE E CANDIDABILE A ESSERE PORTATO NELLA MADRE IN FUTURO: NO
- NOTE:
  - il task cambia la forma prodotto, non la profondita motore
  - analisi guidata profonda, review finale e scritture business dell'Archivista restano fuori da questa patch

### Voce 2026-04-14 174940
- DATA: 2026-04-14
- TITOLO MODIFICA: IA interna card alta `Documento + Analizza` abilitata al multi-file
- OBIETTIVO: estendere alla card alta reale di `/next/ia/interna` la stessa capability multi-file gia approvata nel flusso chat/allegati, lasciando invariato il caso singolo
- FILE TOCCATI:
  - `src/next/NextInternalAiPage.tsx`
  - `docs/STATO_ATTUALE_PROGETTO.md`
  - `docs/product/STATO_MIGRAZIONE_NEXT.md`
...
- COSA E STATO CAMBIATO:
  - il file input della card alta `Documento` accetta ora selezione multipla
  - con `2 o piu file` la card mostra il toggle `Tratta questi file come un unico documento`, attivo di default e in italiano
  - il click su `Analizza` dalla card alta riusa il percorso allegati/orchestrazione gia esistente per produrre un solo riepilogo finale aggregato
  - con `1 file` il ramo continua a usare invariato `useIADocumentiEngine()` e non cambia comportamento
- IMPATTO SU UI / LETTURA / BLOCCO SCRITTURE:
  - UI: la card alta reale di `/next/ia/interna` supporta ora il multi-file senza introdurre una seconda UX separata
  - Lettura: l'estrazione del singolo file resta invariata; il multi-file riusa il percorso allegati gia esistente con prompt neutro
  - Blocco scritture: invariato; nessun writer nuovo, nessun cambio backend, nessuna modifica alla madre
- COME VERIFICARE:
  - eseguire `npx eslint src/next/NextInternalAiPage.tsx src/next/internal-ai/internalAiDocumentAnalysis.ts src/next/internal-ai/internalAiUniversalDocumentRouter.ts src/next/internal-ai/internalAiUniversalHandoff.ts src/next/internal-ai/internalAiUniversalOrchestrator.ts`
  - eseguire `npm run build`
  - aprire `/next/ia/interna`
  - selezionare `1 file` nella card alta e verificare comportamento identico a prima
  - selezionare `2 o piu file` nella card alta e verificare presenza del toggle, default attivo e riepilogo finale unico
- SE E CANDIDABILE A ESSERE PORTATO NELLA MADRE IN FUTURO: DA VALUTARE
- NOTE:
  - il flusso chat/allegati gia patchato non e stato riscritto
  - per mantenere un solo percorso coerente, la card alta multi-file sostituisce il contesto allegati corrente della pagina
...
- OBIETTIVO: impedire che le scelte di sola riconciliazione aggiungano quantita a materiali gia caricati a stock e rendere davvero leggibile/utile la colonna destra della review documento full screen
- FILE TOCCATI:
  - `src/next/NextMagazzinoPage.tsx`
  ...
  - mirror corrispondenti in `docs/fonti-pronte/*`
- COSA E STATO CAMBIATO:
  - il gating documentale `Magazzino` consente la sola `riconciliazione senza carico` solo quando l'arrivo procurement compatibile risulta gia consolidato a stock tramite `stockLoadKeys`;
  - se esiste match procurement + inventario ma la sorgente procurement non e ancora stata consolidata, la sola riconciliazione viene bloccata per evitare che un caso di carico reale venga trattato come semplice collegamento documento/costo;
  - la review full screen della IA interna viene riordinata in 6 sezioni operative e sposta i dettagli tecnici in un box collassabile, lasciando `Righe estratte` come sezione dominante della colonna destra.
- IMPATTO SU UI / LETTURA / BLOCCO SCRITTURE:
  - UI: la review destra e piu leggibile, meno tecnica e piu orientata alla decisione operativa;
  - Lettura: i dati documento e le righe estratte sono immediatamente leggibili, mentre i dettagli tecnici restano disponibili ma secondari;
  - Blocco scritture: nessun writer nuovo, nessuna modifica alla barrier; il fix restringe l'applicazione del ramo `riconcilia_senza_carico` e riduce il rischio di doppio incremento quantita.
- COME VERIFICARE:
  - eseguire `npx eslint src/next/NextMagazzinoPage.tsx src/next/internal-ai/internalAiMagazzinoControlledActions.ts src/next/NextInternalAiPage.tsx src/next/internal-ai/internal-ai.css`
  - eseguire `npm run build`
  - aprire `/next/ia/interna` e caricare:
    - `fattura_mariba_534909.pdf`
    - `fattura_adblue_aprile.pdf`
    - `documento_ambiguo.pdf`
  - verificare nella review full screen la colonna destra con ordine `Documento` -> `Righe estratte` -> `Match inventario` -> `Decisione` -> `Azione proposta IA` -> `Dettagli tecnici`
  - aprire `/next/magazzino?tab=documenti-costi` e verificare che sul dataset live restino `Pronte: 0`, `Bloccate: 3`, quindi la prova end-to-end su candidato live pronto resti ancora `DA VERIFICARE`
- SE E CANDIDABILE A ESSERE PORTATO NELLA MADRE IN FUTURO: DA VALUTARE
- NOTE: la patch runtime e chiusa nel clone/NEXT, ma non auto-certifica il flusso oltre `PARZIALE` perche manca ancora un candidato live `Pronto` che permetta la prova browser end-to-end del ramo `riconciliazione senza carico`

### Voce 2026-04-11 IA INTERNA MAGAZZINO DOCUMENT EXTRACTION PIPELINE
- DATA: 2026-04-11
- TITOLO MODIFICA: pipeline documentale reale per PDF e immagini nella review IA `Magazzino`
- OBIETTIVO: far usare alla review full screen dati estratti davvero dal documento, con header strutturato e righe materiali ricostruite, senza cambiare il perimetro business del Magazzino
- FILE TOCCATI:
  - `backend/internal-ai/server/internal-ai-document-extraction.js`
  - `backend/internal-ai/server/internal-ai-adapter.js`
  ...
  - mirror corrispondenti in `docs/fonti-pronte/*`
- COSA E STATO CAMBIATO:
  - introdotta nel backend IA separato una pipeline che distingue `pdf_text`, `pdf_scan` e `image_document`;
  - ogni allegato puo ora esporre `documentAnalysis` con header documento, warning e righe materiali strutturate;
  - `router` e `handoff` usano i nuovi dati per instradare correttamente materiali, `AdBlue`, preventivi e casi ambigui;
  - la review full screen mostra ora davvero righe con codice articolo, quantita, unita, prezzo e totale.
- IMPATTO SU UI / LETTURA / BLOCCO SCRITTURE:
  - UI: la review e molto piu utile e gestionale perche i dati non dipendono piu solo da classificazione o excerpt;
  - Lettura: il backend IA separato estrae header e righe dal documento e li propaga fino alla UI NEXT;
  - Blocco scritture: invariato; nessun writer nuovo, nessun allargamento barrier, nessuna modifica alle regole business stock.
- COME VERIFICARE:
  - eseguire `npx eslint src/next/internal-ai/internalAiTypes.ts src/next/internal-ai/internalAiDocumentAnalysis.ts src/next/internal-ai/internalAiChatAttachmentsClient.ts src/next/internal-ai/internalAiUniversalEntityResolver.ts src/next/internal-ai/internalAiUniversalDocumentRouter.ts src/next/internal-ai/internalAiUniversalTypes.ts src/next/internal-ai/internalAiUniversalOrchestrator.ts src/next/internal-ai/internalAiUniversalHandoff.ts src/next/NextInternalAiPage.tsx backend/internal-ai/server/internal-ai-document-extraction.js backend/internal-ai/server/internal-ai-chat-attachments.js backend/internal-ai/server/internal-ai-adapter.js`
  - eseguire `npm run build`
  - aprire `/next/ia/interna` e verificare la review full screen con:
    - `tmp-runtime-materiali.png`
    - `tmp-runtime-adblue.pdf`
    - `tmp-runtime-preventivo.pdf`
    - `tmp-runtime-ambiguo.pdf`
  - verificare header documento ricco, righe materiali vere, routing corretto e assenza di label generici spurii
- SE E CANDIDABILE A ESSERE PORTATO NELLA MADRE IN FUTURO: DA VALUTARE
- NOTE: pipeline chiusa lato clone/backend IA separato; restano `DA VERIFICARE` OCR debole, multi-page irregolari e warning font `pdfjs-dist` in ambiente Windows

### Voce 2026-04-11 IA INTERNA MAGAZZINO FULL SCREEN DOCUMENT REVIEW
- DATA: 2026-04-11
- TITOLO MODIFICA: sostituzione della UX documentale IA `Magazzino` con review operativa full screen
- OBIETTIVO: portare la decisione utente su fatture e preventivi dentro una schermata full screen chiara e gestionale, con documento protagonista a sinistra e review operativa a destra
- FILE TOCCATI:
  - `src/next/NextInternalAiPage.tsx`
  - `src/next/internal-ai/internal-ai.css`
  - `docs/STATO_ATTUALE_PROGETTO.md`
  ...
  - mirror corrispondenti in `docs/fonti-pronte/*`
- COSA E STATO CAMBIATO:
  - `NextInternalAiPage.tsx` apre ora automaticamente una review documento full screen dopo l'analisi degli allegati `Magazzino` / `preventivo_fornitore`;
  - il layout usa 2 aree forti: preview grande del documento a sinistra e review gestionale a destra;
  - la decisione utente non passa piu dalla sola card sopra la chat: il modale espone action cards chiare (`Collega`, `Aggiungi costo/documento`, `Crea nuovo articolo`, `Carica stock`, `DA VERIFICARE`) e abilita inline solo cio che e gia consentito nel perimetro;
  - la card dossier sopra la chat resta come riepilogo e punto di riapertura review, mentre `Apri in Magazzino` o `Apri Procurement / ordini / fornitori` restano fallback.
- IMPATTO SU UI / LETTURA / BLOCCO SCRITTURE:
  - UI: il documento diventa il protagonista visivo e la review sembra una schermata gestionale vera, non un messaggio chat evoluto;
  - Lettura: le righe materiali mostrano ora azione proposta, match, inventario collegabile e dati estratti piu ordinati;
  - Blocco scritture: invariato; nessun writer nuovo, nessuna esecuzione automatica, inline ancora limitato ai soli `riconcilia_senza_carico` e `carica_stock_adblue`.
- COME VERIFICARE:
  - eseguire `npx eslint src/next/NextInternalAiPage.tsx`
  - eseguire `npm run build`
  - aprire `/next/ia/interna` e verificare il modale full screen con:
    - fattura materiali (`fattura mariba.jpeg`)
    - fattura AdBlue (`fattura_adblue_aprile.pdf`)
    - preventivo (`preventivo_materiale_test.pdf`)
    - caso ambiguo (`documento_ambiguo_test.pdf`)
  - verificare preview grande a sinistra, review gestionale a destra, decision cards, assenza di auto-esecuzione e fallback corretto verso Magazzino o Procurement
- SE E CANDIDABILE A ESSERE PORTATO NELLA MADRE IN FUTURO: DA VALUTARE
- NOTE: task concentrato sulla UX operativa del modale IA; nessun file runtime fuori whitelist toccato
### Voce 2026-04-11 IA INTERNA MAGAZZINO INLINE CONFIRM + EXECUTION
- DATA: 2026-04-11
- TITOLO MODIFICA: conferma, esecuzione ed esito inline nel modale/chat IA interna `Magazzino`
- OBIETTIVO: completare il flusso standard documento `Magazzino` direttamente nel modale IA per i soli casi `riconcilia_senza_carico` e `carica_stock_adblue`, lasciando `Apri in Magazzino` come fallback
- FILE TOCCATI:
  - `src/next/NextInternalAiPage.tsx`
  - `src/next/internal-ai/internalAiMagazzinoControlledActions.ts`
  - `src/next/internal-ai/internal-ai.css`
  ...
  - mirror corrispondenti in `docs/fonti-pronte/*`
- COSA E STATO CAMBIATO:
  - la scheda dossier documento del modale IA risolve in background i route `Magazzino`, mostra conferma inline solo quando il match e abbastanza forte ed esegue i due casi consentiti senza passaggio obbligatorio nel modulo;
  - e stato introdotto `internalAiMagazzinoControlledActions.ts` per riusare in modo controllato il contesto e le decisioni `riconcilia_senza_carico` / `carica_stock_adblue` fuori da `NextMagazzinoPage.tsx`;
  - `cloneWriteBarrier.ts` aggiunge una scoped allowance temporanea e stretta solo su `@inventario`, attiva esclusivamente mentre l'azione inline IA e in esecuzione;
  - l'esito finale inline mostra dati leggibili sul risultato, mentre `Apri in Magazzino` resta sempre disponibile come fallback.
- IMPATTO SU UI / LETTURA / BLOCCO SCRITTURE:
  - UI: l'utente puo chiudere inline nel modale IA i due flussi consentiti senza perdere la scheda documento;
  - Lettura: il caso ambiguo resta `DA VERIFICARE` e non sblocca azioni;
  - Blocco scritture: nessun writer nuovo oltre ai due casi gia approvati, nessuna apertura su consegne, manutenzioni, ordini, preventivi o listino.
- COME VERIFICARE:
  - eseguire `npx eslint src/utils/cloneWriteBarrier.ts src/next/internal-ai/internalAiMagazzinoControlledActions.ts src/next/NextInternalAiPage.tsx`
  - eseguire `npm run build`
  - aprire `/next/ia/interna` e allegare `fattura_mariba_534909.pdf`, `fattura_adblue_aprile.pdf`, `documento_ambiguo.pdf`
  - verificare che la scheda mostri conferma inline solo sui match forti, `DA VERIFICARE` sui casi ambigui e fallback `Apri in Magazzino`
  - verificare che sul dataset corrente il support snapshot mostri `Pronte: 0`, quindi la prova end-to-end su riga reale pronta resti da rivalidare
- SE E CANDIDABILE A ESSERE PORTATO NELLA MADRE IN FUTURO: DA VALUTARE
- NOTE: patch strutturale chiusa nel clone; audit reale ancora aperto sui casi pronti `MARIBA` / `AdBlue`
...
- OBIETTIVO: Permettere alla chat `/next/ia/interna` di classificare automaticamente i documenti `Magazzino`, proporre l'azione giusta in UI e aprire il modulo target corretto anche con solo allegato o con testo minimale, senza aprire nuovi writer fuori dai due casi gia approvati sulle fatture.
- FILE TOCCATI:
  - `src/next/NextInternalAiPage.tsx`
  - `src/next/internal-ai/internalAiUniversalDocumentRouter.ts`
  - `src/next/internal-ai/internalAiUniversalHandoff.ts`
...
- COSA E STATO CAMBIATO:
  - `NextInternalAiPage.tsx` non costringe piu l'utente a scrivere un prompt tecnico: con un allegato genera un prompt base prudente, consente submit attachment-only, lancia in background l'orchestrazione documentale e mostra una card automatica con tipo rilevato, azione proposta, motivazione, confidenza, eventuale singola domanda di sblocco e CTA verso il modulo target;
  - `internalAiUniversalDocumentRouter.ts` riconosce meglio fatture materiali `Magazzino`, fatture `AdBlue` e documenti ambigui usando i segnali documentali disponibili nel clone, compresi nomi file realistici con `_` e `-`;
  - `internalAiUniversalHandoff.ts` instrada i casi forti al modulo canonico `Magazzino`, applica payload prudente sui casi ambigui e riduce i riferimenti sporchi su `targa/materiale` nel passaggio documentale;
  - il percorso utente finale diventa: allegato -> classificazione automatica -> proposta azione -> CTA semplice -> conferma nel modulo target -> esecuzione solo se rientra nei due casi gia autorizzati (`riconcilia_senza_carico`, `carica_stock_adblue`);
  - nessun writer business nuovo viene aperto: il task rifinisce la UX e l'orchestrazione documentale sopra i guard-rail gia attivi.
- IMPATTO SU UI / LETTURA / BLOCCO SCRITTURE:
  - UI: `/next/ia/interna` mostra ora una proposal card documentale automatica e un composer utilizzabile anche senza prompt rigidi;
  - Lettura: il routing documentale usa segnali disponibili nell'allegato e nel contesto `Magazzino`, ma resta prudente e chiede conferma nel modulo target;
  - Blocco scritture: invariato nel perimetro; restano ammessi solo `riconcilia_senza_carico` e `carica_stock_adblue`, mentre i casi ambigui finiscono in `DA VERIFICARE`.
- COME VERIFICARE:
  - eseguire `npx eslint src/next/NextInternalAiPage.tsx src/next/internal-ai/internalAiUniversalDocumentRouter.ts src/next/internal-ai/internalAiUniversalHandoff.ts`;
  - eseguire `npm run build`;
  - avviare una preview locale e aprire `/next/ia/interna`;
  - allegare un file con nome tipo `fattura_mariba_534909.pdf` e verificare la proposta `Riconcilia documento` verso `Magazzino`;
  - allegare un file con nome tipo `fattura_adblue_aprile.pdf` e verificare la proposta `Carica stock AdBlue`;
  - allegare un file ambiguo e verificare la proposta `DA VERIFICARE` con una sola domanda di sblocco;
  - non eseguire submit mutanti sui dataset reali durante la verifica runtime.
- SE E CANDIDABILE A ESSERE PORTATO NELLA MADRE IN FUTURO: DA VALUTARE
- NOTE: task UX/orchestrazione sopra una deroga scrivente gia esistente; resta necessario un audit separato su PDF/immagini reali con segnali documentali deboli.

### Voce 2026-04-10 IA INTERNA MAGAZZINO FATTURE WRITE EXCEPTION
- DATA: 2026-04-10
- TITOLO MODIFICA: Deroga scrivente controllata della IA interna NEXT per le sole fatture `Magazzino`
- OBIETTIVO: Abilitare nel clone NEXT una sola eccezione scrivente lato IA sul dominio `Magazzino`, confinata alle fatture di magazzino: riconciliazione senza carico per materiale gia arrivato e gia consolidato, oppure carico stock AdBlue non ancora registrato, con anti-doppio-carico obbligatorio.
- FILE TOCCATI:
  - `src/next/NextMagazzinoPage.tsx`
  - `src/next/domain/nextDocumentiCostiDomain.ts`
...
- COSA E STATO CAMBIATO:
  - `nextDocumentiCostiDomain.ts` espone ora sui support docs di `@documenti_magazzino` anche metadata forti (`tipoDocumento`, `numeroDocumento`, `nomeFile`, `fileUrl`, `daVerificare`) per distinguere una fattura da un allegato generico e bloccare i casi non dimostrati;
  - `NextMagazzinoPage.tsx` non tratta piu `@documenti_magazzino.voci` come writer generico: ogni riga documento viene classificata come `riconcilia_senza_carico`, `carica_stock_adblue`, `DA VERIFICARE` o `fuori_perimetro`, con blocco su mismatch UDM, documento `daVerificare`, non-fattura e sorgente gia consolidata;
  - il caso `MARIBA` usa ora solo `Riconcilia senza carico`: se esiste arrivo procurement compatibile e il materiale e gia in inventario, la pagina collega il documento alla voce stock via `stockLoadKeys` senza aumentare la quantita;
  - il caso `AdBlue` usa ora solo `Carica stock AdBlue`: se la fattura e AdBlue, l'unita e `lt`, il match e forte e non esiste doppio carico, la pagina crea o aggiorna l'articolo di inventario e aumenta la giacenza;
  - `internalAiUniversalDocumentRouter.ts`, `internalAiUniversalRequestResolver.ts` e `internalAiUniversalHandoff.ts` instradano ora le fatture materiali verso `next.magazzino` / `documenti-costi`, con priorita documentale anche sui prompt `fattura AdBlue`;
  - `internalAiUniversalContracts.ts` e `internalAiUnifiedIntelligenceEngine.ts` dichiarano la deroga come eccezione stretta del dominio D05, non come riapertura generale dei writer Magazzino.
- IMPATTO SU UI / LETTURA / BLOCCO SCRITTURE:
  - UI: la vista `Documenti e costi` mostra il nuovo pannello `Azione controllata IA su fattura magazzino` e CTA esplicite solo per `Riconcilia senza carico` o `Carica stock AdBlue`;
  - Lettura: la decisione usa metadata documento, matching materiale/fornitore, copertura procurement, UDM canoniche e `stockLoadKeys`, senza riaprire runtime legacy o writer fuori whitelist;
  - Blocco scritture: confermato su consegne, manutenzioni, ordini, preventivi e documenti fuori dai due casi; se il match non e forte oppure la sorgente e gia consolidata, l'esito resta `DA VERIFICARE` o blocco esplicito.
- COME VERIFICARE:
  - eseguire `npx eslint src/next/NextMagazzinoPage.tsx src/next/domain/nextDocumentiCostiDomain.ts src/next/internal-ai/internalAiUniversalRequestResolver.ts src/next/internal-ai/internalAiUniversalDocumentRouter.ts src/next/internal-ai/internalAiUniversalHandoff.ts src/next/internal-ai/internalAiUniversalContracts.ts src/next/internal-ai/internalAiUnifiedIntelligenceEngine.ts`;
  - eseguire `npm run build`;
  - avviare una preview locale e aprire `/next/magazzino?tab=documenti-costi`;
  - verificare che la card `Azione controllata IA su fattura magazzino` sia renderizzata e che il pannello `Carichi stock da documenti` non proponga piu carichi generici da documento;
  - verificare sul codice che il caso `MARIBA` usi `stockLoadKeys` senza aumento stock e che il caso `AdBlue` richieda `lt`, match forte e assenza di doppio carico;
  - non eseguire submit mutanti sui dataset reali durante la verifica runtime.
- SE E CANDIDABILE A ESSERE PORTATO NELLA MADRE IN FUTURO: DA VALUTARE
- NOTE: deroga chirurgica, non refactor generale. Serve audit separato per promuovere a fatto verificato l'assenza di aperture laterali fuori dai due casi documentali.

### Voce 2026-04-10 IA INTERNA MAGAZZINO READ-ONLY
- DATA: 2026-04-10
- TITOLO MODIFICA: Integrazione del dominio `Magazzino` nella IA interna NEXT in sola lettura
- OBIETTIVO: Permettere alla console `/next/ia/interna` di leggere, incrociare e spiegare il dominio `Magazzino` usando i reader NEXT reali (`@inventario`, `@materialiconsegnati`, `@cisterne_adblue`, `@documenti_magazzino`, costi materiali derivati e procurement di supporto), senza toccare la madre e senza esporre writer business.
- FILE TOCCATI:
  - `src/next/NextMagazzinoPage.tsx`
...
- COSA E STATO CAMBIATO:
  - `nextMaterialiMovimentiDomain.ts` espone ora anche gli eventi `@cisterne_adblue` nel composito `readNextMagazzinoRealeSnapshot()`, cosi D05 puo leggere stock, movimenti e AdBlue sullo stesso asse read-only;
  - `internalAiUnifiedIntelligenceEngine.ts` costruisce un cross-read Magazzino che incrocia inventario, movimenti materiali, AdBlue, documenti/fatture, costi di supporto e procurement, restituendo blocchi strutturati `Stock`, `Movimenti`, `Documenti / Fatture`, `Preventivi`, `Costi di supporto`, `Criticita / DA VERIFICARE`;
  - `internalAiUniversalContracts.ts`, `internalAiUniversalRequestResolver.ts` e `internalAiUniversalHandoff.ts` agganciano D05 al modulo canonico `next.magazzino` e alle sue viste dedicate, invece di lasciare il dominio Magazzino appeso ai vecchi path inventario/materiali o a `next.operativita`;
  - `NextMagazzinoPage.tsx` consuma `iaHandoff` e applica prefill su tab/materiale/targa/documento, restando nel modulo canonico `/next/magazzino`;
  - nessun file backend e stato toccato: il bridge server-side IA resta invariato e il dominio Magazzino viene servito dal motore unificato locale gia presente.
- IMPATTO SU UI / LETTURA / BLOCCO SCRITTURE:
  - UI: minima; `/next/ia/interna` puo ora rispondere in modo strutturato su Magazzino e `/next/magazzino` riceve prefill/handoff coerenti;
  - Lettura: l'IA legge dataset reali e layer NEXT gia esistenti senza inventare match forti tra documenti, materiali e procurement quando il legame non e dimostrabile;
  - Blocco scritture: invariato; nessun writer stock, procurement o documentale viene esposto alla IA.
- COME VERIFICARE:
  - eseguire `npx eslint src/next/NextMagazzinoPage.tsx src/next/domain/nextMaterialiMovimentiDomain.ts src/next/internal-ai/internalAiUniversalContracts.ts src/next/internal-ai/internalAiUniversalRequestResolver.ts src/next/internal-ai/internalAiUniversalHandoff.ts src/next/internal-ai/internalAiUnifiedIntelligenceEngine.ts`;
  - eseguire `npm run build`;
  - avviare una preview locale e aprire `/next/ia/interna`;
  - inviare la richiesta `quanta giacenza ho del materiale AdBlue e quali documenti o preventivi risultano collegati` e verificare che la risposta usi i blocchi `Stock`, `Movimenti`, `Documenti / Fatture`, `Preventivi`, `Costi di supporto`, `Criticita / DA VERIFICARE`;
  - verificare che i riferimenti del `Piano universale clone/NEXT` includano D05, i dataset reali del Magazzino e che il modulo target canonico sia `Magazzino` quando il prompt e chiaramente D05;
  - non eseguire submit o mutazioni business nei form runtime collegati ai dataset reali.
- SE E CANDIDABILE A ESSERE PORTATO NELLA MADRE IN FUTURO: DA VALUTARE
- NOTE: task execution del sottosistema IA interna, non audit finale. La capability `Magazzino` resta `PARZIALE` finche non viene rivalidato il planner/handoff universale sui prompt misti materiale/documenti/preventivi.

### Voce 2026-04-10 AUTONOMIA NEXT STOCK MAGAZZINO
- DATA: 2026-04-10
- TITOLO MODIFICA: Autonomia NEXT del dominio stock `Magazzino`
- OBIETTIVO: Rendere `/next/magazzino` il punto operativo canonico del dominio stock nel perimetro NEXT, assorbendo il carico stock degli arrivi procurement dentro `Magazzino` e chiarendo che procurement/documenti restano supporto o preview, non writer canonici del magazzino.
- FILE TOCCATI:
  - `src/next/NextMagazzinoPage.tsx`
...
- COSA E STATO CAMBIATO:
  - `NextMagazzinoPage.tsx` assorbe ora anche il carico stock degli arrivi procurement nella vista `Documenti e costi`, con candidati dedicati, consolidamento su inventario esistente o creazione nuova voce, `stockLoadKeys` e deduplica prudente contro documenti gia caricati;
  - il pulsante di supporto nella vista documentale punta al procurement canonico `/next/materiali-da-ordinare` e la sezione `Ordini e arrivi collegati` spiega che il writer stock degli arrivi e `Magazzino`;
  - `nextProcurementDomain.ts` dichiara in modo esplicito che ordini/arrivi restano supporto read-only e che il consolidamento stock degli arrivi passa da `/next/magazzino?tab=documenti-costi`;
  - `NextProcurementReadOnlyPanel.tsx` riallinea il copy delle viste `Ordini` / `Arrivi` al fatto che il procurement NEXT e supporto e che il dominio stock canonico resta `Magazzino`;
  - `nextData.ts` promuove anche nella shell il dominio stock come punto centrale: `Magazzino` prima di `Materiali da ordinare`, `Operativita Globale` descritta come `Operativo parziale` e non piu come `Importato read-only`.
- IMPATTO SU UI / LETTURA / BLOCCO SCRITTURE:
  - UI: il runtime non rifatto, ma la vista `Documenti e costi` espone ora anche `Carichi stock da arrivi procurement` e il procurement NEXT espone microcopy coerente sul ruolo di supporto;
  - Lettura: procurement/documenti restano leggibili come supporto; il contratto stock lato NEXT viene invece centralizzato in `Magazzino`;
  - Blocco scritture: invariato come perimetro; nessun widening del barrier e nessun writer nuovo su costi/documenti/procurement.
- COME VERIFICARE:
  - eseguire `npx eslint src/next/NextMagazzinoPage.tsx src/next/NextProcurementReadOnlyPanel.tsx src/next/domain/nextProcurementDomain.ts src/next/nextData.ts src/next/domain/nextMagazzinoStockContract.ts src/next/domain/nextManutenzioniDomain.ts src/next/domain/nextMaterialiMovimentiDomain.ts`;
  - eseguire `npm run build`;
  - aprire `/next/magazzino?tab=documenti-costi` e verificare il pannello `Carichi stock da arrivi procurement`;
  - aprire `/next/materiali-da-ordinare?tab=arrivi` e verificare il richiamo operativo a `/next/magazzino?tab=documenti-costi`;
  - aprire `/next/inventario` e `/next/materiali-consegnati` e verificare i redirect verso `/next/magazzino?tab=...`;
  - non eseguire submit browser mutanti in ambienti collegati ai dataset reali senza sandbox dedicata.
- SE E CANDIDABILE A ESSERE PORTATO NELLA MADRE IN FUTURO: DA VALUTARE
- NOTE: task execution, non audit. L'autonomia stock lato NEXT viene centralizzata, ma il dominio complessivo `Magazzino NEXT` resta `PARZIALE` finche un audit separato non rivalida writer legacy, parity PDF e rischi multi-writer extra-perimetro.

### Voce 2026-04-10 PATCH CONTRATTO STOCK MAGAZZINO
- DATA: 2026-04-10
- TITOLO MODIFICA: Contratto stock condiviso per `Magazzino NEXT`
- OBIETTIVO: Rendere coerente e controllato il comportamento di `@inventario` nel perimetro NEXT, allineando `Magazzino`, `Manutenzioni`, `Materiali consegnati`, `Documenti e costi` e `Cisterne AdBlue` sullo stesso contratto logico di stock senza toccare la madre.
- FILE TOCCATI:
  - `src/next/NextMagazzinoPage.tsx`
...
- COSA E STATO CAMBIATO:
  - creato `nextMagazzinoStockContract.ts` come helper condiviso per UDM canoniche (`pz`, `lt`, `kg`, `mt`), chiave materiale pragmatica, `stockKey`, `stockLoadKeys`, compatibilita unita e riconoscimento AdBlue;
  - `NextMagazzinoPage.tsx` consolida i carichi manuali sul materiale esistente quando la chiave stock coincide, blocca aggiornamenti automatici con unita incoerente, preserva gli item a quantita zero e scrive `stockKey` anche su consegne e cambi AdBlue;
  - il tab `Documenti e costi` espone ora `Carichi stock da documenti` con deduplica prudente contro arrivi procurement e contro chiavi sorgente gia consolidate, senza aprire writer nuovi sui dataset documentali;
  - il cambio cisterna AdBlue registra `quantitaLitri` e scala davvero `@inventario`, con rollback se fallisce la seconda scrittura;
  - `nextManutenzioniDomain.ts` allinea writer e delete manutenzioni al nuovo contratto, con mismatch unita esplicito, stock insufficiente esplicito e rollback reale;
  - `nextMaterialiMovimentiDomain.ts` espone `stockKey` e canonicalizza le unita legacy per mantenere compatibilita retroattiva con i lettori.
- IMPATTO SU UI / LETTURA / BLOCCO SCRITTURE:
  - UI: minima; il modulo espone messaggi piu espliciti su unita consentite, selezione inventario AdBlue e carichi controllati da documenti;
  - Lettura: i lettori NEXT dei movimenti materiali ricevono `stockKey` e unita canonicalizzate; il matching magazzino/mezzo diventa piu stabile;
  - Blocco scritture: invariato come perimetro; il task rende piu sicure le scritture gia autorizzate senza widening del barrier.
- COME VERIFICARE:
  - eseguire `npx eslint src/next/NextMagazzinoPage.tsx src/next/domain/nextManutenzioniDomain.ts src/next/domain/nextMaterialiMovimentiDomain.ts src/next/domain/nextMagazzinoStockContract.ts`;
  - eseguire `npm run build`;
  - aprire `/next/magazzino` e verificare che il form inventario esponga solo `pz`, `lt`, `kg`, `mt`;
  - aprire `/next/magazzino?tab=cisterne-adblue` e verificare i campi `Litri scaricati` e `Articolo AdBlue inventario`;
  - aprire `/next/magazzino?tab=documenti-costi` e verificare il pannello `Carichi stock da documenti`, le righe bloccate per unita non risolta e le CTA `Consolida stock` / `Carica in inventario`;
  - non eseguire submit browser mutanti in ambienti collegati ai dataset reali senza sandbox dedicata.
- SE E CANDIDABILE A ESSERE PORTATO NELLA MADRE IN FUTURO: DA VALUTARE
- NOTE: task execution, non audit finale. Il dominio complessivo `Magazzino NEXT` resta `PARZIALE` finche un audit separato non rivalida i writer legacy fuori whitelist.
...
- DATA: 2026-04-09
- TITOLO MODIFICA: Creazione raccolta stabile `docs/fonti-pronte/`
- OBIETTIVO: Avere una cartella unica con copie sempre aggiornate delle fonti piu importanti del progetto, da usare come base rapida nelle nuove chat.
- FILE TOCCATI:
  - `docs/fonti-pronte/AGENTS.md`
  ...
  - `docs/fonti-pronte/REGISTRO_PUNTI_DA_VERIFICARE.md`
  - `docs/fonti-pronte/PROTOCOLLO_SICUREZZA_MODIFICHE.md`
  - `docs/fonti-pronte/PROCEDURA_MADRE_TO_CLONE.md`
  - `docs/fonti-pronte/AUDIT_MANUTENZIONI_NEXT_CROSSMODULO_PROMPT32_2026-04-09.md`
  - `docs/fonti-pronte/20260409_154405_manutenzioni_next_fix_crossmodulo_prompt34.md`
  - `docs/fonti-pronte/20260409_154405_continuity_manutenzioni_next_fix_crossmodulo_prompt34.md`
  - `docs/fonti-pronte/00_INDICE_FONTI_PRONTE.md`
  - `docs/fonti-pronte/01_OVERVIEW_PROGETTO_NEXT.md`
  - `AGENTS.md`
  - `docs/STATO_ATTUALE_PROGETTO.md`
  - `docs/product/STATO_MIGRAZIONE_NEXT.md`
  - `docs/product/REGISTRO_MODIFICHE_CLONE.md`
  ...
  - `docs/change-reports/20260409_170651_fonti_pronte_cartella_fonti_stabili.md`
  - `docs/continuity-reports/20260409_170651_continuity_fonti_pronte_cartella_fonti_stabili.md`
- COSA E STATO CAMBIATO:
  - creata una cartella unica `docs/fonti-pronte/` con mirror dei documenti chiave piu usati;
  - creato un indice con elenco, scopo e ordine di lettura rapido;
  - creata una overview sintetica della NEXT basata su fonti reali gia presenti nel repo;
  - fissata nei documenti guida la regola permanente di sincronizzazione tra sorgente e copia in `docs/fonti-pronte/`.
- IMPATTO SU UI / LETTURA / BLOCCO SCRITTURE:
  - UI: nessun impatto;
  - Lettura: onboarding piu rapido e meno dispersione delle fonti tra cartelle diverse;
  - Blocco scritture: invariato.
- COME VERIFICARE:
  - aprire `docs/fonti-pronte/` e verificare che contenga le copie aggiornate dei file chiave;
  - leggere `docs/fonti-pronte/00_INDICE_FONTI_PRONTE.md`;
  - verificare in `AGENTS.md` e `docs/product/PROTOCOLLO_SICUREZZA_MODIFICHE.md` la regola permanente di sincronizzazione;
  - confermare che non esistano modifiche a `src/*`.
- SE E CANDIDABILE A ESSERE PORTATO NELLA MADRE IN FUTURO: SI
- NOTE: task solo documentale; nessun build runtime richiesto.

### Voce 2026-04-09 PROMPT36
- DATA: 2026-04-09
- TITOLO MODIFICA: Euromecc Mega Patch — nuovi diagram SVG, mappa interattiva riepilogo, fix PDF
- OBIETTIVO: Aggiungere nuova area scaricoFornitore, diagrammi SalaCompressoriDiagram e ScaricoFornitoreDiagram, rendere la mappa nel riepilogo cliccabile con pannello dettaglio, sostituire html2canvas con serializzazione SVG inline nel PDF.
- FILE TOCCATI:
  - `src/next/euromeccAreas.ts` — nuova area `scaricoFornitore` con 6 componenti
  - `src/next/NextEuromeccPage.tsx` — MAP_GENERIC +2 nodi, hotspot arrays x2, SalaCompressoriDiagram, ScaricoFornitoreDiagram, branch condizionali area selector, RiepilogoMappaImpianto interattivo, svgToImageData, rimozione html2canvas
  - `src/next/next-euromecc.css` — classi riepilogo mappa dettaglio
- COSA E STATO CAMBIATO:
  - `scaricoFornitore` aggiunta in euromeccAreas.ts con 6 subcomponenti (SCF-01-*)
  - `compressore2` e `scaricoFornitore` aggiunti a MAP_GENERIC
  - SALA_COMPRESSORI_HOTSPOTS (8 hotspot) e SCARICO_FORNITORE_HOTSPOTS (6 hotspot) aggiunti
  - SalaCompressoriDiagram: SVG 760x640 con 2 accumulatori, 2 compressori, 2 essiccatori
  - ScaricoFornitoreDiagram: SVG 680x700 con silo, tubazioni, attacco fornitore, filtro, coclea, quadro pneumatico
  - Branch selector: compressore/compressore2 → SalaCompressoriDiagram; scaricoFornitore → ScaricoFornitoreDiagram
  - RiepilogoMappaImpianto: stato selectedAreaKey, onSelectArea attivo, pannello dettaglio con pending/done/issues filtrati per area
  - svgToImageData: serializzazione XML→Blob→Image→Canvas per PDF
  - html2canvas rimosso da generatePdfRiepilogo (mappa + diagram area)
- IMPATTO SU UI / LETTURA / BLOCCO SCRITTURE:
  - UI: due nuovi diagram SVG interattivi; mappa riepilogo cliccabile; PDF con SVG renderizzato correttamente
  - Lettura: nessun impatto — solo visualizzazione
  - Blocco scritture: nessuna nuova deroga al cloneWriteBarrier necessaria
- COME VERIFICARE:
  - Aprire area compressore o compressore2: deve mostrare SalaCompressoriDiagram con hotspot cliccabili
  - Aprire area scaricoFornitore: deve mostrare ScaricoFornitoreDiagram con 6 hotspot
  - Nel tab Riepilogo: cliccare un nodo mappa → pannello dettaglio compare a destra
  - Esportare PDF riepilogo: la mappa deve essere visibile nel PDF
  - `npm run build` — zero errori TypeScript
- SE E CANDIDABILE A ESSERE PORTATO NELLA MADRE IN FUTURO: NO — perimetro NEXT
- NOTE: CaricoTrenoDiagram non implementato — SVG body non disponibile nella spec. SERVE FILE EXTRA: corpo SVG CaricoTrenoDiagram.

### Voce 2026-04-09 PROMPT32
- DATA: 2026-04-09
- TITOLO MODIFICA: Audit profondo cross-modulo `Manutenzioni` NEXT
- OBIETTIVO: Verificare sul codice reale del repo se il modulo `/next/manutenzioni` e i suoi collegamenti con `Dossier`, `App Autisti`, `Quadro manutenzioni PDF`, `Dettaglio` e boundary NEXT vs madre restano coerenti dopo i cambi recenti, senza applicare patch runtime.
- FILE TOCCATI:
...
- COSA E STATO CAMBIATO:
  - creato un audit dedicato con verifica di route, reader, writer, filtri, PDF, Dossier e feed autisti realmente collegati al modulo;
  - aggiornati i documenti di stato con il verdetto auditato per blocco;
  - nessun file runtime o legacy e stato modificato.
- IMPATTO SU UI / LETTURA / BLOCCO SCRITTURE:
  - UI: nessun impatto diretto, audit documentale puro;
  - Lettura: chiarito che il modulo e funzionante ma non dimostrato come `CHIUSO`;
  - Blocco scritture: invariato.
- COME VERIFICARE:
  - leggere `docs/audit/AUDIT_MANUTENZIONI_NEXT_CROSSMODULO_PROMPT32_2026-04-09.md`;
  - verificare che il verdetto per tutti i blocchi resti `PARZIALE`;
  - verificare che i file runtime non siano stati patchati in questo run.
- SE E CANDIDABILE A ESSERE PORTATO NELLA MADRE IN FUTURO: NO
- NOTE: l'audit ha confermato una fragilita reale nel writer materiali di `src/next/domain/nextManutenzioniDomain.ts`; il fix richiede un task execution separato.

### Voce 2026-04-09 PROMPT31B
- DATA: 2026-04-09
- TITOLO MODIFICA: Foto reale della targa nel PDF del quadro + riduzione microcopy fissa in `Manutenzioni`
- OBIETTIVO: Stampare nel PDF del `Quadro manutenzioni` la foto reale del mezzo quando l'export riguarda una targa specifica, e alleggerire la UI del modulo spostando le spiegazioni non essenziali su tooltip hover/focus.
- FILE TOCCATI:
  - `src/next/NextManutenzioniPage.tsx`
  - `src/next/NextMappaStoricoPage.tsx`
  ...
  - `docs/product/REGISTRO_MODIFICHE_CLONE.md`
  - `CONTEXT_CLAUDE.md`
  - `docs/change-reports/20260409_141958_next_manutenzioni_pdf_foto_reale_tooltip_cleanup.md`
  - `docs/continuity-reports/20260409_141958_continuity_next_manutenzioni_pdf_foto_reale_tooltip_cleanup.md`
- COSA E STATO CAMBIATO:
  - l'export locale del quadro usa ora `fotoUrl` del mezzo associato alla targa come sorgente immagine primaria del PDF;
  - i PDF di targa singola stampano la foto reale del mezzo in testata, con fallback pulito se la foto manca;
  - rimossi testi descrittivi e note fisse ridondanti in dashboard, form, quadro e dettaglio;
  - aggiunti `title` / `aria-label` ai controlli che richiedono ancora spiegazione.
- IMPATTO SU UI / LETTURA / BLOCCO SCRITTURE:
  - UI: schermata piu pulita e meno verbosa;
  - Lettura: il PDF del quadro usa la foto reale del mezzo quando l'export riguarda una sola targa;
  - Blocco scritture: invariato; nessuna nuova scrittura business o tecnica.
- COME VERIFICARE:
  - aprire `/next/manutenzioni`, filtrare il quadro su una sola targa e generare il PDF;
  - verificare che il PDF mostri la foto reale del mezzo e non la tavola tecnica;
  - controllare che le spiegazioni non siano piu presenti come testo fisso ma risultino nei tooltip dei controlli principali.
- SE E CANDIDABILE A ESSERE PORTATO NELLA MADRE IN FUTURO: DA VALUTARE
- NOTE: il fallback PDF non inventa immagini; se `fotoUrl` manca, il PDF resta leggibile con riquadro neutro.

### Voce 2026-04-09 PROMPT30
- DATA: 2026-04-09
- TITOLO MODIFICA: Ripristino viste tecniche `Sinistra / Destra` nel `Dettaglio` e ricerca rapida `targa / autista` nel `Quadro manutenzioni`
- OBIETTIVO: Riportare nel dettaglio embedded di `/next/manutenzioni` le immagini tecniche da `public/gomme/*`, rimuovere `Fronte/Retro` dal runtime del dettaglio e aggiungere nel quadro un filtro pratico per `targa` e `autista`, senza reintrodurre `Calibra`.
- FILE TOCCATI:
  - `src/next/NextMappaStoricoPage.tsx`
  - `src/next/NextManutenzioniPage.tsx`
  - `src/next/next-mappa-storico.css`
  ...
  - `docs/product/REGISTRO_MODIFICHE_CLONE.md`
  - `CONTEXT_CLAUDE.md`
  - `docs/change-reports/20260409_134912_next_manutenzioni_viste_tecniche_dx_sx_ricerca_quadro.md`
  - `docs/continuity-reports/20260409_134912_continuity_next_manutenzioni_viste_tecniche_dx_sx_ricerca_quadro.md`
- COSA E STATO CAMBIATO:
  - il ramo `embedded` di `NextMappaStoricoPage` usa di nuovo il resolver tecnico gia presente nel domain gomme per mostrare la tavola del mezzo da `public/gomme/*`;
  - nel dettaglio embedded restano solo le tab `Sinistra` e `Destra`;
  - il quadro PDF mostra un nuovo input esplicito `Filtra per targa o autista`, che restringe l'elenco dei risultati mostrati.
- IMPATTO SU UI / LETTURA / BLOCCO SCRITTURE:
  - UI: il viewer del dettaglio torna a essere una vista tecnica pulita del mezzo, senza `Fronte/Retro` e senza ritorno di `Calibra`;
  - Lettura: il quadro PDF e piu rapido da usare per targa/autista;
  - Blocco scritture: invariato; nessuna nuova scrittura o override tecnico.
- COME VERIFICARE:
  - aprire `/next/manutenzioni`, andare nel tab `Dettaglio` e verificare che compaiano solo `Sinistra` / `Destra`;
  - verificare che il viewer mostri la tavola tecnica da `public/gomme/*` coerente con la categoria mezzo;
  - aprire `Quadro manutenzioni PDF`, usare il nuovo campo di ricerca e controllare il filtro per `targa` e `autista`.
- SE E CANDIDABILE A ESSERE PORTATO NELLA MADRE IN FUTURO: DA VALUTARE
- NOTE: il ramo standalone di `NextMappaStoricoPage` non e stato rifatto; il perimetro resta limitato al dettaglio embedded di `Manutenzioni`.

### Voce 2026-04-09 PROMPT29
- DATA: 2026-04-09
- TITOLO MODIFICA: Nuova UI tab Riepilogo Euromecc + export PDF con jsPDF e html2canvas
- OBIETTIVO: Sostituire il tab Riepilogo di `/next/euromecc` (textarea + window.print) con UI visiva professionale: mappa SVG impianto read-only, card per area ordinate per urgenza, layout speciale punti di carico con overlay frecce SVG, export PDF locale con jsPDF.
- FILE TOCCATI:
  - `src/next/NextEuromeccPage.tsx`
  - `src/next/next-euromecc.css`
  - `package.json`
- COSA E STATO CAMBIATO:
  - Rimosso JSX inline tab report (textarea + window.print) e funzioni collegate (buildReportText, copyText, reportText, handleCopyReport).
  - Aggiunto tipo `RiepilogoCardData`, funzione `generatePdfRiepilogo()`, funzione `buildRiepilogoCards()`, componenti `RiepilogoMappaImpianto`, `RiepilogoCaricoDiagram`, `RiepilogoAreaCard`, `RiepilogoTab`.
  - Il tab ora usa `<RiepilogoTab>` con mappa read-only, card area con sezioni da fare/fatte/problemi, layout a due colonne per punti di carico con overlay frecce SVG colorate per attivita aperte, bottone Esporta PDF.
  - Aggiunto `html2canvas ^1.4.1` a package.json (era gia in node_modules come dipendenza transitiva).
  - Aggiunte tutte le classi CSS `eur-riepilogo-*` in `next-euromecc.css`.
- IMPATTO SU UI / LETTURA / BLOCCO SCRITTURE: Solo il tab Riepilogo cambia visivamente. Nessuna scrittura Firestore. Nessun impatto sugli altri tab. Archivio PDF su Firestore non implementato (fase 2).
- COME VERIFICARE: Navigare a `/next/euromecc`, aprire il tab Riepilogo, verificare mappa, card aree, layout carico e generazione PDF locale con bottone Esporta PDF.
- SE E CANDIDABILE A ESSERE PORTATO NELLA MADRE IN FUTURO: DA VALUTARE
- NOTE: MapSvg, SiloDiagram, CaricoDiagram, CARICO_HOTSPOTS, SILO_HOTSPOTS, altri tab, Firestore non toccati. Archivio PDF fase 2 escluso esplicitamente dal prompt.

### Voce 2026-04-09 PROMPT28
- DATA: 2026-04-09
- TITOLO MODIFICA: Rimozione completa `Calibra` dal `Dettaglio` embedded di `Manutenzioni`
- OBIETTIVO: Pulire il tab `Dettaglio` di `/next/manutenzioni`, lasciando solo vista/foto del mezzo e riepilogo della manutenzione selezionata, senza toccare madre, Euromecc, PDF o backend.
- FILE TOCCATI:
  - `src/next/NextMappaStoricoPage.tsx`
  - `src/next/NextManutenzioniPage.tsx`
  ...
  - `docs/product/REGISTRO_MODIFICHE_CLONE.md`
  - `CONTEXT_CLAUDE.md`
  - `docs/change-reports/20260409_130252_next_manutenzioni_remove_calibra_dettaglio.md`
  - `docs/continuity-reports/20260409_130252_continuity_next_manutenzioni_remove_calibra_dettaglio.md`
- COSA E STATO CAMBIATO:
  - rimosso dal ramo `embedded` di `NextMappaStoricoPage` tutto il flusso `Calibra` e ogni marker/overlay tecnico nel viewer del dettaglio;
  - mantenuti solo tab vista, viewer statico foto/placeholder e riepilogo esplicito della manutenzione selezionata;
  - eliminati dal domain NEXT i read/write clone-side degli override tecnici non piu usati;
  - aggiornata la copy del form per non rimandare piu a una gestione foto/calibra non presente nel dettaglio embedded.
- IMPATTO SU UI / LETTURA / BLOCCO SCRITTURE:
  - UI: il `Dettaglio` risulta piu pulito e non mostra piu controlli o marker tecnici;
  - Lettura: la vista del mezzo resta disponibile per `Fronte / Sinistra / Destra / Retro` e i dati della manutenzione aperta sono leggibili in un blocco dedicato;
  - Blocco scritture: invariato; nessuna nuova scrittura business o visuale viene introdotta.
- COME VERIFICARE:
  - aprire `/next/manutenzioni` e andare nel tab `Dettaglio`;
  - verificare l'assenza di `Calibra`, palette, `Salva`, marker e drag tecnici;
  - cambiare vista tra `Fronte / Sinistra / Destra / Retro` e verificare che resti solo la foto/placeholder pulita;
  - selezionare un record dallo storico laterale e verificare il box `Manutenzione selezionata`.
- SE E CANDIDABILE A ESSERE PORTATO NELLA MADRE IN FUTURO: NO
- NOTE:
  - il ramo standalone di `NextMappaStoricoPage` conserva la gestione hotspot/foto e non e stato rifatto in questo task;
  - `npm run build` = `OK`.

### Voce 2026-04-09 PROMPT27
- DATA: 2026-04-09
- TITOLO MODIFICA: Distinzione esplicita cambio gomme ordinario vs straordinario nel clone `Manutenzioni`
- OBIETTIVO: Eliminare l'ambiguita nel runtime NEXT `Manutenzioni` tra rinnovo gomme ordinario per asse e interventi gomme straordinari puntuali, mantenendo leggibile il quadro PDF e senza toccare viewer tecnico, Euromecc o madre legacy.
- FILE TOCCATI:
  - `src/next/NextManutenzioniPage.tsx`
  - `src/next/domain/nextManutenzioniDomain.ts`
  ...
  - `docs/product/REGISTRO_MODIFICHE_CLONE.md`
  - `CONTEXT_CLAUDE.md`
  - `docs/change-reports/20260409_121444_next_manutenzioni_gomme_ordinario_straordinario.md`
  - `docs/continuity-reports/20260409_121444_continuity_next_manutenzioni_gomme_ordinario_straordinario.md`
- COSA E STATO CAMBIATO:
  - esteso in modo retrocompatibile il record clone-side di manutenzione con `gommeInterventoTipo` e `gommeStraordinario`;
  - lasciato il cambio gomme ordinario come flusso strutturato per asse tramite `gommePerAsse`;
  - introdotto un ramo UI dedicato `Gomme straordinarie` con motivo esplicito, asse facoltativo e quantita facoltativa;
  - escluso lo straordinario dal calcolo dello stato finale gomme per asse;
  - separato il quadro del modulo in `Stato gomme ordinario per asse` + `Eventi gomme straordinari`.
- IMPATTO SU UI / LETTURA / BLOCCO SCRITTURE:
  - UI: l'utente sceglie in modo esplicito se registrare un cambio gomme ordinario o uno straordinario;
  - Lettura: il quadro PDF non promuove piu in modo implicito eventi straordinari dentro lo stato ordinario per asse;
  - Blocco scritture: invariato a livello clone-wide; il modulo usa solo la deroga chirurgica gia autorizzata su `Manutenzioni`.
- COME VERIFICARE:
  - aprire `/next/manutenzioni`, tab `Nuova / Modifica`, selezionare `Tipo = Mezzo`;
  - verificare la presenza dei due sottotipi distinti `Gomme ordinarie per asse` e `Gomme straordinarie`;
  - salvare un ordinario e verificare che popoli lo stato per asse nel `Quadro manutenzioni PDF`;
  - salvare uno straordinario e verificare che compaia solo nella sezione `Eventi gomme straordinari`, non nello stato per asse;
  - esportare il PDF tabellare e verificare che la distinzione compaia anche nel sottotipo/descrizione.
- SE E CANDIDABILE A ESSERE PORTATO NELLA MADRE IN FUTURO: DA VALUTARE
- NOTE:
  - i record legacy gomme senza assi vengono trattati in modo prudente come eventi straordinari per non inquinare lo stato per asse;
  - `npm run build` = `OK`.
...
- OBIETTIVO: Eliminare il carattere sporco `Ã—` dal pulsante di chiusura del modale `Controllo originale`, mantenendo invariata la logica dati e il comportamento del bottone.
- FILE TOCCATI:
  ...
  - `docs/continuity-reports/2026-04-07_0941_continuity_lavori_fix_close_icon_controllo.md`
- COSA E STATO CAMBIATO:
  - sostituita nel solo modale `Controllo originale` la stringa corrotta hardcoded del close button con `&times;`;
  - aggiunto `aria-label` esplicito al bottone di chiusura;
  - nessuna modifica a resolver, letture `@controlli_mezzo_autisti`, match forte o UI degli altri modali.
- IMPATTO SU UI / LETTURA / BLOCCO SCRITTURE:
  - UI: il modale `Controllo originale` mostra ora una chiusura leggibile e stabile in alto a destra;
  - Lettura: invariata, nessun impatto sui dati o sui match;
  - Scritture: nessuna nuova scrittura e nessuna modifica al comportamento del modulo.
- COME VERIFICARE:
  - `node_modules\\.bin\\eslint.cmd src\\next\\NextDettaglioLavoroPage.tsx`
  - `npm run build`
  - aprire `/next/dettagliolavori/daade4a2-c681-46d0-99d4-1906d151116d?from=lavori-in-attesa`
  - cliccare `Apri controllo`
  - verificare che il bottone in alto a destra mostri `×`
  - cliccare il bottone e verificare la chiusura del modale
- SE E CANDIDABILE A ESSERE PORTATO NELLA MADRE IN FUTURO: DA VALUTARE
- NOTE:
  - causa reale verificata: nel JSX del modale controllo era presente una sequenza corrotta (`Ã—` / `Ãƒâ€”`), non un problema di matching o di logica dati.

### Voce 2026-04-07 189
- DATA: 2026-04-07
- TITOLO MODIFICA: Dettaglio `Lavori` NEXT esteso ai lavori nati da `controllo mezzo KO`
- OBIETTIVO: Far convivere nel dettaglio lavoro NEXT le due origini autisti reali, mantenendo intatto il flusso segnalazioni e aggiungendo testo reale + modale read-only anche per i lavori nati da `source.type = "controllo"`.
- FILE TOCCATI:
  - `src/next/NextDettaglioLavoroPage.tsx`
  - `docs/product/STATO_MIGRAZIONE_NEXT.md`
  - `docs/product/REGISTRO_MODIFICHE_CLONE.md`
  - `CONTEXT_CLAUDE.md`
  - `docs/change-reports/2026-04-07_0927_lavori_fix_origine_controllo.md`
  - `docs/continuity-reports/2026-04-07_0927_continuity_lavori_fix_origine_controllo.md`
- COSA E STATO CAMBIATO:
  - il resolver del dettaglio distingue ora in modo esplicito `source.type = "segnalazione"` e `source.type = "controllo"`;
  - il blocco UI passa da `Problema segnalato` a `Problema / esito origine`;
  - per i controlli legge davvero `@controlli_mezzo_autisti`, usa prima `source.id/originId` e poi solo il backlink reale `linkedLavoroId/linkedLavoroIds`;
  - il testo origine del controllo usa il campo reale `note` come priorita, con fallback su `dettaglio` / `messaggio` e integrazione dei check KO reali da `check/koItems`;
  - il bottone origine diventa dinamico: `Apri segnalazione` oppure `Apri controllo`, sempre in modale read-only interno al modulo.
- IMPATTO SU UI / LETTURA / BLOCCO SCRITTURE:
  - UI: il dettaglio espone il contesto reale anche per i lavori nati da controllo KO, senza route nuove o uscita dal modulo;
  - Lettura: aggiunta sola lettura di `@controlli_mezzo_autisti` nello stesso dettaglio, senza helper nuovi e senza matching fragile;
  - Scritture: nessuna nuova scrittura; il modulo continua a usare il perimetro reale gia esistente su `@lavori`.
- COME VERIFICARE:
  - `node_modules\\.bin\\eslint.cmd src\\next\\NextDettaglioLavoroPage.tsx`
  - `npm run build`
  - `npm run lint` (oggi resta rosso per debito storico globale fuori perimetro)
  - aprire `/next/lavori-in-attesa` e verificare il dettaglio in modale del lavoro `Segnalazione: freni - Freni da controllare`
  - aprire `/next/lavori-in-attesa` e verificare il dettaglio in modale del lavoro `Controllo KO: GOMME`
  - aprire `/next/dettagliolavori/7c6af494-9b02-4bf2-ac67-c994b39436c0?from=lavori-in-attesa` e verificare `Freni da controllare` + `Apri segnalazione`
  - aprire `/next/dettagliolavori/daade4a2-c681-46d0-99d4-1906d151116d?from=lavori-in-attesa` e verificare `1 asse rimorchio gomme lisce` + `Check KO: GOMME` + `Apri controllo`
  - aprire `/next/dettagliolavori/82df827a-b18b-43fa-b4ee-abf8e3b36389?from=lavori-in-attesa` oppure `/next/dettagliolavori/f8288347-2b06-4976-9e86-8ea152da1bd2?from=lavori-in-attesa` e verificare che il controllo multi-link apra il record corretto via `linkedLavoroIds`
  - verificare con replay locale del resolver che, senza `source.id` e senza backlink reale, il dettaglio non apra alcun controllo.
- SE E CANDIDABILE A ESSERE PORTATO NELLA MADRE IN FUTURO: DA VALUTARE
- NOTE:
  - debug reale verificato: nel caso controllo reale `daade4a2-c681-46d0-99d4-1906d151116d` il campo mostrato e `note = \"1 asse rimorchio gomme lisce \"`;
  - il link forte reale lavoro -> controllo e `source.id = controllo.id`, con fallback al backlink `linkedLavoroId/linkedLavoroIds`;
  - prima il caso non compariva perche il dettaglio leggeva solo `@segnalazioni_autisti_tmp` e non risolveva `@controlli_mezzo_autisti`.

### Voce 2026-04-06 188
- DATA: 2026-04-06
- TITOLO MODIFICA: Rifinitura UI/PDF/Home del modulo `Lavori` NEXT con logica reale invariata
- OBIETTIVO: Correggere difetti reali del modulo `Lavori` gia riaperto in scrittura nella NEXT, mostrando `Segnalato da` e `Autista solito`, migliorando il PDF reale, colorando la priorita in modo leggibile e portando anche i lavori in attesa nel blocco alert/scadenze della Home.
- FILE TOCCATI:
  ...
  - `docs/product/REGISTRO_MODIFICHE_CLONE.md`
  - `CONTEXT_CLAUDE.md`
  - `docs/change-reports/2026-04-06_1540_lavori_next_refine_ui_pdf_home_alert.md`
  - `docs/continuity-reports/2026-04-06_1540_continuity_lavori_next_refine_ui_pdf_home_alert.md`
- COSA E STATO CAMBIATO:
  - le tabelle `In attesa` e `Eseguiti` mostrano ora `Segnalato da` e `Autista solito` usando solo dati reali gia presenti nei record `@lavori` e nei mezzi collegati;
  - il dettaglio lavoro espone gli stessi campi nel riepilogo e li porta anche nel PDF reale del dettaglio;
  - l'export PDF esistente del modulo usa ora un layout tabellare piu largo e leggibile, con colonne che includono `Segnalato da` e `Autista solito`;
  - il form `Aggiungi` evidenzia la priorita selezionata con i tre colori richiesti e le righe tabella usano un evidenziatore tenue coerente con la priorita;
  - la Home NEXT divide il blocco alert in due card coerenti: `Scadenze` e `Lavori in attesa`, con contatori reali e preview dei lavori piu rilevanti.
- IMPATTO SU UI / LETTURA / BLOCCO SCRITTURE:
  - UI: migliora leggibilita del modulo `Lavori` e della Home senza cambiare route o shell;
  - Lettura: riuso dei dati reali gia disponibili in `@lavori` e `@mezzi_aziendali`, senza nuovi dataset o overlay finti;
  - Scritture: nessuna regressione; il modulo continua a usare il motore reale gia riaperto nel perimetro Lavori, senza toccare la deroga chirurgica o aprire altri moduli.
- COME VERIFICARE:
  - `node_modules\\.bin\\eslint.cmd src\\next\\NextLavoriDaEseguirePage.tsx src\\next\\NextDettaglioLavoroPage.tsx src\\next\\NextHomePage.tsx`
  - `npm run build`
  - avviare `npm run preview -- --host 127.0.0.1 --port 4182`
  - aprire `/next/lavori-in-attesa` e verificare `Segnalato da`, `Autista solito`, evidenziatore riga e export PDF
  - aprire `/next/lavori-eseguiti` e verificare gli stessi campi con PDF
  - aprire `/next/lavori-da-eseguire?tab=aggiungi` e verificare i colori `Alta/Media/Bassa`
  - aprire `/next/dettagliolavori/:lavoroId` e verificare i campi nel dettaglio
  - aprire `/next` e verificare la nuova card `Lavori in attesa` nel blocco alert
- SE E CANDIDABILE A ESSERE PORTATO NELLA MADRE IN FUTURO: DA VALUTARE
- NOTE:
  - nessuna modifica a `src/utils/cloneWriteBarrier.ts` in questa patch;
  - stato modulo `Lavori` confermato `PARZIALE` in attesa di audit separato.
...
- OBIETTIVO: Forzare i wrapper root delle pagine autisti inbox/admin a rispettare il box della colonna contenuto della shell globale NEXT, senza toccare file legacy, route o componenti runtime fuori whitelist.
- FILE TOCCATI:
  - `src/next/next-shell.css`
  - `docs/product/STATO_MIGRAZIONE_NEXT.md`
  - `docs/product/REGISTRO_MODIFICHE_CLONE.md`
  - `docs/change-reports/2026-04-03_1750_next-shell-scoped-autisti-inbox-layout-fix.md`
  - `docs/continuity-reports/2026-04-03_1750_continuity_next-shell-scoped-autisti-inbox-layout-fix.md`
- COSA E STATO CAMBIATO:
  - aggiunti override scoped `.next-shell .autisti-home`, `.next-shell .autisti-inbox-wrap` e `.next-shell .autisti-layout` con `width/max-width: 100%`, `min-width: 0`, reset margini laterali e `box-sizing: border-box`;
  - mantenuto il precedente override `.next-shell .autisti-admin-page`, verificando che `autisti-admin` resti corretta senza ulteriori patch.
- IMPATTO SU UI / LETTURA / BLOCCO SCRITTURE:
  - UI: `/next/autisti-inbox` rientra nella colonna contenuto della shell e non resta piu sotto la sidebar; `/next/autisti-admin` resta corretta;
  - Lettura: nessun dato, domain o read model toccato;
  - Blocco scritture: nessuna scrittura business riaperta, nessuna modifica a route o storage.
- COME VERIFICARE:
  - `npm run build`
  - `npm run preview -- --host 127.0.0.1 --port 4174`
  - aprire `/next/autisti-inbox` con sidebar aperta e verificare che `.autisti-inbox-wrap` stia nella colonna contenuto;
  - aprire `/next/autisti-admin` con sidebar aperta e verificare assenza di overlap;
  - aprire `/next/materiali-da-ordinare` e verificare assenza di regressioni shell/layout.
- SE E CANDIDABILE A ESSERE PORTATO NELLA MADRE IN FUTURO: NO
- NOTE:
  - fix confinato alla shell NEXT;
  - nessuna modifica a `src/autistiInbox/*`, `src/autisti/*`, `src/pages/*` o `src/next/NextShell.tsx`.

### Voce 2026-04-03 171
- DATA: 2026-04-03
- TITOLO MODIFICA: Fix shell globale NEXT su toggle sidebar visibile e push layout reale
- OBIETTIVO: Correggere in modo chirurgico due bug reali della shell globale: toggle non percepibile/non riapribile correttamente e sidebar ancora percepita come overlay su alcune superfici (`/next/autisti-admin`, `/next/autisti-inbox`).
- FILE TOCCATI:
  - `src/next/NextShell.tsx`
  - `src/next/next-shell.css`
  - `docs/product/STATO_MIGRAZIONE_NEXT.md`
  - `docs/product/REGISTRO_MODIFICHE_CLONE.md`
  - `docs/change-reports/2026-04-03_1740_next-shell-toggle-push-layout-fix.md`
  - `docs/continuity-reports/2026-04-03_1740_continuity_next-shell-toggle-push-layout-fix.md`
- COSA E STATO CAMBIATO:
  - `NextShell.tsx` mostra ora il toggle di chiusura dentro l'header/sidebar quando la nav e aperta e un bottone flottante esterno di riapertura quando la nav e chiusa;
  - `next-shell.css` mantiene il layout a due colonne reali, forza il contenuto a partecipare davvero alla griglia orizzontale e aggiunge lo stile del bottone flottante di riapertura con contrasto corretto;
  - rimossa la topbar shell usata solo per l'hamburger, cosi il logo non viene piu sovrapposto e la riapertura resta visibile anche a sidebar chiusa.
- IMPATTO SU UI / LETTURA / BLOCCO SCRITTURE:
  - UI: toggle visibile nel punto giusto in open/closed state e nessun overlap tra sidebar e contenuto sui path verificati;
  - Lettura: nessun dato, domain o read model toccato;
  - Blocco scritture: nessuna scrittura business riaperta, nessuna modifica a route o storage.
- COME VERIFICARE:
  - `npm run build`
  - `npm run preview -- --host 127.0.0.1 --port 4174`
  - aprire `/next/autisti-admin`, `/next/autisti-inbox`, `/next/materiali-da-ordinare`;
  - verificare da aperta toggle visibile nell'header sidebar e contenuto spinto a destra;
  - chiudere la sidebar e verificare bottone flottante visibile, colonna sinistra a `0px` e contenuto a tutta larghezza.
- SE E CANDIDABILE A ESSERE PORTATO NELLA MADRE IN FUTURO: NO
- NOTE:
  - fix confinato alla shell NEXT;
  - nessuna modifica a `src/next/NextHomePage.tsx`, `src/App.tsx`, file madre o CSS legacy esterni alla whitelist.

### Voce 2026-04-03 170
- DATA: 2026-04-03
- TITOLO MODIFICA: Fix CSS chirurgico shell globale su `autisti-admin` e `centro-controllo`
- OBIETTIVO: Correggere due bug visivi post-patch shell globale agendo solo su `next-shell.css`, senza toccare CSS legacy madre-like, route o runtime component.
- FILE TOCCATI:
  - `src/next/next-shell.css`
  - `docs/product/STATO_MIGRAZIONE_NEXT.md`
  - `docs/product/REGISTRO_MODIFICHE_CLONE.md`
  - `docs/change-reports/2026-04-03_1712_next-shell-visual-bugs-admin-centro-controllo.md`
  - `docs/continuity-reports/2026-04-03_1712_continuity_next-shell-visual-bugs-admin-centro-controllo.md`
- COSA E STATO CAMBIATO:
  - aggiunto `z-index` esplicito a `.next-shell__sidebar` per chiudere il conflitto di stacking su `/next/centro-controllo`;
  - aggiunto override scoped `.next-shell .autisti-admin-page` con `width: 100%`, `max-width: 100%`, `left: auto`, `transform: none` per neutralizzare il full-bleed di `AutistiAdmin.css` solo dentro la shell NEXT.
- IMPATTO SU UI / LETTURA / BLOCCO SCRITTURE:
  - UI: la sidebar torna visibile e non coperta su `/next/autisti-admin` e `/next/centro-controllo`;
  - Lettura: nessun dato, domain o snapshot toccato;
  - Blocco scritture: nessuna scrittura business riaperta, nessuna modifica a route o storage.
- COME VERIFICARE:
  - `npm run build`
  - `npm run preview -- --host 127.0.0.1 --port 4174`
  - aprire `/next/autisti-admin` e verificare che la sidebar resti visibile a sinistra senza essere invasa dal contenitore admin;
  - aprire `/next/centro-controllo` e verificare che il bordo/sidebar stia sopra la card `.cc-shell`.
- SE E CANDIDABILE A ESSERE PORTATO NELLA MADRE IN FUTURO: NO
- NOTE:
  - fix confinato alla shell NEXT;
  - `src/autistiInbox/AutistiAdmin.css` e `src/pages/CentroControllo.css` restano intatti.

### Voce 2026-04-03 169
- DATA: 2026-04-03
- TITOLO MODIFICA: Sidebar globale NEXT spostata da Home a `NextShell`
- OBIETTIVO: Rendere la sidebar una shell globale per tutte le route figlie di `/next` che passano da `NextShell`, usando un catalogo nav centralizzato e lasciando intatto il subtree separato `/next/autisti/*`.
- FILE TOCCATI:
  - `src/next/NextShell.tsx`
  - `src/next/NextHomePage.tsx`
  - `src/next/next-shell.css`
  ...
  - `docs/product/STATO_MIGRAZIONE_NEXT.md`
  - `docs/product/REGISTRO_MODIFICHE_CLONE.md`
  - `docs/change-reports/2026-04-03_1656_next-shell-global-sidebar.md`
  - `docs/continuity-reports/2026-04-03_1656_continuity_next-shell-global-sidebar.md`
- COSA E STATO CAMBIATO:
  - `nextData.ts` espone ora `NEXT_SHELL_NAV_SECTIONS` come catalogo unico della sidebar, con tutte le sezioni richieste, voci disabled, `IA interna` e sezione `AUTISTI`;
  - `NextShell.tsx` renderizza sidebar, collassi di sezione, stato attivo delle voci, toggle globale hamburger e layout shell a due colonne con `Outlet`;
  - `NextHomePage.tsx` non contiene piu la sidebar e mantiene solo dashboard, banner, pannello `IA interna`, stat card e widget placeholder;
  - `next-shell.css` ospita gli stili della shell globale; `next-home.css` resta limitato alla dashboard destra.
- IMPATTO SU UI / LETTURA / BLOCCO SCRITTURE:
  - UI: la sidebar e ora visibile su tutte le route figlie di `NextShell`, non solo su `/next`;
  - Lettura: nessun dato reale aggiunto, nessun domain/read model toccato;
  - Blocco scritture: nessuna scrittura business riaperta, nessuna modifica a route o storage.
- COME VERIFICARE:
  - `node_modules\\.bin\\eslint.cmd src/next/NextShell.tsx src/next/NextHomePage.tsx src/next/nextData.ts`
  - `npm run build`
  - `npm run preview -- --host 127.0.0.1 --port 4174`
  - verificare la sidebar su `/next`, `/next/mezzi`, `/next/manutenzioni`, `/next/materiali-da-ordinare`, `/next/ia/interna`, `/next/autisti-inbox`, `/next/autisti-admin`;
  - chiudere e riaprire il menu con il toggle e verificare che la shell passi da colonna sidebar `320px` a `0px` e ritorno;
  - aprire `/next/autisti` e verificare assenza della shell globale e presenza del layout separato autisti.
- SE E CANDIDABILE A ESSERE PORTATO NELLA MADRE IN FUTURO: NO
- NOTE:
  - nessuna modifica a `src/App.tsx` o al subtree `src/next/autisti/*`;
  - il catalogo nav e centralizzato in `nextData.ts`, non piu hardcoded in `NextHomePage.tsx`.

### Voce 2026-04-03 168
- DATA: 2026-04-03
- TITOLO MODIFICA: Nuova Home NEXT placeholder con sidebar collassabile e dashboard UI dedicata
- OBIETTIVO: Sostituire la vecchia Home NEXT con una dashboard statica coerente con il layout approvato, lavorando solo sulla UI e senza collegare dati reali, route nuove o logiche business.
- FILE TOCCATI:
  - `src/next/NextHomePage.tsx`
  - `src/next/next-home.css`
  - `docs/product/STATO_MIGRAZIONE_NEXT.md`
  - `docs/product/REGISTRO_MODIFICHE_CLONE.md`
  - `docs/change-reports/2026-04-03_1448_next-home-ui-placeholder-layout.md`
  - `docs/continuity-reports/2026-04-03_1448_continuity_next-home-ui-placeholder-layout.md`
- COSA E STATO CAMBIATO:
  - `NextHomePage.tsx` renderizza ora una Home autonoma con sidebar a sezioni collassabili, topbar, banner alert, pannello `IA interna`, stat card e widget placeholder;
  - le voci sidebar puntano solo a route NEXT gia esistenti quando coerenti, mentre `Rimorchi`, `Scadenze` e `Impostazioni` restano disabled;
  - `next-home.css` introduce il layout desktop-first e il visual system locale della nuova Home senza toccare altri moduli NEXT.
- IMPATTO SU UI / LETTURA / BLOCCO SCRITTURE:
  - UI: la route `/next` mostra una nuova dashboard visiva coerente con lo screenshot approvato;
  - Lettura: nessun aggancio a domain, snapshot o dati runtime reali, solo placeholder statici;
  - Blocco scritture: nessuna scrittura business riaperta, nessuna modifica a route o storage.
- COME VERIFICARE:
  - `node_modules\\.bin\\eslint.cmd src/next/NextHomePage.tsx`
  - `npm run build`
  - `npm run preview -- --host 127.0.0.1 --port 4173`
  - aprire `http://127.0.0.1:4173/next` e verificare sidebar scura collassabile, banner alert, pannello `IA interna`, 4 stat card e 4 widget placeholder.
- SE E CANDIDABILE A ESSERE PORTATO NELLA MADRE IN FUTURO: NO
- NOTE:
  - patch solo UI;
  - nessuna modifica a `NextShell.tsx`, `src/App.tsx`, writer, route o file madre.

### Voce 2026-04-03 167
- DATA: 2026-04-03
- TITOLO MODIFICA: Riallineamento del calcolo stato import preventivi NEXT alla logica reale della madre
- OBIETTIVO: Correggere solo il calcolo di `NON IMPORTATO` / `IMPORTATO PARZIALE` / `IMPORTATO COMPLETO` nel tab `Prezzi & Preventivi` di `/next/materiali-da-ordinare`, replicando la logica riga-per-riga della madre senza toccare writer, route o UI fuori scope.
- FILE TOCCATI:
  - `src/next/domain/nextProcurementDomain.ts`
  - `src/next/NextProcurementConvergedSection.tsx`
  - `docs/product/STATO_MIGRAZIONE_NEXT.md`
  - `docs/product/REGISTRO_MODIFICHE_CLONE.md`
  - `docs/change-reports/2026-04-03_1412_procurement-next-import-status-mother-alignment.md`
  - `docs/continuity-reports/2026-04-03_1412_continuity_procurement-next-import-status-mother-alignment.md`
- COSA E STATO CAMBIATO:
  - `nextProcurementDomain.ts` espone ora nel read model le righe complete del preventivo e il `fontePreventivoId` delle voci listino;
  - `NextProcurementConvergedSection.tsx` non conta piu l'import dal solo numero preventivo o dalla preview parziale, ma ripete la logica madre su match per codice/descrizione, score di compatibilita e riuso dei match gia consumati;
  - lo stato `FONTE DA COLLEGARE` e `IMPORTATO (DA VERIFICARE n)` viene ora calcolato con lo stesso criterio della madre quando emergono quei casi nel dataset.
- IMPATTO SU UI / LETTURA / BLOCCO SCRITTURE:
  - UI: il badge stato import nel tab `Prezzi & Preventivi` riflette ora il criterio reale della madre sui record letti;
  - Lettura: il clone continua a vivere solo sul layer NEXT read-only, con piu dettaglio esposto dal domain ma senza cambiare dataset o contratti writer;
  - Blocco scritture: nessuna scrittura business reale riaperta, nessun `storageSync`, nessun mount runtime madre.
- COME VERIFICARE:
  - `node_modules\\.bin\\eslint.cmd src/next/NextProcurementConvergedSection.tsx src/next/domain/nextProcurementDomain.ts`
  - `npm run build`
  - aprire `/acquisti?tab=preventivi` e `/next/materiali-da-ordinare?tab=preventivi` e verificare:
    - `MARIBA / XC/STD/2600119` -> `IMPORTATO COMPLETO 5/5`;
    - `MARIBA / 534909` -> `IMPORTATO COMPLETO 7/7`.
- SE E CANDIDABILE A ESSERE PORTATO NELLA MADRE IN FUTURO: NO
- NOTE:
  - patch stretta, limitata al solo calcolo stato import preventivi nel clone;
  - `NextMaterialiDaOrdinarePage.tsx` e `NextProcurementReadOnlyPanel.tsx` sono rimasti intatti.

### Voce 2026-04-03 166
- DATA: 2026-04-03
- TITOLO MODIFICA: Chiusura delta visivo runtime della tab `Ordine materiali` nel procurement NEXT
- OBIETTIVO: Riallineare solo la tab `Ordine materiali` di `/next/materiali-da-ordinare` alla resa browser reale della madre `Acquisti`, senza toccare domain, writer, route o altre tab.
- FILE TOCCATI:
  - `src/next/NextMaterialiDaOrdinarePage.tsx`
  - `src/next/next-procurement-route.css`
  - `docs/product/STATO_MIGRAZIONE_NEXT.md`
  - `docs/product/REGISTRO_MODIFICHE_CLONE.md`
  - `docs/change-reports/2026-04-03_1326_materiali-da-ordinare-ordine-materiali-parity-visiva-runtime.md`
  - `docs/continuity-reports/2026-04-03_1326_continuity_materiali-da-ordinare-ordine-materiali-parity-visiva-runtime.md`
- COSA E STATO CAMBIATO:
  - `NextMaterialiDaOrdinarePage.tsx` usa ora nel ramo `Ordine materiali` la stessa gabbia madre `om-wrap` / `om-content`, rimuove le CTA footer extra e riallinea microcopy, KPI label e menu azioni riga al kebab della madre;
  - `next-procurement-route.css` riallinea la palette pulsanti embedded del ramo al tema madre senza toccare i dati;
  - nessun altro tab procurement e stato modificato in questo prompt.
- IMPATTO SU UI / LETTURA / BLOCCO SCRITTURE:
  - UI: il delta visivo reale della tab `Ordine materiali` rispetto a `/acquisti` viene chiuso sul runtime browser;
  - Lettura: restano in uso solo `readNextFornitoriSnapshot()` e `readNextProcurementSnapshot()` gia presenti;
  - Blocco scritture: nessuna scrittura business reale riaperta, nessun writer o storage legacy reintrodotto.
- COME VERIFICARE:
  - `node_modules\\.bin\\eslint.cmd src/next/NextMaterialiDaOrdinarePage.tsx`
  - `node_modules\\.bin\\eslint.cmd src/next/next-procurement-route.css`
  - `npm run build`
  - aprire `/acquisti` e `/next/materiali-da-ordinare`, restare su `Ordine materiali`, aggiungere una riga materiale temporanea e verificare footer essenziale, menu kebab in colonna `Azioni`, label `Totale parziale` e shell madre-like.
- SE E CANDIDABILE A ESSERE PORTATO NELLA MADRE IN FUTURO: NO
- NOTE:
  - verifica runtime reale eseguita con browser headless locale e screenshot comparativi;
  - `eslint` sul CSS segnala file ignorato per assenza di configurazione CSS, non errore runtime;
  - il procurement top-level resta `PARZIALE`.
...
- OBIETTIVO: Correggere `/next/materiali-da-ordinare` sulla base del runtime reale a browser e non dei report, eliminando la shell stretta e il footer overlay dovuti a collisioni CSS globali.
- FILE TOCCATI:
  - `src/next/NextMaterialiDaOrdinarePage.tsx`
  - `src/next/next-procurement-route.css`
  - `docs/product/STATO_MIGRAZIONE_NEXT.md`
  - `docs/product/REGISTRO_MODIFICHE_CLONE.md`
  - `docs/change-reports/2026-04-02_1110_materiali-da-ordinare-runtime-browser-fix-definitivo.md`
  - `docs/continuity-reports/2026-04-02_1110_continuity_materiali-da-ordinare-runtime-browser-fix-definitivo.md`
- COSA E STATO CAMBIATO: Verificato con browser headless che la shell `/next` non imponeva il layout sbagliato; il problema reale era il conflitto globale di `src/pages/DettaglioOrdine.css` su `.mdo-page` e `.mdo-card`, che stringeva il procurement a `430px` e lasciava attiva la `mdo-sticky-bar` dark overlay. Ho introdotto un wrapper locale `next-mdo-route`, un CSS scoped procurement NEXT e ho riallineato la pagina alla resa madre reale vista in `/acquisti`: `mdo-page--embedded mdo-page--single`, `mdo-card--embedded mdo-card--single`, `mdo-single-card`, `mdo-card-footer-bar`.
- IMPATTO SU UI / LETTURA / BLOCCO SCRITTURE: Runtime desktop procurement verificato largo e non piu embedded; nessuna scrittura business riaperta; convergenza procurement mantenuta.
- COME VERIFICARE: Aprire `http://localhost:5173/next/materiali-da-ordinare` e verificare nel browser che `.mdo-card` sia larga circa `1400px`, che non esistano `.mdo-sticky-bar` o `.mdo-workspace` e che il footer sia `mdo-card-footer-bar`; eseguire `npm run build`.
- SE E CANDIDABILE A ESSERE PORTATO NELLA MADRE IN FUTURO: NO
- NOTE: Fix basato su DOM e computed styles reali del browser; il conflitto CSS deriva da classi globali madre omonime e viene neutralizzato solo nel perimetro NEXT.

### Voce 2026-04-02 162
- DATA: 2026-04-02
- TITOLO MODIFICA: Procurement NEXT top-level riportato al layout desktop standalone della madre
- OBIETTIVO: Eliminare definitivamente la resa embedded/stretta di `/next/materiali-da-ordinare` e riportare il modulo canonico sul layout desktop reale della madre, senza rompere la convergenza procurement.
- FILE TOCCATI:
  - `src/next/NextMaterialiDaOrdinarePage.tsx`
  - `docs/product/STATO_MIGRAZIONE_NEXT.md`
  - `docs/product/REGISTRO_MODIFICHE_CLONE.md`
  - `docs/change-reports/2026-04-02_1010_materiali-da-ordinare-next-layout-desktop-standalone-madre.md`
  - `docs/continuity-reports/2026-04-02_1010_continuity_materiali-da-ordinare-next-layout-desktop-standalone-madre.md`
- COSA E STATO CAMBIATO: Verificato che la shell `/next` non imponeva il layout stretto. Il vincolo errato era interno a `NextMaterialiDaOrdinarePage.tsx`, che usava ancora al top-level il ramo embedded della madre (`mdo-page--embedded`, `mdo-card--embedded`, `mdo-page--single`, `mdo-card--single`). Il modulo canonico torna ora al ramo standalone desktop della madre con `mdo-page`, `mdo-card`, `mdo-workspace` e `mdo-sticky-bar`, mantenendo intatte le tab convergenti.
- IMPATTO SU UI / LETTURA / BLOCCO SCRITTURE: Pagina procurement desktop larga e strutturata come modulo vero, non piu come card embedded; nessuna scrittura business riaperta; convergenza `Ordini` / `Arrivi` / `Prezzi & Preventivi` invariata.
- COME VERIFICARE: Aprire `/next/materiali-da-ordinare` su desktop e verificare shell larga, header non compresso, workspace a due pannelli, tabella larga e footer sticky dentro la grammatica standalone della madre; eseguire `npm run build`.
- SE E CANDIDABILE A ESSERE PORTATO NELLA MADRE IN FUTURO: NO
- NOTE: Il wrapper esterno `/next` e stato verificato e non e il responsabile del layout stretto; il problema era locale al top-level procurement NEXT.

### Voce 2026-04-02 161
- DATA: 2026-04-02
- TITOLO MODIFICA: Fix layout desktop di `Materiali da ordinare` sul ramo embedded della madre
- OBIETTIVO: Eliminare shell stretta, pannello destro spezzato e sticky bar scura overlay da `/next/materiali-da-ordinare`, copiando il layout desktop reale della madre il piu 1:1 possibile senza cambiare la convergenza procurement.
- FILE TOCCATI:
  - `src/next/NextMaterialiDaOrdinarePage.tsx`
  - `docs/product/STATO_MIGRAZIONE_NEXT.md`
  - `docs/product/REGISTRO_MODIFICHE_CLONE.md`
  - `docs/change-reports/2026-04-02_0940_materiali-da-ordinare-next-fix-layout-desktop-madre.md`
  - `docs/continuity-reports/2026-04-02_0940_continuity_materiali-da-ordinare-next-fix-layout-desktop-madre.md`
- COSA E STATO CAMBIATO: La pagina NEXT ha abbandonato il ramo standalone stretto a due colonne e usa ora la shell embedded/single-card reale della madre: toolbar unica, tabella larga dentro la card e footer azioni interno `mdo-card-footer-bar`. La `mdo-sticky-bar` dark overlay e stata rimossa dal runtime della pagina.
- IMPATTO SU UI / LETTURA / BLOCCO SCRITTURE: Desktop procurement piu fedele alla madre e senza overlay che coprono il form; nessuna scrittura business riaperta; tab convergenti e architettura procurement invariati.
- COME VERIFICARE: Aprire `/next/materiali-da-ordinare` su desktop e verificare shell larga, assenza di pannello destro spezzato, assenza di barra scura overlay in basso; eseguire `npm run build`.
- SE E CANDIDABILE A ESSERE PORTATO NELLA MADRE IN FUTURO: NO
- NOTE: La fonte di verita usata e il ramo desktop embedded/single-card di `src/pages/MaterialiDaOrdinare.tsx` / `MaterialiDaOrdinare.css`, non una reinterpretazione clone-specifica.

### Voce 2026-04-02 160
- DATA: 2026-04-02
- TITOLO MODIFICA: Fix loop runtime nel procurement convergente NEXT
- OBIETTIVO: Eliminare il loop infinito che mandava in crash `/next/materiali-da-ordinare` dopo la convergenza procurement, senza toccare la madre o cambiare l'architettura appena approvata.
- FILE TOCCATI:
  - `src/next/internal-ai/internalAiUniversalRequestsRepository.ts`
  - `docs/product/STATO_MIGRAZIONE_NEXT.md`
  - `docs/product/REGISTRO_MODIFICHE_CLONE.md`
  - `docs/change-reports/2026-04-02_0915_fix-loop-procurement-convergente-getsnapshot.md`
  - `docs/continuity-reports/2026-04-02_0915_continuity_fix-loop-procurement-convergente-getsnapshot.md`
- COSA E STATO CAMBIATO: Il repository IA universale usato dal procurement restituiva uno snapshot nuovo a ogni `getSnapshot`, con nuovi array clonati, causando il loop di `useSyncExternalStore`. Ho introdotto una cache stabile dello snapshot finche lo stato repository non cambia e ho rimosso la funzione di clone locale non piu usata.
- IMPATTO SU UI / LETTURA / BLOCCO SCRITTURE: Crash eliminato; convergenza procurement e UI restano invariate; nessuna scrittura business riaperta.
- COME VERIFICARE: Aprire `/next/materiali-da-ordinare` senza overlay errore React/Vite; eseguire `npm run build`.
- SE E CANDIDABILE A ESSERE PORTATO NELLA MADRE IN FUTURO: NO
- NOTE: Il fix e tecnico e locale al clone/NEXT; non cambia moduli procurement secondari, route o layout.

### Voce 2026-04-02 159
- DATA: 2026-04-02
- TITOLO MODIFICA: Convergenza definitiva procurement NEXT sul modulo unico `Materiali da ordinare`
- OBIETTIVO: Rendere `/next/materiali-da-ordinare` l'unico procurement top-level reale della NEXT, assorbendo ordini, arrivi e dettaglio nel modulo canonico e declassando i vecchi ingressi secondari a compatibilita.
- FILE TOCCATI:
  - `src/next/NextMaterialiDaOrdinarePage.tsx`
  - `src/next/NextProcurementConvergedSection.tsx`
  - `src/next/NextProcurementReadOnlyPanel.tsx`
  ...
  - `docs/product/STATO_MIGRAZIONE_NEXT.md`
  - `docs/product/REGISTRO_MODIFICHE_CLONE.md`
  - `docs/change-reports/2026-04-02_0835_procurement-next-convergenza-definitiva-materiali-unico.md`
  - `docs/continuity-reports/2026-04-02_0835_continuity_procurement-next-convergenza-definitiva-materiali-unico.md`
- COSA E STATO CAMBIATO: `Materiali da ordinare` conserva la shell madre-like ma ora incorpora ordini, arrivi, dettaglio ordine e preview prezzi/preventivi nello stesso modulo canonico. Le CTA interne non aprono piu moduli procurement paralleli: cambiano tab o dettaglio nel path canonico `/next/materiali-da-ordinare`. `NextProcurementReadOnlyPanel` e stato riusato in modalita embedded e `NextProcurementStandalonePage` e stato ridotto a redirect di compatibilita dai path procurement storici verso il modulo unico.
- IMPATTO SU UI / LETTURA / BLOCCO SCRITTURE: Un solo procurement top-level visibile nella NEXT; ordini/arrivi/dettaglio restano consultabili ma dentro il modulo unico; nessuna scrittura business riaperta.
- COME VERIFICARE: Aprire `/next/materiali-da-ordinare`, passare tra `Fabbisogni`, `Ordini`, `Arrivi`, `Prezzi & Preventivi`, aprire un dettaglio ordine e verificare che i vecchi path `/next/acquisti`, `/next/ordini-in-attesa`, `/next/ordini-arrivati`, `/next/dettaglio-ordine/:ordineId` redirigano tutti al modulo unico; eseguire `npm run build`.
- SE E CANDIDABILE A ESSERE PORTATO NELLA MADRE IN FUTURO: DA VALUTARE
- NOTE: `NextProcurementReadOnlyPanel` non e stato rimosso dal codice perche resta consumato da runtime NEXT fuori perimetro prompt; il declassamento e stato quindi fatto in modo compatibile e non distruttivo.

### Voce 2026-04-02 158
- DATA: 2026-04-02
- TITOLO MODIFICA: Convergenza procurement NEXT sul ramo standalone della madre
- OBIETTIVO: Portare `/next/materiali-da-ordinare` il piu vicino possibile al modulo madre reale lato struttura, UI e comportamento pratico, senza toccare la madre e senza rompere i runtime procurement secondari ancora vivi.
- FILE TOCCATI:
  - `src/next/NextMaterialiDaOrdinarePage.tsx`
  - `docs/product/STATO_MIGRAZIONE_NEXT.md`
  - `docs/product/REGISTRO_MODIFICHE_CLONE.md`
...
- COSA E STATO CAMBIATO: La pagina NEXT e stata riscritta seguendo il ramo standalone reale della madre `MaterialiDaOrdinare`: header, tabs, workspace a due pannelli, quick link, tabella, sticky bar e modale placeholder. Sono stati rimossi gli override clone-specifici `embedded` e gli inline style madre-like locali. Restano solo gli adattamenti strettamente necessari al clone: lettura fornitori dal domain NEXT, navigate ai path `/next/*`, preview foto locale/automatica e blocco read-only di aggiunta materiale, eliminazione materiale, conferma ordine, PDF e preventivo.
- IMPATTO SU UI / LETTURA / BLOCCO SCRITTURE: Parity esterna del modulo principale procurement piu fedele alla madre; nessuna scrittura business riaperta; `Ordini in attesa`, `Ordini arrivati` e `Dettaglio ordine` restano secondari dietro le quinte e non vengono rimossi.
- COME VERIFICARE: Aprire `/next/materiali-da-ordinare`, verificare shell standalone della madre, aggiunta/rimozione locale materiali, quick link verso ordini/arrivi, sticky bar e modale placeholder; eseguire `npm run build`.
- SE E CANDIDABILE A ESSERE PORTATO NELLA MADRE IN FUTURO: NO
- NOTE: Nessuna modifica a `src/App.tsx` o ai runtime procurement secondari, perche i consumer clone-side reali risultano ancora vivi dagli audit.
...
### Voce 2026-04-02 125
- DATA: 2026-04-02
- TITOLO MODIFICA: Procurement convergente NEXT allineato su prefill, bozza locale e ricerca cross-tab
- OBIETTIVO: Ridurre i delta comportamentali reali di `/next/materiali-da-ordinare` rispetto alla madre senza riaprire moduli procurement top-level secondari e senza introdurre scritture business.
- FILE TOCCATI:
  - `src/next/NextMaterialiDaOrdinarePage.tsx`
  - `src/next/NextProcurementConvergedSection.tsx`
  - `src/next/NextProcurementReadOnlyPanel.tsx`
  - `docs/product/STATO_MIGRAZIONE_NEXT.md`
  - `docs/product/REGISTRO_MODIFICHE_CLONE.md`
  - `docs/change-reports/2026-04-02_1215_materiali-da-ordinare-next-parity-comportamentale-base.md`
  - `docs/continuity-reports/2026-04-02_1215_continuity_materiali-da-ordinare-next-parity-comportamentale-base.md`
- COSA E STATO CAMBIATO:
  - il payload `iaHandoff` procurement viene applicato davvero a fornitore, descrizione, ricerca e tab canonica;
  - la bozza locale del modulo unico viene salvata/ripristinata in `sessionStorage`;
  - la riga materiale temporanea conserva il fornitore scelto;
  - la ricerca top-level filtra anche le viste convergenti `Ordini`, `Arrivi`, `Preventivi` e `Listino`;
  - `CONFERMA ORDINE` resta read-only ma svuota la bozza locale come nel post-salvataggio madre.
- IMPATTO SU UI / LETTURA / BLOCCO SCRITTURE:
  - UI: nessun redesign; il modulo unico si comporta in modo piu coerente tra form, tab e dettaglio;
  - Lettura: si continuano a usare `@fornitori`, `@ordini`, `@preventivi`, `@preventivi_approvazioni`, `@listino_prezzi` tramite layer NEXT read-only;
  - Blocco scritture: nessuna scrittura business reale aggiunta.
- COME VERIFICARE:
  - `npm run build`
  - aprire `/next/materiali-da-ordinare`;
  - verificare prefill `iaHandoff`, aggiunta materiale con fornitore visibile in tabella, persistenza locale del draft e ricerca nelle tab convergenti.
- SE E CANDIDABILE A ESSERE PORTATO NELLA MADRE IN FUTURO: `DA VERIFICARE`; il comportamento converge meglio ma il modulo resta ancora read-only e non replica i writer reali della madre.
- NOTE:
  - build `OK`;
...
### Voce 2026-04-02 126
- DATA: 2026-04-02
- TITOLO MODIFICA: Procurement convergente NEXT esteso su listino fornitore, preview e totali del modulo unico
- OBIETTIVO: Ridurre altri delta reali tra madre completa e `/next/materiali-da-ordinare` portando nel modulo unico preview fornitore, suggerimenti listino, prezzo/totale riga e risultati completi `Prezzi & Preventivi`.
- FILE TOCCATI:
  - `src/next/NextMaterialiDaOrdinarePage.tsx`
  - `src/next/NextProcurementConvergedSection.tsx`
  - `docs/product/STATO_MIGRAZIONE_NEXT.md`
  - `docs/product/REGISTRO_MODIFICHE_CLONE.md`
  - `docs/change-reports/2026-04-02_1335_procurement-next-parity-estesa-materiali-da-ordinare.md`
  - `docs/continuity-reports/2026-04-02_1335_continuity_procurement-next-parity-estesa-materiali-da-ordinare.md`
- COSA E STATO CAMBIATO:
  - il ramo `Fabbisogni` usa ora suggerimenti listino filtrati per fornitore attivo e descrizione articolo;
  - la selezione listino precompila prezzo, valuta, UDM e riferimento preventivo della riga temporanea;
  - la UI mostra preview sintetica del fornitore selezionato, warning UDM, prezzi mancanti, totali per valuta e note ordine della bozza;
  - le azioni riga restano clone-safe ma ora espongono nota, foto, documento collegato e jump a `Listino` quando manca il prezzo;
  - `Prezzi & Preventivi` non taglia piu il dataset a 12 record.
- IMPATTO SU UI / LETTURA / BLOCCO SCRITTURE:
  - UI: la parte `Ordine materiali` converge meglio sulla madre `Acquisti` senza riaprire moduli procurement separati;
  - Lettura: si continua a usare il layer NEXT read-only su `@fornitori`, `@ordini`, `@preventivi`, `@preventivi_approvazioni`, `@listino_prezzi`;
  - Blocco scritture: nessuna scrittura business reale introdotta; conferma ordine, PDF e upload preventivi restano bloccati.
- COME VERIFICARE:
  - `npm run build`
  - aprire `/next/materiali-da-ordinare`;
  - selezionare un fornitore, digitare una descrizione compatibile con il listino e verificare suggerimenti, preview prezzo, totale e footer con warning/bozza;
  - aprire `Prezzi & Preventivi` e verificare che il dataset non sia piu troncato a 12 righe.
- SE E CANDIDABILE A ESSERE PORTATO NELLA MADRE IN FUTURO: `DA VERIFICARE`; la convergenza comportamentale cresce ma la parity totale resta aperta.
- NOTE:
  - build `OK`;
  - il procurement top-level resta `PARZIALE`, non `CHIUSO`.
...
### Voce 2026-04-06 132
- DATA: 2026-04-06
- TITOLO MODIFICA: `Lavori` NEXT con UI unificata, logica reale e deroga chirurgica su `@lavori`
- OBIETTIVO: Portare nella NEXT il redesign unificato del modulo `Lavori` mantenendo aggiunta, modifica, eliminazione, esecuzione e dettaglio reali, senza aprire globalmente il clone.
- FILE TOCCATI:
  - `src/utils/cloneWriteBarrier.ts`
  - `src/next/NextLavoriDaEseguirePage.tsx`
  - `src/next/NextLavoriInAttesaPage.tsx`
  ...
  - `docs/product/REGISTRO_MODIFICHE_CLONE.md`
  - `CONTEXT_CLAUDE.md`
  - `docs/change-reports/2026-04-06_1215_lavori_next_ui_unificata_deroga_chirurgica.md`
  - `docs/continuity-reports/2026-04-06_1215_continuity_lavori_next_ui_unificata_deroga_chirurgica.md`
- COSA E STATO CAMBIATO:
  - creata una dashboard `Lavori` unificata con topbar a tab, stat card, filtri, tabelle e dettaglio in modale;
  - le route `/next/lavori-in-attesa` e `/next/lavori-eseguiti` convergono sulla stessa dashboard, forzando solo la tab iniziale;
  - il dettaglio lavoro usa ora un solo componente reale condiviso, montabile sia come modale sia come route diretta;
  - `Aggiungi` torna a usare davvero `getItemSync("@lavori")` e `setItemSync("@lavori", ...)` per salvare il gruppo;
  - modifica, eliminazione e `Segna come eseguito` tornano reali nel dettaglio.
- IMPATTO SU UI / LETTURA / BLOCCO SCRITTURE:
  - UI: il modulo non usa piu le superfici clone-safe read-only precedenti e adotta la shell dashboard richiesta dalla spec;
  - Lettura: il runtime ufficiale usa `@lavori` e `@mezzi_aziendali` tramite `storageSync`, non un reader clone-safe impoverito;
  - Blocco scritture: la barriera clone resta attiva ovunque, tranne per `storageSync.setItemSync("@lavori")` sui soli pathname Lavori/dettaglio.
- COME VERIFICARE:
  - `node_modules\\.bin\\eslint.cmd src\\utils\\cloneWriteBarrier.ts src\\next\\NextLavoriDaEseguirePage.tsx src\\next\\NextLavoriInAttesaPage.tsx src\\next\\NextLavoriEseguitiPage.tsx src\\next\\NextDettaglioLavoroPage.tsx`
  - `npm run build`
  - aprire `/next/lavori-da-eseguire?tab=aggiungi`, aggiungere una voce e salvare il gruppo;
  - aprire `/next/lavori-in-attesa`, verificare i dati reali e aprire il dettaglio in modale;
  - dal dettaglio verificare modifica, esecuzione e delete reali;
  - aprire `/next/dettagliolavori/:lavoroId` e verificare il dettaglio diretto;
  - aprire `/next/autisti-inbox` e verificare che un tentativo di write su `@lavori` resti bloccato.
- SE E CANDIDABILE A ESSERE PORTATO NELLA MADRE IN FUTURO: `DA VERIFICARE`; la UI dashboard e nuova, mentre la logica reale resta quella legacy di Lavori.
- NOTE:
  - runtime locale verificato con record temporaneo reale creato e poi rimosso;
  - il modulo viene lasciato `PARZIALE` finche non passa audit separato post-redesign.

### Voce 2026-04-06 133
- DATA: 2026-04-06
- TITOLO MODIFICA: `Lavori` NEXT dettaglio con problema reale e apertura segnalazione autista originale
- OBIETTIVO: Arricchire il dettaglio lavoro NEXT per mostrare la descrizione reale del problema segnalato dall'autista e aprire la segnalazione originale in modale read-only, senza uscire dal modulo `Lavori`.
- FILE TOCCATI:
  - `src/next/NextDettaglioLavoroPage.tsx`
  - `src/next/next-lavori.css`
  - `docs/product/STATO_MIGRAZIONE_NEXT.md`
  - `docs/product/REGISTRO_MODIFICHE_CLONE.md`
  - `CONTEXT_CLAUDE.md`
  - `docs/change-reports/2026-04-06_2044_lavori_dettaglio_segnalazione_origine.md`
  - `docs/continuity-reports/2026-04-06_2044_continuity_lavori_dettaglio_segnalazione_origine.md`
- COSA E STATO CAMBIATO:
  - aggiunta la sezione `Problema segnalato` nel dettaglio lavoro;
  - aggiunto il recupero read-only delle segnalazioni autisti reali via reader NEXT esistente;
  - aggiunto il bottone discreto `Apri segnalazione` e il modale read-only con dati reali della segnalazione originale;
  - introdotto matching stretto con priorita a `source.id/originId` e fallback univoco su targa + autore + descrizione.
- IMPATTO SU UI / LETTURA / BLOCCO SCRITTURE:
  - UI: il dettaglio mostra piu contesto reale senza cambiare route o flussi principali del modulo;
  - Lettura: solo lettura di segnalazioni autisti tramite `readNextAutistiReadOnlySnapshot(...)`;
  - Scritture: nessuna nuova scrittura o mutazione introdotta dal modale segnalazione.
- COME VERIFICARE:
  - `node_modules\\.bin\\eslint.cmd src\\next\\NextDettaglioLavoroPage.tsx`
  - `npm run build`
  - aprire `/next/lavori-in-attesa`, entrare nel dettaglio di un lavoro nato da segnalazione e verificare `Problema segnalato`;
  - cliccare `Apri segnalazione` e verificare autore, data/ora, mezzo e descrizione reale;
  - aprire `/next/dettagliolavori/:lavoroId` e verificare la stessa resa su route diretta;
  - verificare che un lavoro senza match sicuro non apra una segnalazione sbagliata.
- SE E CANDIDABILE A ESSERE PORTATO NELLA MADRE IN FUTURO: `DA VERIFICARE`; il pattern e utile, ma va validato contro la UI legacy del dettaglio lavori.
- NOTE:
...
### Voce 2026-04-06 134
- DATA: 2026-04-06
- TITOLO MODIFICA: Fix reale del testo segnalazione autista nel dettaglio `Lavori`
- OBIETTIVO: Correggere il blocco `Problema segnalato` del dettaglio lavoro NEXT, che a runtime restava `—` anche quando la segnalazione autista originale conteneva un testo reale.
- FILE TOCCATI:
  - `src/next/NextDettaglioLavoroPage.tsx`
  - `docs/product/STATO_MIGRAZIONE_NEXT.md`
  - `docs/product/REGISTRO_MODIFICHE_CLONE.md`
  - `CONTEXT_CLAUDE.md`
  - `docs/change-reports/2026-04-06_2102_lavori_fix_testo_reale_segnalazione.md`
  - `docs/continuity-reports/2026-04-06_2102_continuity_lavori_fix_testo_reale_segnalazione.md`
- COSA E STATO CAMBIATO:
  - il dettaglio lavoro legge ora il payload reale di `@segnalazioni_autisti_tmp`;
  - il resolver usa prima `source.id/originId`, poi `linkedLavoroId/linkedLavoroIds`, poi solo fallback stretto e univoco;
  - il testo del blocco `Problema segnalato` viene letto dai campi reali `descrizione`, `note`, `messaggio`, `dettaglio`, `testo` in quest'ordine.
- IMPATTO SU UI / LETTURA / BLOCCO SCRITTURE:
  - UI: il dettaglio mostra finalmente il testo vero della segnalazione quando esiste;
  - Lettura: usa il dataset reale delle segnalazioni autisti, senza nuove route o nuova logica di scrittura;
  - Scritture: nessuna mutazione nuova, il modale segnalazione resta read-only.
- COME VERIFICARE:
  - `node_modules\\.bin\\eslint.cmd src\\next\\NextDettaglioLavoroPage.tsx`
  - `npm run build`
...
- ### Voce 2026-04-15 140
- DATA: 2026-04-15
- TITOLO MODIFICA: Chiusura `Archivista V1` lato documenti / archiviazione
- OBIETTIVO: Chiudere il perimetro documentale di `Archivista` attivando tutte e sole le quattro famiglie V1, aggiungendo review coerente, controllo duplicati, conferma esplicita utente e archiviazione finale reale senza introdurre azioni business dopo l'archivio.
- FILE TOCCATI:
  - `src/next/NextIAArchivistaPage.tsx`
  - `src/next/internal-ai/ArchivistaMagazzinoBridge.tsx`
  - `src/next/internal-ai/ArchivistaManutenzioneBridge.tsx`
...
- COSA E STATO CAMBIATO:
  - `Archivista` attiva ora davvero tutte e sole le quattro famiglie V1 `Fattura / DDT + Magazzino`, `Fattura / DDT + Manutenzione`, `Documento mezzo`, `Preventivo + Magazzino`;
  - `Magazzino` viene riusato senza rifacimento e riceve solo il tratto finale di chiusura documentale: review, duplicati, conferma utente, archiviazione finale;
  - `Manutenzione` resta OpenAI-only e aggiunge controllo duplicati e archiviazione in `@documenti_mezzi`, senza creare manutenzioni e senza usare `@costiMezzo`;
  - il nuovo bridge `Documento mezzo` consente sottotipo `Libretto` / `Assicurazione` / `Revisione` / `Collaudo`, review dei campi letti, archiviazione dell'originale e update di `@mezzi_aziendali` solo su conferma esplicita;
  - il nuovo bridge `Preventivo + Magazzino` analizza, mostra review, controlla i duplicati e archivia nel target coerente `storage/@preventivi` senza listino automatico;
  - introdotto helper locale `ArchivistaArchiveClient` per upload originali, salvataggio record archivio, ricerca duplicati e update mezzo confermato;
  - il backend IA separato espone endpoint OpenAI dedicati per `documento_mezzo` e `preventivo_magazzino` con campi, righe, avvisi e campi mancanti coerenti con la review;
  - `cloneWriteBarrier.ts` apre solo le scritture minime necessarie a upload originali, `@documenti_magazzino`, `@documenti_mezzi`, `storage/@preventivi` e `@mezzi_aziendali`.
- IMPATTO SU UI / LETTURA / BLOCCO SCRITTURE:
  - UI: `Archivista` resta non chat, ma ora completa davvero il lato documenti/archivio delle quattro famiglie V1 con callout chiari `Documento analizzato`, `Non ancora archiviato`, `Documento archiviato`;
  - Lettura: review e duplicati si basano sui campi forti estratti, con controllo prudente `Stesso documento` / `Versione migliore` / `Documento diverso`;
  - Scritture: abilitate solo upload originali e salvataggi archivio confermati; nessuna apertura per `@costiMezzo`, `@manutenzioni`, `@listino_prezzi` o azioni business post-archivio.
- COME VERIFICARE:
  ...
  - `npm run build`
  - aprire `/next/ia/archivista` e verificare che siano operative solo le quattro famiglie V1;
  - per ciascuna famiglia: analizzare un file, eseguire controllo duplicati, scegliere una delle tre opzioni quando il match e forte e confermare l'archiviazione;
  - per `Documento mezzo`: verificare che l'update del mezzo avvenga solo se selezionato esplicitamente.
- SE E CANDIDABILE A ESSERE PORTATO NELLA MADRE IN FUTURO: `DA VERIFICARE`
- ### Voce 2026-04-12 140
- DATA: 2026-04-12
...
### Voce 2026-04-07 135
- DATA: 2026-04-07
- TITOLO MODIFICA: Irrigidimento del link forte segnalazione -> dettaglio `Lavori`
- OBIETTIVO: Allineare il dettaglio lavoro NEXT al flusso dati reale verificato, evitando fallback fragili o scorciatoie su `lavoro.dettagli` quando si deve mostrare la segnalazione autista origine.
- FILE TOCCATI:
  - `src/next/NextDettaglioLavoroPage.tsx`
  - `docs/product/STATO_MIGRAZIONE_NEXT.md`
  - `docs/product/REGISTRO_MODIFICHE_CLONE.md`
  - `CONTEXT_CLAUDE.md`
  - `docs/change-reports/2026-04-07_0842_lavori_fix_link_forte_segnalazione.md`
  - `docs/continuity-reports/2026-04-07_0842_continuity_lavori_fix_link_forte_segnalazione.md`
- COSA E STATO CAMBIATO:
  - il dettaglio usa ancora `source.id/originId` e il backlink `linkedLavoroId/linkedLavoroIds` come collegamenti forti;
  - il testo del blocco `Problema segnalato` viene letto solo dal record segnalazione matchato, nell'ordine `descrizione`, `note`, `messaggio`, `dettaglio`, `testo`;
  - rimosso l'uso di `lavoro.dettagli/lavoro.note` come scorciatoia per il testo origine;
  - messaggio allineato a `Nessuna descrizione presente nella segnalazione originale` quando il record origine non contiene testo.
- IMPATTO SU UI / LETTURA / BLOCCO SCRITTURE:
  - UI: il dettaglio mostra il testo reale della segnalazione senza appoggiarsi a campi del lavoro non canonici;
  - Lettura: resta confinata a `@lavori` + `@segnalazioni_autisti_tmp`, senza nuove route o nuovi domain obbligatori;
  - Scritture: nessuna nuova mutazione, il modale segnalazione resta read-only.
- COME VERIFICARE:
  - `node_modules\\.bin\\eslint.cmd src\\next\\NextDettaglioLavoroPage.tsx`
  - `npm run build`
  - aprire `/next/lavori-in-attesa`, dettaglio del lavoro `7c6af494-9b02-4bf2-ac67-c994b39436c0` e verificare `Problema segnalato = Freni da controllare`
  - cliccare `Apri segnalazione` e verificare che il modale mostri `Freni da controllare`
  - aprire `/next/dettagliolavori/7c6af494-9b02-4bf2-ac67-c994b39436c0?from=lavori-in-attesa` e verificare la stessa resa
  - aprire `/next/dettagliolavori/daade4a2-c681-46d0-99d4-1906d151116d?from=lavori-in-attesa` e verificare che non compaia `Apri segnalazione`.
- SE E CANDIDABILE A ESSERE PORTATO NELLA MADRE IN FUTURO: `DA VERIFICARE`
- NOTE:
  - nessun file fuori whitelist toccato;
...
- ### Voce 2026-04-09 138
- DATA: 2026-04-09
- TITOLO MODIFICA: Correzione metriche `Compressore` nel `Quadro manutenzioni PDF` NEXT
- OBIETTIVO: Impedire che il filtro `Compressore` continui a mostrare/derivare `km` nel quadro PDF e usare invece `ore`, senza rompere i rami `Mezzo` e `Attrezzature`.
- FILE TOCCATI:
  - `src/next/NextManutenzioniPage.tsx`
  - `docs/STATO_ATTUALE_PROGETTO.md`
  - `docs/product/STATO_MIGRAZIONE_NEXT.md`
  - `docs/product/REGISTRO_MODIFICHE_CLONE.md`
  - `CONTEXT_CLAUDE.md`
  - `docs/change-reports/20260409_162049_manutenzioni_next_quadro_pdf_compressore_ore.md`
  - `docs/continuity-reports/20260409_162049_continuity_manutenzioni_next_quadro_pdf_compressore_ore.md`
- COSA E STATO CAMBIATO:
  - il pannello risultati del `Quadro manutenzioni PDF` non usa piu un blocco metrica fisso `Km`;
  - il ramo `Compressore` mostra ora `Ore attuali`, `Ore intervento` e `Δ ore` solo quando i dati sono davvero calcolabili;
  - il ramo `Mezzo` resta su `km`;
  - il ramo `Attrezzature` non forza piu `km/ore` come default e mostra solo la misura presente nel record;
  - l'export PDF locale usa ora una colonna misura coerente con il filtro attivo (`Km`, `Ore`, `Misura`).
- IMPATTO SU UI / LETTURA / BLOCCO SCRITTURE:
  - UI: il quadro rende il ramo compressore coerente e non rumoroso;
  - Lettura: nessuna nuova fonte dati, solo uso corretto dei campi `ore` gia presenti;
  - Scritture: nessuna variazione.
- COME VERIFICARE:
  - `npx eslint src/next/NextManutenzioniPage.tsx src/next/domain/nextManutenzioniDomain.ts src/next/domain/nextManutenzioniGommeDomain.ts`
  - `npm run build`
  - aprire `/next/manutenzioni` -> `Quadro manutenzioni PDF` -> filtro `Compressore` e verificare che non compaiano etichette `Km`;
  - esportare un PDF con filtro `Compressore` e verificare intestazioni/valori coerenti in `ore`.
- SE E CANDIDABILE A ESSERE PORTATO NELLA MADRE IN FUTURO: `DA VERIFICARE`
- ### Voce 2026-04-09 137
- DATA: 2026-04-09
- TITOLO MODIFICA: Fix cross-modulo `Manutenzioni` NEXT dopo audit `PROMPT32`
- OBIETTIVO: Correggere il bug prioritario del writer materiali su `@materialiconsegnati` e allineare `Dossier`, `Dossier Gomme` e `Operativita` ai campi gomme strutturati senza rompere la compatibilita dei record legacy.
- FILE TOCCATI:
  - `src/next/domain/nextManutenzioniDomain.ts`
  - `src/next/domain/nextManutenzioniGommeDomain.ts`
  - `src/next/NextGommeEconomiaSection.tsx`
...
- COSA E STATO CAMBIATO:
  - `nextManutenzioniDomain.ts`: il writer materiali usa ora un solo `setItemSync()` su `@materialiconsegnati` e salva record unificati compatibili con i reader materiali/movimenti e con il Dossier mezzo-centrico;
  - `nextManutenzioniGommeDomain.ts`: i campi strutturati `gommePerAsse`, `gommeInterventoTipo`, `gommeStraordinario` vengono privilegiati in lettura prima del parsing della descrizione; la vista legacy manutenzioni costruisce descrizioni coerenti per ordinario e straordinario;
  - `NextGommeEconomiaSection.tsx`: il `Dossier Gomme` mostra ora `Stato ordinario per asse` ed `Eventi straordinari`, con motivo straordinario quando disponibile;
  - `nextDossierMezzoDomain.ts` + `NextDossierMezzoPage.tsx`: il Dossier principale espone riepilogo gomme per asse e straordinari senza perdere la lista manutenzioni generale;
  - `nextOperativitaGlobaleDomain.ts`: il reader di `@manutenzioni` costruisce descrizioni piu coerenti per gli interventi gomme strutturati e aggiunge metadati dedicati.
- IMPATTO SU UI / LETTURA / BLOCCO SCRITTURE:
  - UI: Dossier principale e Dossier Gomme rendono finalmente visibili i nuovi campi gomme strutturati;
  - Lettura: `Operativita` e i reader dossier/manutenzioni convergono meglio sui record nuovi, mantenendo fallback prudente per quelli vecchi;
  - Scritture: nessun widening del clone; la sola scrittura modificata e il flusso gia autorizzato di `Manutenzioni`, reso coerente e non sovrascrivente su `@materialiconsegnati`.
- COME VERIFICARE:
  - `npx eslint src/next/domain/nextManutenzioniDomain.ts src/next/domain/nextManutenzioniGommeDomain.ts src/next/NextGommeEconomiaSection.tsx src/next/NextDossierMezzoPage.tsx src/next/domain/nextDossierMezzoDomain.ts src/next/domain/nextOperativitaGlobaleDomain.ts`
  - `npm run build`
  - aprire `/next/manutenzioni`, registrare una manutenzione con materiale da inventario e verificare che il dataset `@materialiconsegnati` non perda il record appena scritto;
  - aprire `/next/dossier-mezzo/<targa>` e verificare la presenza di `Stato gomme per asse` ed `Eventi gomme straordinari`;
  - aprire `/next/dossier-gomme/<targa>` e verificare che l'ordinario per asse non dipenda piu solo dal parsing descrizione.
- SE E CANDIDABILE A ESSERE PORTATO NELLA MADRE IN FUTURO: `DA VERIFICARE`

- ### Voce 2026-04-15 139
- DATA: 2026-04-15
- TITOLO MODIFICA: Fix OpenAI-only del ramo `Archivista` -> `Fattura / DDT + Manutenzione`
- OBIETTIVO: Eliminare dal ramo Manutenzione ogni chiamata a `estrazioneDocumenti` e spostare la sola review documentale su backend OpenAI server-side dedicato, senza toccare Magazzino, writer o barrier.
- FILE TOCCATI:
  - `src/next/internal-ai/ArchivistaManutenzioneBridge.tsx`
  - `backend/internal-ai/server/internal-ai-adapter.js`
  - `backend/internal-ai/server/internal-ai-document-extraction.js`
...
- COSA E STATO CAMBIATO:
  - il bridge `ArchivistaManutenzioneBridge` non punta piu alla cloud function `estrazioneDocumenti`;
  - il backend IA separato espone ora un endpoint dedicato `documents.manutenzione-analyze`;
  - l'estrazione documentale supporta un profilo `manutenzione` con prompt/schema OpenAI dedicati e provider obbligatorio;
  - la review frontend riceve riassunto breve, targa, fornitore, data, totale, righe, avvisi e campi mancanti senza cambiare il prodotto visibile.
- IMPATTO SU UI / LETTURA / BLOCCO SCRITTURE:
  - UI: invariata o quasi, con stessa review manutenzione gia costruita;
  - Lettura: il ramo Manutenzione usa ora solo backend OpenAI server-side;
  - Scritture: nessuna variazione, nessun writer business nuovo.
- COME VERIFICARE:
  - `npx eslint src/next/internal-ai/ArchivistaManutenzioneBridge.tsx backend/internal-ai/server/internal-ai-adapter.js backend/internal-ai/server/internal-ai-document-extraction.js`
  - `npm run build`
  - aprire `/next/ia/archivista`, scegliere `Fattura / DDT` + `Manutenzione`, caricare un documento officina e verificare che la review mostri i dati estratti senza passare da `estrazioneDocumenti`.
- SE E CANDIDABILE A ESSERE PORTATO NELLA MADRE IN FUTURO: `DA VERIFICARE`

### Voce 2026-04-22 2229
- DATA: 2026-04-22
- TITOLO MODIFICA: Implementazione `Preventivo manuale` NEXT in `Prezzi & Preventivi`
- OBIETTIVO: Permettere dal tab `/next/materiali-da-ordinare?tab=preventivi` il salvataggio di preventivi manuali reali con foto opzionali e aggiornamento contestuale di `@listino_prezzi`, senza toccare la madre e senza allargare il barrier oltre il perimetro autorizzato.
- FILE TOCCATI:
  - `src/utils/cloneWriteBarrier.ts`
  - `src/next/NextProcurementConvergedSection.tsx`
  - `src/next/NextMaterialiDaOrdinarePage.tsx`
  ...
  - `docs/product/REGISTRO_MODIFICHE_CLONE.md`
  - `CONTEXT_CLAUDE.md`
  - `docs/change-reports/2026-04-22_1443_preventivo_manuale_next.md`
  - `docs/continuity-reports/2026-04-22_1443_continuity_preventivo_manuale_next.md`
- COSA E STATO CAMBIATO:
  - aggiunto il pulsante `PREVENTIVO MANUALE` nel branch NEXT `Prezzi & Preventivi` e montato il nuovo modale `NextPreventivoManualeModal`;
  - creato `nextPreventivoManualeWriter.ts` con replica byte-per-byte delle 8 funzioni madre richieste dalla spec v2 e con i tre ingressi di alto livello `saveNextPreventivoManuale`, `upsertListinoFromPreventivoManuale`, `saveAndUpsert`;
  - il writer salva il record `Preventivo` reale in `storage/@preventivi`, carica le foto opzionali in `preventivi/manuali/` e aggiorna il listino reale in `storage/@listino_prezzi` usando la logica semplificata autorizzata dalla spec;
  - `NextMaterialiDaOrdinarePage.tsx` passa ora `onPreventivoSaved` al tab convergente per ricaricare lo snapshot procurement dopo il salvataggio;
  - `cloneWriteBarrier.ts` autorizza ora sul solo pathname `/next/materiali-da-ordinare` esclusivamente `firestore.setDoc` per `storage/@preventivi` / `storage/@listino_prezzi` e `storage.uploadBytes` per `preventivi/manuali/`.
- IMPATTO SU UI / LETTURA / BLOCCO SCRITTURE:
  - UI: nuovo modale operativo per il preventivo manuale senza CSS nuovi, con validazione inline, multi-riga e preview foto;
  - Lettura: nessun cambio a domain o shape in lettura;
  - Scritture: riaperte in modo chirurgico solo per i due documenti Firestore condivisi e per il prefisso Storage dei preventivi manuali.
- COME VERIFICARE:
  - `npx eslint src/next/NextPreventivoManualeModal.tsx src/next/nextPreventivoManualeWriter.ts src/next/NextProcurementConvergedSection.tsx src/next/NextMaterialiDaOrdinarePage.tsx src/utils/cloneWriteBarrier.ts`
  - `npm run build`
  - aprire `/next/materiali-da-ordinare?tab=preventivi`
  - verificare `PREVENTIVO MANUALE` accanto a `CARICA PREVENTIVO`
  - aprire il modale, controllare default `Numero preventivo` / `Data preventivo` / `Valuta`
  - verificare la validazione inline senza salvare dati invalidi
  - su dataset di test, salvare un preventivo manuale e verificare presenza in `storage/@preventivi`, update coerente di `storage/@listino_prezzi` e assenza di `[CLONE_NO_WRITE]`
- SE E CANDIDABILE A ESSERE PORTATO NELLA MADRE IN FUTURO: `DA VERIFICARE`
- NOTE:
  - applicata l'unica deviazione autorizzata della spec v2: la `valuta` NON viene persistita dentro `Preventivo`, ma viene passata come parametro esplicito al solo `upsertListinoFromPreventivoManuale(preventivo, valuta, codiciArticoloPerRiga)`;
  - nessun campo nuovo aggiunto a `Preventivo`, `PreventivoRiga` o `ListinoVoce`;
  - runtime browser completo non certificato in questa patch per indisponibilita del backend browser MCP in sessione.

### Voce 2026-04-22 1506
- DATA: 2026-04-22
- TITOLO MODIFICA: Fix refresh listino NEXT dopo salvataggio preventivo manuale
- OBIETTIVO: Chiudere il bug runtime per cui il tab NEXT `Listino Prezzi` non rileggeva subito lo snapshot procurement dopo `onPreventivoSaved`, pur con scritture gia presenti in `storage/@preventivi` e `storage/@listino_prezzi`.
- FILE TOCCATI:
  - `src/next/NextMaterialiDaOrdinarePage.tsx`
  - `docs/product/REGISTRO_MODIFICHE_CLONE.md`
  - `CONTEXT_CLAUDE.md`
- COSA E STATO CAMBIATO:
  - la causa primaria era `H1`: il modale `NextPreventivoManualeModal` aspettava `onSaved`, ma il parent `NextMaterialiDaOrdinarePage.tsx` gli passava solo `setProcurementRefreshCounter(...)`, quindi la Promise della callback si chiudeva prima della rilettura reale di `readNextProcurementSnapshot(...)`;
  - il parent ora espone `refreshProcurementSnapshot` come `useCallback` async reale, lo usa sia al mount sia come `onPreventivoSaved`, e il modale aspetta quindi il completamento del reload vero dello snapshot prima di chiudersi;
  - la reidratazione della bozza locale procurement non dipende piu dal trigger di refresh procurement e torna a girare solo al mount, evitando side-effect locali sul refresh post-salvataggio.
- IMPATTO SU UI / LETTURA / BLOCCO SCRITTURE:
  - UI: il tab NEXT `Listino Prezzi` deve mostrare subito la nuova voce senza reload manuale pagina;
  - Lettura: nessuna nuova fonte dati, solo riuso corretto del loader snapshot gia esistente;
  - Scritture: nessun cambio a writer, barrier o shape persistite.
- COME VERIFICARE:
  - `npx eslint src/next/NextMaterialiDaOrdinarePage.tsx`
  - `npm run build`
  - aprire `/next/materiali-da-ordinare?tab=preventivi`
  - salvare un `Preventivo manuale`
  - passare a `/next/materiali-da-ordinare?tab=listino` e verificare che la voce appaia immediatamente senza reload pagina.
- SE E CANDIDABILE A ESSERE PORTATO NELLA MADRE IN FUTURO: `NO`
- NOTE:
  - fix chirurgico limitato al refresh UI NEXT;
  - nessun change a `src/pages/Acquisti.tsx`, `cloneWriteBarrier.ts`, writer o shape dati.

### Voce 2026-04-22 1915
- DATA: 2026-04-22
- TITOLO: Riorganizzazione doc + compressione storico + policy AGENTS
- FILE TOCCATI: `docs/_live/**` (nuovi), `AGENTS.md`, `CONTEXT_CLAUDE.md`, `git mv` di `SPEC/STATO/REGISTRO/architecture/data/flow-master`
- COSA: Creati compressi `STORICO_PATCH` e `STORICO_AUDIT`; spostati i file LIVE in `docs/_live/`; aggiunta policy documentale in `AGENTS.md`. File originali report/audit/mirror NON cancellati, restano in attesa di verifica utente.
- ESITO: FATTO_CON_VERIFICA_MANCANTE
- NOTE: Utente deve verificare qualita dei due file compressi prima della cancellazione degli originali. Le cancellazioni saranno in un prompt successivo.

### Voce 2026-04-22 1600
- DATA: 2026-04-22
- TITOLO MODIFICA: Fix leak preventivi manutenzione nel layer procurement NEXT
- OBIETTIVO: Escludere i record `ambitoPreventivo: "manutenzione"` dalle pipeline di lettura procurement e dal registry IA universale, eliminando il leak che mostrava i preventivi Archivista Manutenzione in "Materiali da ordinare" e "Magazzino documenti-costi".
- FILE TOCCATI:
  - `src/next/domain/nextProcurementDomain.ts`
  - `src/next/domain/nextDocumentiCostiDomain.ts`
  - `src/next/internal-ai/internalAiUnifiedIntelligenceEngine.ts`
- COSA E STATO CAMBIATO:
  - `nextProcurementDomain.ts`: aggiunto filtro `.filter()` prima del `.map()` in `readNextProcurementSnapshot()` — esclude i record in cui `ambitoPreventivo === "manutenzione"` (uguaglianza stretta); record senza il campo restano inclusi per retrocompatibilita;
  - `nextDocumentiCostiDomain.ts`: estesa la condizione del filtro `preventiviItems` in `readNextDocumentiCostiProcurementSupportSnapshot()` con la stessa condizione di esclusione;
  - `internalAiUnifiedIntelligenceEngine.ts`: nel loop che materializza i descrittori `raw_storage_doc`, aggiunto filtro speciale per `sourceId === "storage/@preventivi"` che esclude i record `ambitoPreventivo: "manutenzione"` prima di passare a `snapshotFromRawStorage()`.
- IMPATTO SU UI / LETTURA / BLOCCO SCRITTURE:
  - UI: i preventivi manutenzione non compaiono piu in "Materiali da ordinare" (tab Prezzi & Preventivi) ne in "Magazzino documenti-costi";
  - Lettura: tre pipeline di lettura ristrettive senza cambiamento ai shape Firestore;
  - Scritture: nessun cambio a writer, barrier o regole storage.
- COME VERIFICARE:
  - `npm run build` -> OK (gia verificato)
  - `npm run lint` -> 582/567/15 (gia verificato, delta zero)
  - archiviare un preventivo Manutenzione e verificare assenza in `/next/materiali-da-ordinare?tab=preventivi`
  - verificare assenza in `/next/magazzino?tab=documenti-costi`
- SE E CANDIDABILE A ESSERE PORTATO NELLA MADRE IN FUTURO: `NO`
- NOTE:
