...
- la macro-area `/next/mezzi-dossier` non e piu solo placeholder generico: ospita ora sia l'elenco mezzi NEXT `read-only` basato su `D01` sia un primo Dossier Mezzo read-only che combina `D01` con una convergenza minima `D02` su `@lavori` e `@manutenzioni` e un primo layer `D04` a `canonico ridotto` su `@rifornimenti.items`, sempre senza writer.
- `D04 Rifornimenti e consumi` entra nella NEXT dal `2026-03-08` solo come layer di normalizzazione dedicato e confinato: legge esclusivamente `storage/@rifornimenti.items`, espone un modello pulito interno alla NEXT e non usa `@rifornimenti_autisti_tmp`, `value.items` o merge reader-side. Il blocco resta volutamente ridotto: `data` e usata solo come label display; `km` e `costo` restano opzionali/non garantiti.
...
| Dossier Mezzo | IMPORTATO READ-ONLY | RIUSO LOGICA + UI NUOVA | si | no | NEXT | `src/next/NextDossierMezzoPage.tsx`; `src/next/nextAnagraficheFlottaDomain.ts`; `src/next/nextOperativitaTecnicaDomain.ts`; `src/next/nextRifornimentiConsumiDomain.ts`; `src/App.tsx`; `src/next/next-shell.css`; `docs/data/DOMINI_DATI_CANONICI.md`; `docs/data/CHECK_REALE_RIFORNIMENTI_ITEMS.md` | `/next/mezzi-dossier/:targa` ospita ora un Dossier Mezzo NEXT `read-only` a tre ingressi controllati: identita mezzo su `D01`, primo blocco tecnico reale su `D02` e primo blocco rifornimenti `D04` tramite layer NEXT dedicato. Il blocco rifornimenti e volutamente `canonico ridotto`: legge solo `storage/@rifornimenti.items`, espone `id`, `mezzoTarga`, `data`, `litri`, `distributore`, `note` e marca `km` e `costo` come opzionali/non garantiti. Nessun clone del dossier legacy, nessun writer, nessuna lettura `tmp`, nessun fallback `value.items` e nessun merge reader-side. | 2026-03-08 | N/A - patch locale next d04 canonico ridotto |
