...
- la macro-area `/next/mezzi-dossier` non e piu solo placeholder generico: ospita ora sia l'elenco mezzi NEXT `read-only` basato su `D01` sia un primo Dossier Mezzo read-only che combina `D01` con una convergenza minima `D02` su `@lavori` e `@manutenzioni`, sempre senza writer.
- verifica `D04 Rifornimenti e consumi` eseguita il `2026-03-08`: nessun blocco rifornimenti aggiunto nella NEXT; il dataset business target `@rifornimenti.items` non e ancora allineato in modo dimostrabile al contratto runtime richiesto (`timestamp`, `source`, `validation`), quindi il dominio resta fuori dal Dossier NEXT.
...
| Dossier Mezzo | IMPORTATO READ-ONLY | RIUSO LOGICA + UI NUOVA | si | no | NEXT | `src/next/NextDossierMezzoPage.tsx`; `src/next/nextAnagraficheFlottaDomain.ts`; `src/next/nextOperativitaTecnicaDomain.ts`; `src/App.tsx`; `src/next/next-shell.css`; `docs/data/DOMINI_DATI_CANONICI.md` | `/next/mezzi-dossier/:targa` ospita ora un Dossier Mezzo NEXT `read-only` a due livelli: identita mezzo su `D01` e primo blocco tecnico reale su `D02`, letto da `@lavori` e `@manutenzioni` con filtro per `targa`. Il blocco tecnico resta volutamente minimo: backlog lavori, lavori chiusi e manutenzioni essenziali. Nessun clone del dossier legacy, nessun writer, nessun collegamento a materiali/costi e nessuna importazione di domini oltre il perimetro verificato. Verifica `D04` del 2026-03-08: nessun reader NEXT aggiunto per rifornimenti; `@rifornimenti.items` non garantisce ancora in modo dimostrabile il contratto runtime richiesto (`timestamp` numerico, `source`, `validation`) e la NEXT non puo usare fallback su `value.items` o `@rifornimenti_autisti_tmp`. | 2026-03-08 | N/A - verifica D04 non importabile |
