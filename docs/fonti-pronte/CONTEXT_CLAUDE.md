...
## 3. STATO ATTUALE
- Ultimo task completato: chiusura `Archivista V1` lato documenti / archiviazione il `2026-04-15`. `src/next/NextIAArchivistaPage.tsx` rende ora operative tutte e sole le quattro famiglie V1 `Fattura / DDT + Magazzino`, `Fattura / DDT + Manutenzione`, `Documento mezzo`, `Preventivo + Magazzino`; i bridge Archivista applicano review non chat, controllo duplicati obbligatorio con scelta utente `Stesso documento / Versione migliore / Documento diverso`, conferma finale e archiviazione vera. I target reali usati sono `@documenti_magazzino`, `@documenti_mezzi`, `storage/@preventivi`, con update `@mezzi_aziendali` solo nel ramo `Documento mezzo` e solo su conferma esplicita. `backend/internal-ai/server/internal-ai-adapter.js` e `internal-ai-document-extraction.js` espongono ora anche gli endpoint/profili OpenAI dedicati `documents.documento-mezzo-analyze` e `documents.preventivo-magazzino-analyze`, oltre al ramo `manutenzione` gia corretto; `src/utils/cloneWriteBarrier.ts` apre solo le scritture minime richieste da Archivista V1 su upload originali e target archivio confermati, senza toccare `@manutenzioni`, `@costiMezzo` o listino. Verifiche tecniche: `npx eslint` sui file runtime/backend toccati `OK` con warning noto solo sul CSS ignorato dalla config, `npm run build` `OK`. Stato onesto: Archivista V1 lato documenti/archivio `FATTO`; IA Report e azioni business dopo archivio restano `NON FATTE`.
