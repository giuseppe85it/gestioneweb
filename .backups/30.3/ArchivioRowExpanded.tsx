...
export function ArchivioRowExpanded({
  record,
  onOpenEventoModal,
}: Props): ReactElement {
  ...
  const handleOpenDettaglio = (e: React.MouseEvent): void => {
    e.stopPropagation();
    switch (record.kind) {
      case "lavoro": {
        navigate(buildNextDettaglioLavoroPath({ lavoroId: record.data.id }));
        return;
      }
      case "manutenzione": {
        navigate(
          `/next/manutenzioni?recordId=${encodeURIComponent(record.data.id)}`,
        );
        return;
      }
      case "segnalazione": {
  ...
  const disabled: boolean =
    (record.kind === "segnalazione" || record.kind === "richiesta") &&
    !onOpenEventoModal;

  return (
    <div className="archivio-row-extra">
      {body}
      <div className="archivio-row-extra-foot">
        <button
          type="button"
          className="archivio-open-dettaglio"
          onClick={handleOpenDettaglio}
          disabled={disabled}
        >
          Apri dettaglio →
        </button>
