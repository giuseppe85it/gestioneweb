...
const CHAT_TOOL_USE_LIMITS = Object.freeze({
  maxIterations: 4,
...
function normalizeToolUseText(value, fallback = "") {
...
function normalizeToolUseTools(tools) {
  ...
  return tools
    .map((tool) => {
      ...
      const name = normalizeToolUseText(tool.name);
      const description = normalizeToolUseText(tool.description);
...
function normalizeToolUseMessages(messages) {
  ...
  return messages
    .map((message) => {
      ...
      const content = normalizeToolUseText(message.content);
...
function stringifyToolUseFunctionOutput(toolResult) {
  try {
    return JSON.stringify(toolResult);
  } catch {
    return JSON.stringify({
      ok: false,
      error: "Risultato tool non serializzabile.",
    });
  }
}
...
function normalizeToolUseFunctionCallOutputs(toolResults) {
  ...
  return toolResults
    .map((toolResult) => {
      ...
      const callId = normalizeToolUseText(toolResult.toolCallId);
      if (!callId) return null;
      return {
        type: "function_call_output",
        call_id: callId,
        output: stringifyToolUseFunctionOutput(toolResult),
      };
    })
    .filter(Boolean);
}

function buildToolUseInitialInput(body) {
  const messages = normalizeToolUseMessages(body.messages);
  ...
  return [
    {
      role: "user",
      content: normalizeToolUseText(body.prompt),
    },
  ];
}

function parseToolUseArguments(rawValue) {
...
function extractToolUseCalls(response) {
  const outputItems = Array.isArray(response?.output) ? response.output : [];
  return outputItems
    .filter((item) => item?.type === "function_call" || item?.type === "tool_call")
    .slice(0, CHAT_TOOL_USE_LIMITS.maxToolCallsPerTurn)
    .map((item, index) => ({
      id:
        normalizeToolUseText(item.call_id) ||
        normalizeToolUseText(item.callId) ||
        normalizeToolUseText(item.id) ||
        `tool-call-${index + 1}`,
      name: normalizeToolUseText(item.name),
      arguments: parseToolUseArguments(item.arguments ?? item.input ?? item.input_json),
    }))
    .filter((call) => call.name);
}

function extractToolUseOutputText(response) {
  const directText = normalizeToolUseText(response?.output_text);
  ...
  for (const item of outputItems) {
    ...
    for (const contentItem of contentItems) {
      const text =
        normalizeToolUseText(contentItem?.text) ||
        normalizeToolUseText(contentItem?.output_text);
      if (text) return text;
    }
  }
...
function parseToolUseFinalMessageJson(rawText) {
...
function buildToolUseCostEstimate(usage) {
  if (!usage) {
    return null;
  }

  const inputUsd =
    usage.inputTokens === null ? null : Number(((usage.inputTokens / 1_000_000) * 0.15).toFixed(6));
  const outputUsd =
    usage.outputTokens === null ? null : Number(((usage.outputTokens / 1_000_000) * 0.6).toFixed(6));
  const totalUsd =
    inputUsd === null || outputUsd === null ? null : Number((inputUsd + outputUsd).toFixed(6));
  ...
  return {
    ...
    outputUsd,
    totalUsd,
    pricingVersion: "gpt-4o-mini-2026-04-diagnostic-estimate",
  };
}

function isToolUseRequestBody(body) {
  return (
    isPlainObject(body) &&
    body.operation === "run_tool_use_turn" &&
    typeof body.requestId === "string" &&
    typeof body.sessionId === "string" &&
    typeof body.prompt === "string" &&
    Number.isInteger(body.iteration) &&
    body.iteration >= 0 &&
    body.iteration <= CHAT_TOOL_USE_LIMITS.maxIterations &&
...
app.post("/internal-ai-backend/documents/documento-cisterna-analyze", async (req, res) => {
  const fileName = typeof req.body?.fileName === "string" ? req.body.fileName.trim() : "";
  const mimeType =
    typeof req.body?.mimeType === "string" && req.body.mimeType.trim()
      ? req.body.mimeType.trim()
  ...
  const requestedProfile =
    typeof req.body?.profile === "string" && req.body.profile.trim()
      ? req.body.profile.trim()
      : "documento_cisterna";

  if (requestedProfile !== "documento_cisterna") {
    sendEnvelope(res, {
      httpStatus: 400,
      ok: false,
      endpointId: "documents.documento-cisterna-analyze",
      status: "validation_error",
      message: "Profilo non valido per documento-cisterna-analyze.",
      data: { profile: requestedProfile },
    });
    return;
  }
  ...
  if (!fileName || !contentBase64) {
    sendEnvelope(res, {
      ...
      endpointId: "documents.documento-cisterna-analyze",
      status: "validation_error",
      message: "File o contenuto documento non validi per la review Cisterna.",
      data: { fileName },
    });
    return;
  }
  ...
  if (!isSupportedMime) {
    sendEnvelope(res, {
      ...
      endpointId: "documents.documento-cisterna-analyze",
      status: "validation_error",
      message: "Formato non supportato per documento Cisterna. Usa PDF o immagine.",
      data: {
        fileName,
        mimeType,
      },
  ...
  if (!isProviderConfigured()) {
    sendEnvelope(res, {
      ...
      status: "provider_not_configured",
      message:
        "Backend OpenAI non configurato. Imposta OPENAI_API_KEY lato server per analizzare i documenti Cisterna.",
      data: {
        providerConfigured: false,
        providerTarget: getProviderTarget(),
      },
  ...
  try {
    const providerClient = getProviderClient();
    const providerTarget = getProviderTarget();
    const result = await extractInternalAiDocumentAnalysis({
      fileName,
      mimeType,
      contentBase64,
      providerClient,
    ...
    const traceEntry = await appendTraceabilityEntry(
      buildTraceabilityEntry({
        ...
        actorId: req.body?.actorId,
        requestId: req.body?.requestId,
        note: `Review documento Cisterna OpenAI per ${fileName}.`,
      }),
    );
    ...
    sendEnvelope(res, {
      ...
      endpointId: "documents.documento-cisterna-analyze",
      status: "ok",
      message: "Analisi OpenAI documento Cisterna completata dal backend server-side.",
      data: {
        result,
        providerTarget,
        traceEntryId: traceEntry.id,
    ...
    sendEnvelope(res, {
      ...
      message:
        error instanceof Error
          ? `Analisi OpenAI documento Cisterna non completata: ${error.message}`
          : "Analisi OpenAI documento Cisterna non completata.",
      data: {
        providerConfigured: isProviderConfigured(),
        providerTarget: getProviderTarget(),
      },
...
app.post("/internal-ai-backend/documents/scheda-cisterna-analyze", async (req, res) => {
  const requestedProfile =
    typeof req.body?.profile === "string" && req.body.profile.trim()
      ? req.body.profile.trim()
      : "scheda_cisterna";
  ...
  if (requestedProfile !== "scheda_cisterna") {
    sendEnvelope(res, {
      httpStatus: 400,
      ok: false,
      endpointId: "documents.scheda-cisterna-analyze",
      status: "validation_error",
      message: "Profilo non valido per scheda-cisterna-analyze.",
      data: { profile: requestedProfile },
    });
    return;
  }
  ...
  const normalizedCells = cells
    .map((cell, index) => {
      ...
      const rowIndex =
        typeof cell.rowIndex === "number" && Number.isFinite(cell.rowIndex)
          ? cell.rowIndex
          : index;
      const dataBase64 =
        typeof cell.dataBase64 === "string" && cell.dataBase64.trim()
          ? cell.dataBase64.trim()
          : "";
      const targaBase64 =
        typeof cell.targaBase64 === "string" && cell.targaBase64.trim()
          ? cell.targaBase64.trim()
          : "";
      const litriBase64 =
        typeof cell.litriBase64 === "string" && cell.litriBase64.trim()
          ? cell.litriBase64.trim()
          : "";
      return { rowIndex, dataBase64, targaBase64, litriBase64 };
    })
  ...
  if (normalizedCells.length === 0) {
    sendEnvelope(res, {
      ...
      endpointId: "documents.scheda-cisterna-analyze",
      status: "validation_error",
      message: "Batch celle non valido per scheda Cisterna.",
      data: { total: 0 },
    });
    return;
  }

  const hasInvalidCell = normalizedCells.some(
    (cell) => !cell.dataBase64 || !cell.targaBase64 || !cell.litriBase64,
  );
  if (hasInvalidCell) {
    sendEnvelope(res, {
      httpStatus: 400,
      ...
      endpointId: "documents.scheda-cisterna-analyze",
      status: "validation_error",
      message: "Ogni riga deve includere immagini data, targa e litri.",
      data: { total: normalizedCells.length },
    });
    return;
  }
  ...
  if (!isProviderConfigured()) {
    sendEnvelope(res, {
      ...
      status: "provider_not_configured",
      message:
        "Backend OpenAI non configurato. Imposta OPENAI_API_KEY lato server per analizzare le schede Cisterna.",
      data: {
        providerConfigured: false,
        providerTarget: getProviderTarget(),
      },
  ...
  try {
    ...
    const traceEntry = await appendTraceabilityEntry(
      buildTraceabilityEntry({
        ...
        actorId: req.body?.actorId,
        requestId: req.body?.requestId,
        note: `Review scheda Cisterna OpenAI per ${normalizedCells.length} righe.`,
      }),
    );
    ...
    sendEnvelope(res, {
      ...
      endpointId: "documents.scheda-cisterna-analyze",
      status: "ok",
      message: "Analisi OpenAI scheda Cisterna completata dal backend server-side.",
      data: {
        result,
        providerTarget,
        traceEntryId: traceEntry.id,
    ...
    sendEnvelope(res, {
      ...
      message:
        error instanceof Error
          ? `Analisi OpenAI scheda Cisterna non completata: ${error.message}`
          : "Analisi OpenAI scheda Cisterna non completata.",
      data: {
        providerConfigured: isProviderConfigured(),
        providerTarget: getProviderTarget(),
      },
...
app.post("/internal-ai-backend/chat/tool-use", async (req, res) => {
  if (!isToolUseRequestBody(req.body)) {
    sendEnvelope(res, {
      httpStatus: 400,
      ok: false,
      ...
      status: "validation_error",
      message:
        "Operazione chat.tool-use non valida. E ammessa solo `run_tool_use_turn` con prompt, sessione, iterazione 0-4 e tool disponibili.",
      data: {
        allowedOperations: ["run_tool_use_turn"],
        maxIterations: CHAT_TOOL_USE_LIMITS.maxIterations,
      },
    });
    return;
  }
  ...
  if (!isProviderConfigured()) {
    sendEnvelope(res, {
      ...
      status: "provider_not_configured",
      message:
        "Provider reale non configurato. Impostare OPENAI_API_KEY lato server per attivare la chat tool use.",
      data: {
        providerConfigured: false,
        providerTarget: getToolUseProviderTarget(),
      },
    });
    return;
  }
  ...
  try {
    ...
    const traceEntry = await appendTraceabilityEntry(
      buildTraceabilityEntry({
        ...
        note:
          mode === "tool_calls"
            ? `Chat tool use con ${providerTarget.provider}/${providerTarget.model}: ${toolCalls.length} tool call richieste.`
            : `Chat tool use con ${providerTarget.provider}/${providerTarget.model}: risposta finale strutturata.`,
        entityCount: toolCalls.length,
      }),
    );

    console.info("[chat.tool-use] usage/cost", {
      requestId: req.body.requestId,
      iteration: req.body.iteration,
      usage,
      costEstimate,
    ...
    sendEnvelope(res, {
      ...
      message:
        mode === "tool_calls"
          ? "OpenAI ha richiesto tool calls per la chat NEXT."
          : "OpenAI ha prodotto la risposta finale tool use.",
      data:
        mode === "tool_calls"
          ? {
              mode,
    ...
    sendEnvelope(res, {
      ...
      message:
        error instanceof Error
          ? `Errore provider tool use: ${error.message}`
          : "Errore non previsto durante la chat tool use.",
      data: {
        providerConfigured: isProviderConfigured(),
        providerTarget,
      },
