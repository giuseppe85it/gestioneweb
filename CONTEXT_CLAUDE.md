...
## 3. STATO ATTUALE
- Ultimo task completato: nel runtime `/next/manutenzioni` il cambio gomme non e piu trattato come dato unico generico; il record clone-side puo salvare `gommePerAsse` con `asseId`, `dataCambio`, `kmCambio`, il form `Nuova / Modifica` costruisce ora il payload per asse e il `Quadro manutenzioni PDF` mostra per i mezzi lo stato gomme finale per asse, con filtro `Attrezzature` aggiunto.
...
- In `Manutenzioni` NEXT gli override del viewer tecnico sono ora persistiti solo clone-side nel domain `nextMappaStoricoDomain`, senza toccare writer business o madre legacy.
- In `Manutenzioni` NEXT il viewer tecnico `Sinistra/Destra` usa ora un bottone `Salva` esplicito in `Calibra`; click e drag preparano la posizione, ma la persistenza avviene solo alla conferma utente.
...
22. Il modulo `Manutenzioni` nel clone NEXT non e piu read-only: `/next/manutenzioni` scrive ora in modo compatibile su `@manutenzioni`, `@inventario` e `@materialiconsegnati`, riusa la convergenza gomme gia verificata, salva `assiCoinvolti?: string[]` e anche `gommePerAsse?: { asseId; dataCambio; kmCambio }[]` in modo clone-side retrocompatibile, e apre solo metadati visuali separati su `@mezzi_foto_viste`, `@mezzi_hotspot_mapping` e Storage `mezzi_foto/...`; stato corretto del modulo: `PARZIALE` finche non passa audit separato.
...
26. In `/next/manutenzioni` la UI resta allineata al riferimento approvato ma con perimetro piu corretto: shell esterna e tab scuri, fascia dati chiara a 5 blocchi, dashboard con 4 KPI + 4 pulsanti + `Ultimi interventi`, form grande per `Nuova / Modifica`, dettaglio a 2 card e tab finale `Quadro manutenzioni PDF` su superfici operative chiare; la riga `Data / KM-Ore / Fornitore` usa ora proporzioni desktop stabili, l'autosuggest inventario mette la descrizione materiale davanti al fornitore, la gestione foto non compare piu nel form ma solo in `Dettaglio` e le viste `Sinistra/Destra` del dettaglio usano ora una tavola tecnica reale `public/gomme/*` con highlight automatico degli assi strutturati salvati nella manutenzione.
...
35. In `/next/manutenzioni` il viewer tecnico embedded del tab `Dettaglio` distingue ora davvero la grammatica target: `assi` con glow circolare, `fanali/specchi` con capsule e `attrezzature` con marker tecnici non circolari; i marker neutrali del renderer non restano piu visibili in modalita normale e compaiono solo in `Calibra`.
36. In `/next/manutenzioni` il binding del viewer tecnico e ora esplicito: `NextManutenzioniPage` mantiene `selectedDetailRecordId`, passa a `NextMappaStoricoPage` il record selezionato e aggiorna il dettaglio da `Dashboard`, `Quadro manutenzioni PDF` e liste storico laterali, senza fallback alla "ultima manutenzione con assi".
37. In `/next/manutenzioni` il form `Nuova / Modifica` espone ora davvero 3 tipi intervento (`Mezzo`, `Compressore`, `Attrezzature`) e `src/next/domain/nextManutenzioniDomain.ts` accetta in modo retrocompatibile il nuovo valore `attrezzature`.
38. In `/next/manutenzioni` il comando `Calibra` del viewer tecnico embedded non usa piu la variante secondaria quasi invisibile su pannello chiaro: il bottone e ora reso con variante dedicata visibile nel toolbar tecnico del tab `Dettaglio`.
39. In `/next/manutenzioni` il viewer tecnico `Sinistra/Destra` mostra ora i target salvati anche in modalita normale (non-calibra): i marker con override persistito compaiono come span read-only (`man2-technical-marker--readonly`) sopra la tavola tecnica; solo in `Calibra` compaiono i marker interattivi con drag e draft non ancora confermati.
