...
import {
  extractCisternaDocumentoFromImage,
  extractCisternaDocumentoFromPdf,
  type CisternaDocumentoExtractResult,
} from "./nextCisternaIaClient";
import {
  createNextCisternaIaDocumentRecord,
  uploadNextCisternaDocumentFile,
...
function buildFormFromExtract(data: CisternaDocumentoExtractResult): DocumentoForm {
  return {
...
export default function NextCisternaIAPage() {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      const uploaded = await uploadNextCisternaDocumentFile({
        file: selectedFile,
        storagePath: path,
      });
      setStorageFileUrl(uploaded.fileUrl);
      setStoragePath(uploaded.storagePath);

      const extracted = isPdf(selectedFile)
      ...
      setError(toErrorMessage(err, "Errore durante l'analisi."));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const payload = sanitizeForFirestore({
        ...
        iaEngine: "openai-responses",
      });

      const saved = await createNextCisternaIaDocumentRecord({ payload });
      await readNextCisternaSnapshot(yearMonth, { includeCloneOverlays: false });
