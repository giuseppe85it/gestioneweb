...
export async function archiveArchivistaDocumentRecord(
  ...
  const payload = sanitizeValue({
    ...args.basePayload,
    ...(args.archivistaAnalysis != null
      ? { archivistaAnalysis: sanitizeValue(args.archivistaAnalysis) }
      : {}),
...
export async function archiveArchivistaPreventivoRecord(
  ...
  const nuovoPreventivo = sanitizeValue({
    ...
    metadatiMezzo: args.metadatiMezzo
      ...
      : null,
    ...(args.archivistaAnalysis != null
      ? { archivistaAnalysis: args.archivistaAnalysis }
      : {}),
    ...buildDuplicateMetadata(duplicateChoice, duplicateCandidate),
  });
...
export async function updateArchivistaDocumentRecordAnalysis(args: {
  ...
  const refDoc = doc(db, args.targetCollection, args.archiveId);
...
export async function updateArchivistaPreventivoRecordAnalysis(args: {
  ...
  const currentRaw = snapshot.exists() ? snapshot.data() : null;
  const currentPreventivi = Array.isArray(currentRaw?.preventivi) ? currentRaw.preventivi : [];
  const nextPreventivi = currentPreventivi.map((entry) =>
    entry && typeof entry === "object" && normalizeText((entry as Record<string, unknown>).id) === args.archiveId
      ? {
