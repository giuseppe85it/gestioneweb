...
Nota iniziale:
- questa versione del registro e aggiornata allo stato reale del repo al `2026-03-08`;
- nel repository e ora dimostrata una shell NEXT runtime separata, raggiungibile con route dedicate `/next/*`;
- le macro-aree create in questo task sono shell navigabili con placeholder statici coerenti al blueprint, senza lettura o scrittura dati reali;
...
- la macro-area `/next/mezzi-dossier` non e piu solo placeholder generico: e ora la prima area NEXT con struttura UI reale e navigabile, ancora senza dati runtime.
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Shell globale NEXT | SHELL CREATA | RISCRITTURA PULITA | no | no | NEXT | `src/App.tsx`; `src/next/NextShell.tsx`; `src/next/NextAreaPage.tsx`; `src/next/NextCentroControlloPage.tsx`; `src/next/NextMezziDossierPage.tsx`; `src/next/NextOperativitaGlobalePage.tsx`; `src/next/NextIAGestionalePage.tsx`; `src/next/NextStrumentiTrasversaliPage.tsx`; `src/next/nextData.ts`; `src/next/nextAccess.ts`; `src/next/NextRoleGuard.tsx`; `src/next/NextAccessDeniedPage.tsx`; `src/next/NextDriverExperiencePage.tsx`; `src/next/NextRoleLandingRedirect.tsx`; `src/next/next-shell.css` | Shell runtime separata e navigabile creata sotto `/next/*`. Legacy invariata: nessuna route legacy sostituita, nessun dato letto o scritto. `Centro di Controllo`, `Mezzi / Dossier`, `Operativita Globale`, `IA Gestionale` e `Strumenti Trasversali` superano ora il placeholder generico con UI strutturate, sempre senza reader o writer reali. | 2026-03-08 | N/A - patch locale strumenti-trasversali next |
...
| Flotta | IMPORTATO SOLO UI | RIUSO LOGICA + UI NUOVA | no | no | NEXT | `src/next/NextMezziDossierPage.tsx`; `src/next/next-shell.css`; `docs/ui-blueprint/WIREFRAME_LOGICI_NEXT.md` | La macro-area condivisa `/next/mezzi-dossier` mostra ora un ingresso area mezzi reale: ruolo dell'elenco mezzi, filtri previsti, CTA `Apri Dossier` e distinzione netta tra overview flotta e dettaglio targa. Nessun reader mezzi runtime ancora collegato. | 2026-03-08 | N/A - patch locale mezzi-dossier next |
| Dossier Mezzo | IMPORTATO SOLO UI | RIUSO LOGICA + UI NUOVA | no | no | NEXT | `src/next/NextMezziDossierPage.tsx`; `src/next/next-shell.css`; `docs/ui-blueprint/BLUEPRINT_GRAFICO_NEXT.md`; `docs/ui-blueprint/MAPPA_PATTERN_DA_RIUSARE.md` | `/next/mezzi-dossier` chiarisce ora il Dossier come cuore del sistema: header mezzo forte, overview, reminder, pannelli principali, convergenze mezzo-centriche e confine rispetto a `Operativita Globale`. Nessun modulo dossier reale importato e nessun dato runtime letto. | 2026-03-08 | N/A - patch locale mezzi-dossier next |
