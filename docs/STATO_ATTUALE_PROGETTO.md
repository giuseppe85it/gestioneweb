...
- **Readiness Firebase tipizzata con whitelist candidate della nuova IA interna (2026-03-22)**: il backend IA separato espone ora anche prerequisiti condivisi e whitelist candidate non attive per un futuro bridge Firestore/Storage read-only; il repo dimostra ancora blocchi reali su `firebase-admin` governato dal backend IA, credenziali server-side dedicate, `firestore.rules` assente e `storage.rules` in conflitto con l'uso legacy.
