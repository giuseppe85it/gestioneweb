...
const TMP_RIFORNIMENTI_KEY = "@rifornimenti_autisti_tmp";
...
const extractAutistaNome = (record: any): string => {
...
const toUpperText = (value: unknown): string => safeText(value).toUpperCase();
...
const normalizeSegnalazioneRecord = (record: any, index: number): SegnalazioneRow => {
  const dataTs = toTimestamp(record?.data);
  const fallbackTs = toTimestamp(record?.timestamp);
  const ts = dataTs || fallbackTs;
  const statoRaw = safeText(record?.stato);
  const letta = typeof record?.letta === "boolean" ? record.letta : null;
  const targaPrimary = toUpperText(record?.targa || record?.targaCamion || "-") || "-";
  const targaFilterKey = [
    normalizeTargaFilter(record?.targa),
    normalizeTargaFilter(record?.targaCamion),
    normalizeTargaFilter(record?.targaRimorchio),
  ]
    .filter(Boolean)
    .join(" ");

  return {
    id: safeText(record?.id) || `segn_${ts || index}`,
    ts,
    ...
    tipo: safeText(record?.tipoProblema ?? record?.tipo) || "-",
    descrizione: safeText(record?.descrizione) || "-",
    stato: statoRaw ? statoRaw.toUpperCase() : "-",
    letta,
    isNuova: letta === false || statoRaw.toLowerCase() === "nuova",
    fotoCount: Array.isArray(record?.fotoUrls) ? record.fotoUrls.filter(Boolean).length : 0,
  };
};

const normalizeControlloRecord = (record: any, index: number): ControlloRow => {
  const ts = toTimestamp(record?.timestamp);
  const targaMotrice = toUpperText(record?.targaCamion ?? record?.targaMotrice);
  const targaRimorchio = toUpperText(record?.targaRimorchio);
  const check = record?.check && typeof record.check === "object" ? record.check : {};
  const koList = Object.entries(check)
    .filter(([, value]) => value === false)
    .map(([key]) => String(key).toUpperCase());
  const targaFilterKey = [
    normalizeTargaFilter(targaMotrice),
    normalizeTargaFilter(targaRimorchio),
  ]
    .filter(Boolean)
    .join(" ");
  const targaLabel =
  ...
  return {
    id: safeText(record?.id) || `ctrl_${ts || index}`,
    ts,
    dateObj: ts ? new Date(ts) : null,
    targaMotrice: targaMotrice || "-",
    targaRimorchio: targaRimorchio || "-",
    targaLabel,
    targaFilterKey,
    autistaNome: extractAutistaNome(record) || null,
    badgeAutista: safeText(record?.badgeAutista ?? record?.badge) || null,
...
const normalizeRichiestaRecord = (record: any, index: number): RichiestaRow => {
  const ts = toTimestamp(record?.timestamp);
  const statoRaw = safeText(record?.stato);
  const letta = typeof record?.letta === "boolean" ? record.letta : null;
  const targa = toUpperText(record?.targa || record?.targaCamion || "-") || "-";
  const targaFilterKey = [
    normalizeTargaFilter(record?.targa),
    normalizeTargaFilter(record?.targaCamion),
    normalizeTargaFilter(record?.targaRimorchio),
  ...
  return {
    id: safeText(record?.id) || `req_${ts || index}`,
    ts,
    dateObj: ts ? new Date(ts) : null,
    targa,
    targaFilterKey,
    autistaNome: extractAutistaNome(record) || null,
    badgeAutista: safeText(record?.badgeAutista ?? record?.badge) || null,
    testo: safeText(record?.testo) || "-",
    stato: statoRaw ? statoRaw.toUpperCase() : "-",
    letta,
    isNuova: letta === false || statoRaw.toLowerCase() === "nuova",
    hasFoto: Boolean(safeText(record?.fotoUrl)),
  };
};
...
const unwrapList = (raw: any): any[] => {
  ...
  if (Array.isArray(raw.value?.items)) return raw.value.items;
...
const absMinutesDiff = (a: Date, b: Date): number =>
...
const areLitriComparable = (a: number | null, b: number | null): boolean => {
  if (a === null || b === null) return true;
  return Math.abs(a - b) < 0.001;
};

const pickHeuristicTmpIndex = (
  base: RefuelRow,
  tmpRows: RefuelRow[],
  usedTmpIndices: Set<number>
): number | null => {
  let bestIndex: number | null = null;
  let bestDiff = Number.POSITIVE_INFINITY;

  tmpRows.forEach((candidate, idx) => {
    if (usedTmpIndices.has(idx)) return;
    if (candidate.targa !== base.targa) return;
    if (!areLitriComparable(base.litri, candidate.litri)) return;
    const diff = absMinutesDiff(base.dateObj, candidate.dateObj);
  ...
  tmpRows.forEach((candidate, idx) => {
    ...
    if (candidate.targa !== base.targa) return;
    if (!areLitriComparable(base.litri, candidate.litri)) return;
    if (!isSameDay(base.dateObj, candidate.dateObj)) return;
    const diff = absMinutesDiff(base.dateObj, candidate.dateObj);
    if (diff < bestDiff) {
      bestIndex = idx;
      bestDiff = diff;
...
const likelyDuplicateRefuel = (a: RefuelRow, b: RefuelRow): boolean => {
  if (a.originId && b.originId && a.originId === b.originId) return true;
  if (a.targa !== b.targa) return false;
  if (!areLitriComparable(a.litri, b.litri)) return false;
  if (!isSameDay(a.dateObj, b.dateObj)) return false;
  return absMinutesDiff(a.dateObj, b.dateObj) <= 10;
};

const mergeRefuelRows = (dossierRows: RefuelRow[], tmpRows: RefuelRow[]): RefuelRow[] => {
  const usedTmpIndices = new Set<number>();
  const tmpByOriginId = new Map<string, number[]>();

  tmpRows.forEach((tmp, idx) => {
    if (!tmp.originId) return;
    const list = tmpByOriginId.get(tmp.originId) ?? [];
    list.push(idx);
    tmpByOriginId.set(tmp.originId, list);
  });

  const mergedBase = dossierRows.map((base) => {
    let candidateIndex: number | null = null;

    if (base.originId && tmpByOriginId.has(base.originId)) {
      const list = tmpByOriginId.get(base.originId) ?? [];
      candidateIndex =
        list.find((idx) => !usedTmpIndices.has(idx) && tmpRows[idx].targa === base.targa) ?? null;
    }

    if (candidateIndex === null && (base.km === null || base.costo === null)) {
      candidateIndex = pickHeuristicTmpIndex(base, tmpRows, usedTmpIndices);
    }

    if (candidateIndex === null) return base;

    usedTmpIndices.add(candidateIndex);
    const candidate = tmpRows[candidateIndex];

    return {
      ...base,
      autistaNome: base.autistaNome || candidate.autistaNome || null,
      badgeAutista: base.badgeAutista || candidate.badgeAutista || null,
      km: base.km ?? candidate.km,
      costo: base.costo ?? candidate.costo,
      distributore: base.distributore || candidate.distributore,
      note: base.note || candidate.note,
  ...
  tmpRows.forEach((tmp, idx) => {
    if (usedTmpIndices.has(idx)) return;
    const duplicated = mergedBase.some((existing) => likelyDuplicateRefuel(existing, tmp));
    if (!duplicated) mergedBase.push(tmp);
  });

  return mergedBase.sort((a, b) => b.dateObj.getTime() - a.dateObj.getTime());
};

export default function CentroControllo() {
  ...
  const loadScheduledMaintenances = async () => {
    ...
    setMezziError(null);
    try {
      const raw = await getItemSync(STORAGE_MEZZI_KEY);
      const mezzi = unwrapList(raw);
      const today = new Date();

      const mapped = mezzi
        .filter((m) => Boolean(m?.manutenzioneProgrammata))
        .map((m, idx) => {
          const targaNorm = normalizeTarga(m?.targa);
          const rowId = safeText(m?.id) || targaNorm || `mezzo_${idx}`;
          const dataFineRaw = safeText(m?.manutenzioneDataFine);
          const dataFine = parseDateFlexible(dataFineRaw);
          const revisioneDate = parseDateFlexible(m?.dataScadenzaRevisione);
          const { status, daysToDeadline, priority } = evaluateMaintenanceStatus(dataFine, today);
          return {
            id: rowId,
            targa: targaNorm || "-",
            categoria: safeText(m?.categoria) || "-",
            manutenzioneDataFine: dataFine,
            manutenzioneDataFineRaw: dataFineRaw,
            manutenzioneContratto: safeText(m?.manutenzioneContratto),
            manutenzioneKmMax: safeText(m?.manutenzioneKmMax),
        ...
        .map(({ _priority, ...row }) => row as ScheduledMaintenanceRow);
  ...
  const loadRefuels = async () => {
    ...
    try {
      ...
      const tmpRef = doc(db, "storage", TMP_RIFORNIMENTI_KEY);
      const [dossierSnap, tmpSnap] = await Promise.all([getDoc(dossierRef), getDoc(tmpRef)]);

      const dossierList = unwrapList(dossierSnap.exists() ? dossierSnap.data() : null);
      const tmpList = unwrapList(tmpSnap.exists() ? tmpSnap.data() : null);

      const dossierRows = dossierList
        .map((item, idx) => normalizeRefuelRecord(item, idx, "dossier"))
        .filter((item): item is RefuelRow => Boolean(item));
      const tmpRows = tmpList
        .map((item, idx) => normalizeRefuelRecord(item, idx, "tmp"))
        .filter((item): item is RefuelRow => Boolean(item));

      const merged = mergeRefuelRows(dossierRows, tmpRows);
      setRefuelRows(merged);
    } catch (err: unknown) {
  ...
  const loadAutistiReadOnlySections = async () => {
    ...
    const [segnalazioniRes, controlliRes, richiesteRes] = await Promise.allSettled([
      getItemSync(SEGNALAZIONI_KEY),
      getItemSync(CONTROLLI_KEY),
      getItemSync(RICHIESTE_KEY),
    ]);

    if (segnalazioniRes.status === "fulfilled") {
      const rows = unwrapList(segnalazioniRes.value)
        .map((item, idx) => normalizeSegnalazioneRecord(item, idx))
        .sort((a, b) => b.ts - a.ts);
      setSegnalazioniRows(rows);
      setSegnalazioniError(null);
    } else {
      const message =
        segnalazioniRes.reason instanceof Error
          ? segnalazioniRes.reason.message
    ...
    if (controlliRes.status === "fulfilled") {
      const rows = unwrapList(controlliRes.value)
        .map((item, idx) => normalizeControlloRecord(item, idx))
        .sort((a, b) => b.ts - a.ts);
      setControlliRows(rows);
      setControlliError(null);
    } else {
      const message =
        controlliRes.reason instanceof Error
          ? controlliRes.reason.message
          : "Errore caricamento controlli.";
      setControlliRows([]);
      setControlliError(message);
    }
    setLoadingControlli(false);

    if (richiesteRes.status === "fulfilled") {
      const rows = unwrapList(richiesteRes.value)
        .map((item, idx) => normalizeRichiestaRecord(item, idx))
        .sort((a, b) => b.ts - a.ts);
      setRichiesteRows(rows);
      setRichiesteError(null);
    } else {
      const message =
        richiesteRes.reason instanceof Error
          ? richiesteRes.reason.message
          : "Errore caricamento richieste attrezzature.";
      setRichiesteRows([]);
    ...
    <div className="cc-page">
      <div className="cc-shell">
        <header className="cc-header">
          <button className="cc-back" type="button" onClick={() => navigate("/gestione-operativa")}>
