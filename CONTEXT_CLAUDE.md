...
| NEXT IA interna universale | Chat controllata, richieste, sessioni, artifacts, audit, registry universale, handoff IA, retriever Euromecc read-only e capability `Magazzino` ora anche document-driven nella chat libera: allegato -> classificazione automatica -> proposta azione -> CTA verso `Magazzino` / `DA VERIFICARE`, con deroga scrivente stretta solo sulle fatture magazzino (`riconcilia_senza_carico` o `carica_stock_adblue`) verso il modulo canonico `/next/magazzino`; il risultato documento e ora impaginato come scheda dossier gestionale con sezioni leggibili nel modale/chat. | in sviluppo |
...
48. La IA interna della NEXT puo leggere il dominio `Magazzino` tramite il motore unificato sopra `@inventario`, `@materialiconsegnati`, `@cisterne_adblue`, `@documenti_magazzino`, costi materiali derivati e procurement di supporto; la UX chat `/next/ia/interna` e ora attachment-first/document-driven per i documenti `Magazzino`, con classificazione automatica prudente e action card verso `Magazzino` o `DA VERIFICARE` anche senza prompt rigidi. Lato scrittura non esistono writer generali D05 e l'unica deroga attiva resta confinata a `/next/magazzino?tab=documenti-costi` per due soli casi: `riconcilia_senza_carico` su fattura gia coperta da arrivo/materiale e `carica_stock_adblue` su fattura AdBlue con match forte, UDM `lt` e anti-doppio-carico. Resta `DA VERIFICARE` l'audit su prompt/allegati misti e la robustezza del routing documentale su PDF/immagini reali.
